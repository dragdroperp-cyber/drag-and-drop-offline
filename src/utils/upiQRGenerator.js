...
export const generateBillPaymentQR = async (bill, options = {}) => {
  try {
    console.log('🔧 generateBillPaymentQR called with bill:', bill);
    ...
    upiId = upiId.trim();
    console.log('🔧 Using seller UPI ID for QR generation:', upiId);
    ...
    if (bill.splitPaymentDetails && bill.splitPaymentDetails.onlineAmount > 0) {
      ...
      isSplitPayment = true;
      console.log('🔧 Split payment detected - using online amount:', paymentAmount);
    }
    ...
    console.log('🔧 Payment amount for QR code:', paymentAmount);
