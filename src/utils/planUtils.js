...
export const isModuleUnlocked = (moduleName, currentPlan, currentPlanDetails = null) => {
  ...
  if (!currentPlanDetails || !currentPlanDetails.unlockedModules || !Array.isArray(currentPlanDetails.unlockedModules)) {
    console.log('🚫 isModuleUnlocked: No valid plan details or unlockedModules array, denying access to', moduleName);
    return false;
  }

  console.log(`🔍 Checking if "${moduleName}" is unlocked:`, {
    currentPlan,
    unlockedModules: currentPlanDetails.unlockedModules,
    unlockedModulesCount: currentPlanDetails.unlockedModules.length,
    normalizedModuleName: normalizeModuleName(moduleName),
    moduleInList: currentPlanDetails.unlockedModules.some(m => m.toLowerCase().includes(moduleName.toLowerCase()))
  });
  ...
  if (currentPlanDetails && currentPlanDetails.unlockedModules) {
    ...
    if (normalizedDbModules.includes(normalizedModuleName)) {
      console.log(`✅ MODULE UNLOCKED: Direct normalized match found for "${moduleName}" (normalized: "${normalizedModuleName}")`);
      return true;
    }

    console.log(`🔍 Checking exact match (case-insensitive) for "${moduleName}"...`);
    console.log('Available normalized modules:', normalizedDbModules);
    console.log('Original modules:', currentPlanDetails.unlockedModules);
    ...
    console.log(`❌ MODULE LOCKED: "${moduleName}" not found in unlockedModules`);
    return false;
  }

  // No fallback - only current plan's unlockedModules are allowed
  console.log(`❌ MODULE LOCKED: "${moduleName}" - no valid plan details`);
  return false;
};
