...
export const isModuleUnlocked = (moduleName, currentPlan, currentPlanDetails = null) => {
  // Always allow access to upgrade page, dashboard, settings, and marketing
...
export class DataCreationManager {
  ...
  async createCustomer(customerData) {
    ...
    try {
      ...
      const syncResult = await syncData('customers', [customerWithId], this.state.currentUser?.sellerId);

      // If sync successful, update IndexedDB with _id and isSynced: true
      if (syncResult && syncResult.success && syncResult.data && syncResult.data.results && syncResult.data.results.success && syncResult.data.results.success.length > 0) {
        const successItem = syncResult.data.results.success[0];
        if (successItem.id === customerWithId.id) {
          const { updateItem } = await import('../utils/indexedDB');

          customerWithId._id = successItem._id;
          customerWithId.isSynced = true;

          await updateItem('customers', customerWithId);
      ...
      // Fallback: Dispatch the offline item (already added to IDB)
      // If sync failed, background sync will pick it up later
      this.dispatch({
        type: 'ADD_CUSTOMER',
        payload: customerWithId
      });
  ...
  async createProduct(productData) {
    ...
    try {
      ...
      // Trigger sync and wait for result
      const { syncData } = await import('../utils/api');
      const syncResult = await syncData('products', [productWithId], this.state.currentUser?.sellerId);

      // If sync successful, update IndexedDB with _id and isSynced: true
      if (syncResult && syncResult.success && syncResult.data && syncResult.data.results && syncResult.data.results.success && syncResult.data.results.success.length > 0) {
        const successItem = syncResult.data.results.success[0];
        if (successItem.id === productWithId.id) {
          const { updateItem } = await import('../utils/indexedDB');

          productWithId._id = successItem._id;
          productWithId.isSynced = true;

          await updateItem('products', productWithId);
  ...
  async createOrder(orderData) {
    ...
    try {
      ...
      // Trigger sync and wait for result
      const { syncData } = await import('../utils/api');
      const syncResult = await syncData('orders', [orderWithId], this.state.currentUser?.sellerId);

      // If sync successful, update IndexedDB with _id and isSynced: true
      if (syncResult && syncResult.success && syncResult.data && syncResult.data.results && syncResult.data.results.success && syncResult.data.results.success.length > 0) {
        const successItem = syncResult.data.results.success[0];
        if (successItem.id === orderWithId.id) {
          const { updateItem } = await import('../utils/indexedDB');

          orderWithId._id = successItem._id;
          orderWithId.isSynced = true;

          await updateItem('orders', orderWithId);
