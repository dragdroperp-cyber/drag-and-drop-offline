...
export const backgroundSyncWithBackend = async (dispatch, ActionTypes, options = {}) => {
  ...
  activeSyncPromise = (async () => {
    ...
    try {
      ...
      const { customers, products, productBatches, orders, transactions, purchaseOrders, expenses, categories, customerTransactions, planOrders, suppliers, supplierTransactions, dProducts, settings } = result.data.data;
          ...
          try {
            ...
            if (fullSyncResult.success && fullSyncResult.data?.data) {
              ...
              if (hasActualData) {
                ...
                const { customers, products, productBatches, orders, transactions, purchaseOrders, categories, refunds, plans, planOrders, staff, expenses, customerTransactions, suppliers, supplierTransactions, dProducts, settings } = fullSyncResult.data.data;
...
const updateAllFetchTimes = async () => {
  try {
    const allDataTypes = ['customers', 'products', 'categories', 'orders', 'transactions', 'purchaseOrders', 'refunds', 'plans', 'staff', 'expenses', 'customerTransactions', 'suppliers', 'supplierTransactions', 'dProducts', 'settings'];
...
export const autoRefreshLatestData = async (dispatch, ActionTypes, lastFetchTimesArg) => {
  ...
  try {
    ...
    if (response.data && typeof response.data === 'object') {
      ...
      const dataTypesToUpdate = ['customers', 'products', 'productBatches', 'orders', 'transactions', 'purchaseOrders', 'categories', 'refunds', 'plans', 'expenses', 'customerTransactions', 'suppliers', 'supplierTransactions', 'dProducts', 'settings'];
      const updatedData = {};
    ...
    // Try to fetch coupons if sync was successful
    try {
      // Since we are in dataFetcher, we can call fetchCoupons directly if it wasn't circular.
      // But fetchCoupons is in this file!
      // However, we don't have dispatch here... strictly speaking we return data to the caller (manualRefresh)
      // BUT autoRefreshLatestData takes dispatch as an arg!

      // Let's import fetchCoupons locally to be safe or just define the logic
      // Actually, fetching coupons is a separate GET.
      // We should do it here to ensure it happens with auto-sync.
