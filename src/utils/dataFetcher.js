          ...
          try {
            ...
            if (createResult.success) {
              ...
              if (backendProduct && (backendProduct.id || backendProduct._id)) {
                ...
                // Prepare synced product for local storage
                const syncedProduct = {
                  ...backendProduct,
                  id: backendId,
                  _id: backendId,
...
export const backgroundSyncWithBackend = async (dispatch, ActionTypes, options = {}) => {
  ...
  try {
    ...
    const productsWithBatches = associateBatchesWithProducts(
      (normalizedProducts || []).filter(i => i.isDeleted !== true),
      (normalizedProductBatches || []).filter(i => i.isDeleted !== true)
    );
    ...
    // planOrders no longer stored - using planDetails for limits and usage

    // Last fetch times are already updated above in the main logic
    ...
    if (unsyncedItems.length > 0) {
      ...
      for (const unsyncedItem of unsyncedItems) {
        try {
          ...
          const isStrictIdMatch = itemsToInsert.some(i =>
          ...
          if (isStrictIdMatch) {
            // CRITICAL BUG FIX: If we discard a local unsynced item because a backend version exists, 
            // we MUST delete the local version from IndexedDB to avoid persistent duplicates.
            // This happens most often when a local ID (e.g. 'prod-123') is replaced by a MongoDB ID.
