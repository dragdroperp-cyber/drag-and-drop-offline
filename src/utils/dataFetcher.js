          ...
          try {
            ...
            if (batchResult.success) {
              ...
              if (backendBatch) {
                ...
                // Delete the local record with temporary ID and add the MongoDB record
                // This ensures only the latest authoritative data exists in IndexedDB
                const { deleteItem, addItem, STORES } = await import('../utils/indexedDB');

                // First, remove the local record with temporary ID
                try {
                  await deleteItem(STORES.productBatches, batchData.id);

                } catch (deleteError) {
                  //('⚠️ Could not delete local record (might not exist):', deleteError.message);
                }

                // Then add the MongoDB record with MongoDB ID
                const mongoBatch = {
                  ...backendBatch,  // Use full MongoDB data
                  id: backendBatch._id || backendBatch.id,  // MongoDB ID as IndexedDB key
        ...
        case 'batch_create_manual':
          ...
          try {
            ...
            if (existingBatchCheck.success && existingBatchCheck.data && existingBatchCheck.data.length > 0) {
              ...
              const syncedBatch = {
                ...batchData,
                id: existingBatch._id || existingBatch.id,
                _id: existingBatch._id || existingBatch.id,
                isSynced: true,
                syncedAt: new Date().toISOString()
              };
              await updateItem(STORES.productBatches, syncedBatch);
            ...
            if (batchResult.success) {
              ...
              if (backendBatch && (backendBatch._id || backendBatch.id)) {
                // Construct the new synced batch record
                const mongoBatch = {
                  ...backendBatch,  // Use full MongoDB data
                  id: backendBatch._id || backendBatch.id,  // MongoDB ID as IndexedDB key
                  _id: backendBatch._id || backendBatch.id,  // MongoDB ID
                  ...
                  // Preserve any local UI state that backend might not return
                  createdAt: backendBatch.createdAt || batchData.createdAt,
                  localId: batchData.id // CRITICAL: Pass localId so Reducer knows which temp batch to replace
                };

                try {
                  // SAFE SYNC PATTERN: Add new verified record FIRST, then delete old temp record
                  // This prevents data loss if the Add fails
                  const { deleteItem, updateItem, STORES } = await import('../utils/indexedDB');

                  console.log('💾 Adding synced batch to DB:', mongoBatch);
                  // Use updateItem (put) instead of addItem to avoid "duplicate batch" error 
                  // caused by the existing temp batch having the same batch number
                  await updateItem(STORES.productBatches, mongoBatch);

                  // Only delete the temp record if the new one was added successfully
                  console.log('🗑️ Deleting temp batch:', batchData.id);
                  try {
                    await deleteItem(STORES.productBatches, batchData.id);
                  } catch (deleteError) {
                    console.warn('⚠️ Could not delete local temp record:', deleteError.message);
                  }
                  ...
                  console.warn('⚠️ Add synced batch failed:', addError.message);
                }
              } else {
                // Fallback: just mark as synced
                const syncedBatch = {
            ...
            if (poResult.success) {
              ...
              if (backendOrder) {
                ...
                const mongoOrder = {
                  ...
                  id: backendOrder._id || backendOrder.id,  // MongoDB ID as IndexedDB key
                ...
                const { getAllItems, deleteItem, addItem, STORES } = await import('../utils/indexedDB');
                const ordersBefore = await getAllItems(STORES.purchaseOrders);

                ordersBefore.forEach(o => (`  - ${o.id} (${o.supplierName}, synced: ${o.isSynced})`));

                // Find the exact temp order
                const tempOrder = ordersBefore.find(o => o.id === operation.data.orderData.id);

                // ('🔍 Available order IDs:', ordersBefore.map(o => o.id));

                // Check for similar IDs (debugging)
                const similarIds = ordersBefore.filter(o => o.id.includes('PO_') || o.supplierName === operation.data.orderData.supplierName);
                //('🔍 Similar orders found:', similarIds.map(o => ({ id: o.id, supplier: o.supplierName })));

                if (tempOrder) {

                  try {
                    await deleteItem(STORES.purchaseOrders, operation.data.orderData.id);

                    // Verify deletion
                    const ordersAfterDelete = await getAllItems(STORES.purchaseOrders);
                    const tempStillExists = ordersAfterDelete.some(o => o.id === operation.data.orderData.id);

                    if (tempStillExists) {

                      // Force cleanup: remove all temp orders with same supplier (likely duplicates)

                      const duplicateTempOrders = ordersAfterDelete.filter(o =>
                        o.id.startsWith('PO_') &&
                  ...
                  // Check if there are other temp orders with same supplier that should be cleaned up
                  const otherTempOrders = ordersBefore.filter(o =>
                    o.id.startsWith('PO_') &&
                    o.supplierName === operation.data.orderData.supplierName &&
                    o.id !== mongoOrder.id
                  );

                  if (otherTempOrders.length > 0) {

                    for (const tempOrder of otherTempOrders) {
                      try {
                        await deleteItem(STORES.purchaseOrders, tempOrder.id);
                ...
                try {
                  await addItem(STORES.purchaseOrders, mongoOrder);

                } catch (addError) {

                  return; // Exit if we can't add the MongoDB order
                }
          ...
          try {
            ...
            if (createResult.success) {
              ...
              if (backendProduct && (backendProduct.id || backendProduct._id)) {
                ...
                // Update IndexedDB: Delete temp, Add real
                const { deleteItem, addItem, STORES } = await import('../utils/indexedDB');

                try {
                  await deleteItem(STORES.products, tempId);
                } catch (e) {
                  // Ignore if temp doesn't exist
                }

                await addItem(STORES.products, syncedProduct);

                // Update State: Remove temp, Add real
                if (dispatch && ActionTypes) {
                  ...
                  dispatch({
                    ...
                    payload: { ...syncedProduct, skipAutoSync: true } // Skip auto sync to avoid loop
                  });
                }
              }
            }
          ...
          if (expenseResult.success) {
            ...
            if (operation.data.id && String(operation.data.id).startsWith('temp_')) {
              try {
                ...
                await addItem(STORES.expenses, { ...addedExpense, isSynced: true });
              } catch (e) {
                // Fallback if delete fails (maybe already gone?)
                await addItem(STORES.expenses, { ...addedExpense, isSynced: true });
              }
            } else {
            ...
            if (isTemp) {
              ...
              if (result.success) {
                ...
                if (backendData && (backendData.id || backendData._id)) {
                  // Success - replace local temp item with backend item
                  const newItem = {
                    ...backendData,
                  ...
                  // Delete temp and add new
                  await deleteItem(config.store, tempId);
                  await addItem(config.store, newItem);
                }
              }
            } else {
              // Update operation
...
export const backgroundSyncWithBackend = async (dispatch, ActionTypes, options = {}) => {
  ...
  try {
    ...
    const { customers, products, productBatches, orders, transactions, purchaseOrders, expenses, categories, customerTransactions } = result.data.data;
    ...
    const syncResults = await Promise.all([
      ...
      // planOrders are no longer stored in IndexedDB - using planDetails instead
    ]);
    ...
    // planOrders are no longer tracked for sync - using planDetails instead
...
export const fetchAllData = async () => {
  ...
  // Fetch from IndexedDB (offline or backend failed)
  const [customers, products, orders, transactions, purchaseOrders, categories, expenses, customerTransactions] = await Promise.all([
    getAllItems(STORES.customers).catch(() => []),
    getAllItems(STORES.products).catch(() => []),
    getAllItems(STORES.orders).catch(() => []),
    getAllItems(STORES.transactions).catch(() => []),
  ...
  return {
    ...
    categories: categories || [],
    expenses: expenses || [],
    customerTransactions: customerTransactions || [] // Ensure customerTransactions is returned
  };
};
...
const fetchAllDataWithDeltaSync = async (options = {}) => {
  ...
  try {
    ...
    const dataTypesToLoad = ['customers', 'products', 'orders', 'transactions', 'purchaseOrders', 'categories', 'refunds', 'plans', 'planOrders', 'staff', 'expenses', 'customerTransactions'];
          ...
          try {
            ...
            if (fullSyncResult.success && fullSyncResult.data?.data) {
              ...
              if (hasActualData) {
                ...
                const { customers, products, orders, transactions, purchaseOrders, categories, refunds, plans, planOrders, staff, customerTransactions } = fullSyncResult.data.data;
...
const updateAllFetchTimes = async () => {
  try {
    const allDataTypes = ['customers', 'products', 'categories', 'orders', 'transactions', 'purchaseOrders', 'refunds', 'plans', 'staff', 'expenses', 'customerTransactions'];
...
const syncToIndexedDB = async (storeName, backendItems, options = {}) => {
  ...
  try {
    ...
    for (const backendItem of backendItems) {
      ...
      //(`🔄[syncToIndexedDB] Using key for ${ storeName }: ${ key } (from ${ normalizedItem._id ? '_id' : 'id' })`);
    ...
    if (unsyncedItems.length > 0) {
      ...
      for (const unsyncedItem of unsyncedItems) {
        try {
          // Check if this unsynced item is already present in the backend items (match by natural keys)
          // CRITICAL CHANGE: Only check for strict ID Match. 
          // Previous semantic check (isDuplicateInBatch) was deleting valid new items that shared names with existing items.
          const isStrictIdMatch = finalItemsToInsert.some(i =>
          ...
          if (isStrictIdMatch) {
            // CRITICAL BUG FIX: Only delete if the IDs are different (e.g. temp ID replaced by Mongo ID)
            // If IDs are the same, we simply want to OVERWRITE the backend data with our protected local data.
            // Previous logic blindly deleted identical IDs, causing data loss.
            ...
            if (matchingBackendItem && String(matchingBackendItem.id) !== String(unsyncedItem.id)) {
              // IDs differ (e.g. Local 'temp-1' vs Backend 'real-1'), so delete the old local key
              console.log(`[syncToIndexedDB] 🗑️ Cleaning up replaced local ID ${unsyncedItem.id} in favor of ${matchingBackendItem.id}`);
            ...
            if (matchingBackendItem) {
              const localTime = unsyncedItem.updatedAt ? new Date(unsyncedItem.updatedAt).getTime() : 0;
              const serverTime = matchingBackendItem.updatedAt ? new Date(matchingBackendItem.updatedAt).getTime() : 0;

              if (serverTime >= localTime) {
                // Backend is up to date. Discard local unsynced copy in favor of backend.
                // console.log(`[syncToIndexedDB] ⏩ Skipping re-insertion of ${unsyncedItem.id} - Backend is up to date`);
                discardedCount++;
                continue;
              }
            }
...
export const compareSyncTimestamps = (mongoTracking, indexedDBTimes) => {
  ...
  const dataTypes = ['customers', 'products', 'categories', 'orders', 'transactions', 'purchaseOrders', 'refunds', 'plans', 'planOrders', 'staff', 'expenses'];
...
export const autoRefreshLatestData = async () => {
  ...
  try {
    ...
    //('🔄 AUTO REFRESH: Current time for comparison:', new Date().toISOString());
    ...
    const result = await deduplicateRequest('auto-refresh-sync', async () => {
      console.log('🔄 AUTO REFRESH: Making API request to /data/all');
    ...
    if (response.data && typeof response.data === 'object') {
      ...
      const dataTypesToUpdate = ['customers', 'products', 'productBatches', 'orders', 'transactions', 'purchaseOrders', 'categories', 'refunds', 'plans', 'expenses', 'customerTransactions'];
      const updatedData = {};
    ...
    for (const orderItem of order.items) {
      ...
      if (product) {
        ...
        const updatedProduct = {
          ...
          isSynced: product.isSynced // Preserve sync status (don't mark as unsynced for batch deductions)
        };
