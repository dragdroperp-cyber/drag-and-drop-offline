...
export const backgroundSyncWithBackend = async (dispatch, ActionTypes, options = {}) => {
  ...
  try {
    ...
    const { customers, products, productBatches, orders, transactions, purchaseOrders, categories } = result.data.data;
    ...
    const normalizedProducts = (products || []).map(product => {
      ...
      if (normalized.categoryId && categoryMap[normalized.categoryId]) {
        ...
        normalized.category = normalized.category || '';
        if (normalized.categoryId) {
          //(`❌ BACKGROUND SYNC: Category NOT resolved for ${product.name}: categoryId=${normalized.categoryId}, available categories:`, Object.keys(categoryMap));
        }
      }
      return normalized;
    });
      ...
      if (auth) {
        ...
        //
              ...
              if (hasActualData) {
                ...
                const { customers, products, orders, transactions, purchaseOrders, categories, refunds, plans, staff } = fullSyncResult.data.data;
                ...
                allData.plans = plans || [];
                // allData.planOrders removed - using planDetails for limits and usage
                allData.staff = staff || [];
...
const updateAllFetchTimes = async () => {
  try {
    const allDataTypes = ['customers', 'products', 'categories', 'orders', 'transactions', 'purchaseOrders', 'refunds', 'plans', 'staff'];
...
export const autoRefreshLatestData = async () => {
  ...
  try {
    ...
    if (response.data && typeof response.data === 'object') {
      ...
      const dataTypesToUpdate = ['customers', 'products', 'productBatches', 'orders', 'transactions', 'purchaseOrders', 'categories', 'refunds', 'plans'];
      const updatedData = {};
