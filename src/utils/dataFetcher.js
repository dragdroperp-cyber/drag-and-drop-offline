        ...
        case 'batch_create_manual':
          ...
          try {
            ...
            if (batchResult.success) {
              ...
              //(`❌ Manual batch creation failed (will retry): ${operation.data.batchId}`, batchResult);
...
export const backgroundSyncWithBackend = async (dispatch, ActionTypes, options = {}) => {
  ...
  try {
    ...
    const syncResults = await Promise.all([
      ...
      syncToIndexedDB(STORES.expenses, expenses || [], { merge: true }), // Add expenses sync
...
export const fetchAllLatestData = async (lastFetchTime) => {
  try {
    ...
    const latestData = result.data || {};
    //('🔄 FETCH ALL LATEST: Received data:', Object.keys(latestData));
...
export const autoRefreshLatestData = async () => {
  ...
  try {
    ...
    //('🔄 AUTO REFRESH: Request body:', JSON.stringify(requestBody, null, 2));
  ...
  try {
    ...
    for (const orderItem of order.items) {
      ...
      for (const batch of productBatches) {
        ...
        const batchReduction = Math.min(remainingQuantity, batch.quantity || 0);
        if (batchReduction > 0) {
    ...
    // Update IndexedDB with the changes
    if (updatedProducts.length > 0) {
      await syncToIndexedDB(STORES.products, updatedProducts, { merge: true });
    }

    if (updatedBatches.length > 0) {
      await syncToIndexedDB(STORES.productBatches, updatedBatches, { merge: true });
    }
