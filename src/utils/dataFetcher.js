...
export const backgroundSyncWithBackend = async (dispatch, ActionTypes, options = {}) => {
  ...
  try {
    ...
    const { customers, products, productBatches, orders, transactions, purchaseOrders, expenses, categories, customerTransactions, planOrders, suppliers, supplierTransactions } = result.data.data;
          ...
          try {
            ...
            if (fullSyncResult.success && fullSyncResult.data?.data) {
              ...
              if (hasActualData) {
                ...
                const { customers, products, productBatches, orders, transactions, purchaseOrders, categories, refunds, plans, planOrders, staff, expenses, customerTransactions, suppliers, supplierTransactions } = fullSyncResult.data.data;
...
const updateAllFetchTimes = async () => {
  try {
    const allDataTypes = ['customers', 'products', 'categories', 'orders', 'transactions', 'purchaseOrders', 'refunds', 'plans', 'staff', 'expenses', 'customerTransactions', 'suppliers', 'supplierTransactions'];
  ...
  try {
    ...
    for (const backendItem of backendItems) {
      ...
      const itemToInsert = {
        ...
        // Use backend isSynced status if provided, otherwise default to true for backend data
...
export const compareSyncTimestamps = (mongoTracking, indexedDBTimes) => {
  ...
  const dataTypes = ['customers', 'products', 'categories', 'orders', 'transactions', 'purchaseOrders', 'refunds', 'plans', 'planOrders', 'staff', 'expenses', 'suppliers', 'supplierTransactions'];
...
export const autoRefreshLatestData = async (dispatch, ActionTypes, lastFetchTimesArg) => {
  ...
  try {
    ...
    if (response.data && typeof response.data === 'object') {
      ...
      const dataTypesToUpdate = ['customers', 'products', 'productBatches', 'orders', 'transactions', 'purchaseOrders', 'categories', 'refunds', 'plans', 'expenses', 'customerTransactions', 'suppliers', 'supplierTransactions'];
      const updatedData = {};
