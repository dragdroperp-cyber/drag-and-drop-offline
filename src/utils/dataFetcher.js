...
const normalizeCustomer = (customer) => {
  ...
  if ((customer.dueAmount !== undefined && customer.balanceDue === undefined) ||
    ...
    ('Normalized customer (fixed missing field):', {
    ...
    const normalizedProducts = (products || []).map(product => {
      ...
      if (normalized.categoryId && categoryMap[normalized.categoryId]) {
        ...
        if (normalized.categoryId) {
          //(`❌ FAST LOAD: Category NOT resolved for ${product.name}: categoryId=${normalized.categoryId}, available categories:`, Object.keys(categoryMap));
        }
      }
      return normalized;
    });
...
const fetchAllDataWithDeltaSync = async (options = {}) => {
  ...
  try {
    ...
    for (const [dataType, deltaData] of Object.entries(data || {})) {
      ...
      (`🔄 DELTA SYNC: ${dataType} hasValidData check:`, {
    ...
    if (shouldDoFullSync) {
      ...
      if (auth) {
        ...
        //
            ('🔄 DELTA SYNC: API full sync result:', {
      ...
      if (storeName === STORES.orders) {
        ...
        if ('splitPaymentDetails' in backendItem) {
          ...
          if (backendItem.paymentMethod === 'split') {
            (`[syncToIndexedDB] Preserving splitPaymentDetails for order ${itemToInsert.id}:`, {
...
export const autoRefreshLatestData = async () => {
  try {
    ...
    if (response.data && typeof response.data === 'object') {
      ...
      const dataTypesToUpdate = ['customers', 'products', 'productBatches', 'orders', 'transactions', 'purchaseOrders', 'categories', 'refunds', 'plans', 'planOrders'];
      const updatedData = {};
