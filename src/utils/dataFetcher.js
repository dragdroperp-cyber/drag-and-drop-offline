...
export const backgroundSyncWithBackend = async (dispatch, ActionTypes, options = {}) => {
  ...
  try {
    ...
    const { customers, products, productBatches, orders, transactions, purchaseOrders, expenses, categories } = result.data.data;
...
export const fetchAllData = async () => {
  ...
  // Fetch from IndexedDB (offline or backend failed)
  const [customers, products, orders, transactions, purchaseOrders, categories, expenses] = await Promise.all([
    getAllItems(STORES.customers).catch(() => []),
    getAllItems(STORES.products).catch(() => []),
    getAllItems(STORES.orders).catch(() => []),
    getAllItems(STORES.transactions).catch(() => []),
...
const fetchAllDataWithDeltaSync = async (options = {}) => {
  ...
  try {
    ...
    const dataTypesToLoad = ['customers', 'products', 'orders', 'transactions', 'purchaseOrders', 'categories', 'refunds', 'plans', 'planOrders', 'staff', 'expenses'];
...
const updateAllFetchTimes = async () => {
  try {
    const allDataTypes = ['customers', 'products', 'categories', 'orders', 'transactions', 'purchaseOrders', 'refunds', 'plans', 'staff', 'expenses'];
...
const isDuplicateInBatch = (storeName, itemToInsert, itemsToInsert) => {
  if (storeName === STORES.products) {
    ...
    return itemsToInsert.some(o => {
      // Check for exact ID match first
      if (o.id === itemToInsert.id || o._id === itemToInsert.id || o.id === itemToInsert._id) return true;
    ...
    return itemsToInsert.some(po => {
      // Check for exact ID match first
      if (po.id === itemToInsert.id || po._id === itemToInsert.id || po.id === itemToInsert._id) return true;
...
export const autoRefreshLatestData = async () => {
  ...
  try {
    ...
    if (response.data && typeof response.data === 'object') {
      ...
      const dataTypesToUpdate = ['customers', 'products', 'productBatches', 'orders', 'transactions', 'purchaseOrders', 'categories', 'refunds', 'plans', 'expenses'];
      const updatedData = {};
