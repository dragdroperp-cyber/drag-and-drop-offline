...
const DB_VERSION = 34; // Bump for purchaseOrders payment fields support
    ...
    if (storeName === STORES.products) {
      ...
      if (duplicateProduct) {
        console.warn('⚠️ Duplicate product detected in IndexedDB (skipping add):', {
          ...
          existingIsSynced: duplicateProduct.isSynced,
          existingIsDeleted: duplicateProduct.isDeleted,
          newId: item.id
        });
...
export const syncToIndexedDB = async (items, storeName, forceRefresh = false) => {
  ...
  try {
    ...
    for (const item of items) {
      ...
      const existing = existingMap.get(item._id) || existingMap.get(item.id);
      ...
      if (existing) {
        ...
        // Check for duplicate by content or mapping (specifically for new items with changed IDs)
        const duplicate = findDuplicate(item);
        if (duplicate) {
          // If we found a local duplicate, handle the ID swap or overwrite
          if (!duplicate.isSynced || forceRefresh) {
            console.log(`[Sync] Found local version for ${storeName} (Local ID: ${duplicate.id}, Mongo ID: ${item._id}). Cleaning up local entry.`);
            // Important: we delete the old ID but the new itemToSync already has the new ID
