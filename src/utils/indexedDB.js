...
export const isIndexedDBAvailable = async () => {
  ...
  try {
    ...
    indexedDBAvailabilityCheckPromise = new Promise((resolve) => {
      ...
      testDB.onerror = () => {
        console.warn('⚠️ IndexedDB test failed - likely private browsing or storage disabled');
        indexedDBAvailabilityCache = false;
      ...
      setTimeout(() => {
        ...
        if (now - lastTimeoutWarningTime > 30000) {
          console.warn('⚠️ IndexedDB availability check timed out - but operations may still work');
    ...
    console.warn('⚠️ IndexedDB availability check failed:', error);
    indexedDBAvailabilityCache = false;
    indexedDBAvailabilityCheckPromise = null;
    return false;
  }
...
const DB_VERSION = 25; // Force migration for lastFetchTime
...
const withIndexedDB = async (operation, fallback = null) => {
  try {
    ...
    if (!db) {
      console.log(`ℹ️ IndexedDB unavailable - using fallback`);
      return fallback;
    }
    return await operation(db);
  } catch (error) {
    console.warn('⚠️ IndexedDB operation failed:', error);
    return fallback;
  }
};
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onerror = () => {
      ...
      if (errorMessage.includes('Internal error opening backing store') ||
        ...
        console.warn('⚠️ IndexedDB storage error - app will continue with in-memory data only');
    ...
    request.onupgradeneeded = (event) => {
      ...
      if (!db.objectStoreNames.contains(STORES.orders)) {
        ...
        if (oldVersion < 17) {
          console.log('[IndexedDB Migration v17] Orders store now supports splitPaymentDetails');
      ...
      if (oldVersion < 22 && db.objectStoreNames.contains(STORES.syncTracking)) {
        ...
        clearRequest.onsuccess = () => {
          console.log('🧹 Cleared syncTracking store for simplified schema');
        };
      }
      ...
      if (oldVersion < 24 && db.objectStoreNames.contains(STORES.syncTracking)) {
        console.log('🔄 Version 24 migration: syncTracking store should have correct schema');
      ...
      if (oldVersion < 25 && db.objectStoreNames.contains(STORES.syncTracking)) {
        console.log('🔄 Version 25 migration: Clearing syncTracking store to ensure clean state');
        const syncTrackingStore = transaction.objectStore(STORES.syncTracking);
        const clearRequest = syncTrackingStore.clear();
        clearRequest.onsuccess = () => {
          console.log('🧹 Cleared syncTracking store in version 25 migration');
        };
        clearRequest.onerror = (event) => {
          console.error('❌ Failed to clear syncTracking store:', event);
        };
      }
    ...
    request.onerror = () => {
      console.error('❌ IndexedDB upgrade failed:', request.error);
...
export const addItem = async (storeName, item, skipValidation = false) => {
  ...
  if (!db.objectStoreNames.contains(storeName)) {
    ...
    console.warn(`⚠️ Store "${storeName}" does not exist. Attempting to trigger database upgrade...`);
    console.log('Current database version:', db.version);
    console.log('Expected database version:', DB_VERSION);
    ...
    try {
      ...
      if (!db.objectStoreNames.contains(storeName)) {
        ...
        console.error('Database version after reopen:', db.version);
        ...
        console.log(`✅ Store "${storeName}" created after database upgrade`);
      }
    } catch (closeError) {
      console.error('Error closing/reopening database:', closeError);
    ...
    return new Promise((resolve, reject) => {
      ...
      const promises = items.map(item => {
        return new Promise((res, rej) => {
          ...
          request.onerror = () => {
            console.error(`[IndexedDB] Error storing item ${item.id} in ${storeName}:`, request.error);
            rej(request.error);
          };
        });
      });
...
export const updateMultipleItems = async (storeName, items, skipValidation = false) => {
  console.log(`🔄 [updateMultipleItems] Starting update for ${storeName} with ${items.length} items`);
  ...
  if (!skipValidation) {
    ...
    if (validator) {
      ...
      if (validationErrors.length > 0) {
        console.error(`❌ [updateMultipleItems] Validation failed for ${storeName}:`, validationErrors);
        return Promise.reject(new Error(`Validation failed: ${validationErrors.join('; ')}`));
      }
    }
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    console.log(`🔄 [updateMultipleItems] Created transaction for ${storeName}`);

    const promises = items.map((item, index) => {
      return new Promise((res, rej) => {
        console.log(`🔄 [updateMultipleItems] Putting item ${index} into ${storeName}:`, {
          id: item.id,
        ...
        request.onsuccess = (event) => {
          console.log(`✅ [updateMultipleItems] Successfully put item ${index} into ${storeName}, result:`, event.target.result);
          res(event.target.result);
        };
        request.onerror = (event) => {
          console.error(`❌ [updateMultipleItems] Failed to put item ${index} into ${storeName}:`, request.error, event);
          rej(request.error);
        };
      });
    });
    ...
    transaction.oncomplete = (event) => {
      console.log(`✅ [updateMultipleItems] Transaction completed successfully for ${storeName}`);
      resolve();
    };
    transaction.onerror = (event) => {
      console.error(`❌ [updateMultipleItems] Transaction failed for ${storeName}:`, transaction.error, event);
      reject(transaction.error);
    };

    Promise.all(promises).catch((error) => {
      console.error(`❌ [updateMultipleItems] Promise.all failed for ${storeName}:`, error);
      reject(error);
    });
  });
};
...
export const clearAllItems = async (storeName) => {
  try {
    if (!(await isIndexedDBAvailable())) {
      console.warn(`⚠️ [clearAllItems] IndexedDB unavailable - skipping clear for ${storeName}`);
      return; // Skip operation gracefully
    }
    ...
    console.warn(`⚠️ [clearAllItems] IndexedDB error for ${storeName} - skipping clear:`, error.message);
...
export const exportData = async () => {
  try {
    const [customers, products, orders, transactions, purchaseOrders, categories, refunds, plans, planOrders, activities] = await Promise.all([
...
export const importData = async (data, clearExisting = true) => {
  try {
    ...
    if (data.refunds && Array.isArray(data.refunds) && data.refunds.length > 0) {
      await addMultipleItems(STORES.refunds, data.refunds);
      imported.refunds = data.refunds.length;
    }
 ...
 * @returns {Promise<any>} - The key of the saved permissions
 */

/**
 * Get staff permissions from IndexedDB
 * @param {string} staffId - Staff member ID
 * @returns {Promise<Object|null>} - Permissions object or null if not found
 */

/**
 * Update staff permissions in IndexedDB (merge with existing)
 * @param {string} staffId - Staff member ID
 * @param {Object} newPermissions - New permissions to merge
 * @param {string} sellerId - Seller ID
 * @returns {Promise<any>} - The key of the updated permissions
 */
...
/**
 * Get sync tracking data for a specific data type
 */
export const getSyncTracking = async (dataType) => {
  try {
    console.log(`🔍 Getting sync tracking for ${dataType}`);
    const result = await getItem(STORES.syncTracking, dataType);
    console.log(`🔍 Sync tracking result for ${dataType}:`, result);
    return result;
  } catch (error) {
...
/**
 * Update sync tracking for a specific data type
 */
export const updateSyncTracking = async (dataType, updates) => {
  try {
    console.log(`🔄 updateSyncTracking called for ${dataType} with updates:`, updates);

    const db = await openDB();
    return new Promise((resolve, reject) => {
      const transaction = db.transaction([STORES.syncTracking], 'readwrite');
      const store = transaction.objectStore(STORES.syncTracking);

      // First try to get existing record
      const getRequest = store.get(dataType);

      getRequest.onsuccess = () => {
        const existing = getRequest.result;
        console.log(`🔄 Existing tracking for ${dataType}:`, existing);

        const trackingData = {
          dataType,
          lastFetchTime: updates.lastFetchTime || existing?.lastFetchTime || null
        };

        console.log(`🔄 Final tracking data for ${dataType}:`, trackingData);

        const putRequest = store.put(trackingData);
        putRequest.onsuccess = () => {
          console.log(`✅ Successfully saved tracking data for ${dataType}`);
          resolve(trackingData);
        };
        putRequest.onerror = () => {
          console.error('❌ Error putting tracking data:', putRequest.error);
          reject(putRequest.error);
        };
      };

      getRequest.onerror = () => {
        console.error('Error getting existing tracking data:', getRequest.error);
...
/**
 * Get all sync tracking data
 */
export const getAllSyncTracking = async () => {
  try {
    return await getAllItems(STORES.syncTracking);
  } catch (error) {
    console.error('Error getting all sync tracking:', error);
    return [];
  }
};

/**
 * Update last fetch time for a data type
 */
export const updateLastFetchTime = async (dataType, lastFetchTime = new Date().toISOString()) => {
  try {
    console.log(`🔄 Updating last fetch time for ${dataType}:`, lastFetchTime);
    const result = await updateSyncTracking(dataType, { lastFetchTime });
    console.log(`✅ Updated last fetch time for ${dataType}:`, result);
    return result;
  } catch (error) {
...
/**
 * Get last fetch times object for API calls
 * Sets default starting time if lastFetchTime is null (for new users)
 */
export const getLastFetchTimesForAPI = async () => {
  try {
    ...
    // Default starting time: 24 hours ago (to get recent data for new users)
    const defaultStartTime = new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString();

    allTracking.forEach(tracking => {
      // If lastFetchTime is null, set default starting time
      result[tracking.dataType] = tracking.lastFetchTime || defaultStartTime;
    });

    console.log('🔄 Last fetch times for API (with defaults):', result);
    return result;
  } catch (error) {
    console.error('Error getting last fetch times for API:', error);
    return {};
  }
};

/**
 * Clear sync tracking for a data type (useful for full resync)
 */
export const clearSyncTracking = async (dataType) => {
  try {
    await deleteItem(STORES.syncTracking, dataType);
    return true;
  } catch (error) {
...
/**
 * Initialize sync tracking for all data types
 */
export const initializeSyncTracking = async () => {
  try {
    const dataTypes = ['customers', 'products', 'productBatches', 'categories', 'orders', 'planOrders', 'refunds', 'transactions', 'purchaseOrders', 'plans', 'staff'];

    for (const dataType of dataTypes) {
      const existing = await getSyncTracking(dataType);
      if (!existing) {
        await updateSyncTracking(dataType, {
          lastFetchTime: null
        });
      }
    }

    console.log('✅ Sync tracking initialized for all data types');
  } catch (error) {
    console.error('Error initializing sync tracking:', error);
  }
};

/**
 * Ensure ERP_DB database exists and create it if needed
 * Call this before operations that require the database
 */
export const ensureDatabaseReady = async () => {
  console.log('🔍 Ensuring ERP_DB database is ready...');

  try {
    // Try to open the database - this will create it if it doesn't exist
    const db = await openDB();

    if (db) {
      console.log('✅ ERP_DB database is ready for operations');
      db.close();
      return true;
    } else {
      console.log('⚠️ ERP_DB database creation failed (browser storage restrictions)');
      return false;
    }
  } catch (error) {
    console.error('❌ Failed to ensure ERP_DB database is ready:', error);
    return false;
  }
};

/**
 * Reset database flags (call on logout/login)
 */
export const resetDatabaseFlags = () => {
  databaseMissingLogoutTriggered = false;
  indexedDBStorageRestricted = false;
  localStorage.removeItem('indexeddb_failed');
  localStorage.removeItem('indexeddb_last_error');
  console.log('🔄 All database flags reset');
};
