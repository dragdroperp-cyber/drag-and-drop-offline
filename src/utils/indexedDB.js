...
const DB_VERSION = 27; // Force migration for achievements
  ...
  try {
    ...
    for (const mongoOrder of mongoOrders) {
      ...
      const matchingTempOrders = tempOrders.filter(tempOrder => {
        ...
        console.log(`      Item count match: ${itemCountMatch} (${mongoItemCount} vs ${tempItemCount})`);

        // Match by status
        const statusMatch = tempOrder.status === mongoOrder.status;
        console.log(`      Status match: ${statusMatch} ("${tempOrder.status}" vs "${mongoOrder.status}")`);
...
export const cleanupDuplicateProductBatches = async () => {
  try {
    ...
    // Group batches by productId + batchNumber to avoid deleting batches from different products
    const batchGroups = {};
    allBatches.forEach(batch => {
      const productId = batch.productId || 'none';
      const batchNumber = (batch.batchNumber || '').trim().toLowerCase();

      if (batchNumber) {
        const key = `${productId}_${batchNumber}`;
        if (!batchGroups[key]) {
          batchGroups[key] = [];
        }
        batchGroups[key].push(batch);
      }
    });
