...
export const addItem = async (storeName, item, skipValidation = false) => {
  ...
  try {
    ...
    if (storeName === STORES.products) {
      ...
      // Product Batches: batchNumber (unique per batch)
      const batchNumber = (item.batchNumber || '').trim();

      if (batchNumber) {
        const duplicateBatch = existingItems.find(b => {
          const existingBatchNumber = (b.batchNumber || '').trim();
  ...
  if (!db.objectStoreNames.contains(storeName)) {
    ...
    if (storeName === STORES.syncMetadata) {
      //(`⚠️ syncMetadata store not yet created (database upgrade pending). Using localStorage fallback.`);
    ...
    //('Available stores:', Array.from(db.objectStoreNames));
...
export const getTotalPurchaseOrders = async (startDate = null, endDate = null) => {
  ...
  let filtered = orders.filter(order => {
    ...
    return !order.isDeleted && order.status === 'completed';
  });
...
export const getLastFetchTimesForAPI = async () => {
  try {
    ...
    //('🔄 Last fetch times for API (with defaults):', result);
    return result;
  } catch (error) {
...
export const ensureDatabaseReady = async () => {
  ...
  try {
    ...
    if (db) {
      ...
      //('⚠️ ERP_DB database creation failed (browser storage restrictions)');
      return false;
    }
  } catch (error) {
...
/**
 * Clean up duplicate purchase orders - keep only the latest version (MongoDB ID vs temp ID)
 * @returns {Promise<Object>} - Cleanup results with counts of duplicates found and removed
 */
export const cleanupDuplicatePurchaseOrders = async () => {
  try {

    const allOrders = await getAllItems(STORES.purchaseOrders);
    if (!allOrders || allOrders.length === 0) {

      return { duplicatesFound: 0, removed: 0, kept: 0 };
    }
    ...
    // Separate MongoDB orders from temp orders
    const mongoOrders = [];
    const tempOrders = [];

    allOrders.forEach(order => {
      // Check if it's a MongoDB ObjectId (24-character hex string) OR if _id looks like truncated MongoDB ID
      const hasMongoId = order._id && typeof order._id === 'string' &&
        (order._id.match(/^[a-f\d]{24}$/i) || order._id.match(/^[a-f\d]{1,23}$/i)); // Allow truncated IDs too

      // Check if it's a temp ID (starts with "PO_")
      const isTempId = order.id && typeof order.id === 'string' && order.id.startsWith('PO_');

      if (hasMongoId && !isTempId) {
        mongoOrders.push(order);
      } else if (isTempId) {
        tempOrders.push(order);
      } else {
        // Unclear type - log for debugging
    ...
    // For each MongoDB order, find and remove matching temp orders
    for (const mongoOrder of mongoOrders) {

      // Find temp orders that match this MongoDB order
      const matchingTempOrders = tempOrders.filter(tempOrder => {

        // Match by supplier name and total (within small tolerance)
        const supplierMatch = tempOrder.supplierName === mongoOrder.supplierName;
        const mongoTotal = mongoOrder.total || 0;
        const tempTotal = tempOrder.total || tempOrder.totalCostValue || 0;
        const totalMatch = Math.abs(mongoTotal - tempTotal) < 0.01;

        // Match by item count
        const mongoItemCount = mongoOrder.items?.length || 0;
        const tempItemCount = tempOrder.items?.length || 0;
        const itemCountMatch = mongoItemCount === tempItemCount;
        //(`      Item count match: ${itemCountMatch} (${mongoItemCount} vs ${tempItemCount})`);

        // Match by status
        const statusMatch = tempOrder.status === mongoOrder.status;
        //(`      Status match: ${statusMatch} ("${tempOrder.status}" vs "${mongoOrder.status}")`);

        // All must match for it to be considered a duplicate
        const isMatch = supplierMatch && totalMatch && itemCountMatch && statusMatch;
      ...
      // Remove the matching temp orders
      for (const tempOrder of matchingTempOrders) {
        try {

          await deleteItem(STORES.purchaseOrders, tempOrder.id);
          totalRemoved++;

          // Remove from tempOrders array to avoid double processing
      ...
      totalKept++; // Keep the MongoDB order
    }

    // Keep all remaining temp orders (no MongoDB equivalent found)
    totalKept += tempOrders.length;

    return {
      duplicatesFound: mongoOrders.length > 0 ? mongoOrders.length : 0,
    ...
    return { duplicatesFound: 0, removed: 0, kept: 0, error: error.message };
  }
};

/**
 * Clean up duplicate product batches by batchNumber - keep only one batch per batchNumber
 * @returns {Promise<Object>} - Cleanup results with counts of duplicates found and removed
 */
  ...
  try {
    ...
    allBatches.forEach(batch => {
      ...
      if (batchNumber) {
        ...
        batchGroups[batchNumber].push(batch);
      }
    });

    // Find groups with duplicates
    const duplicateGroups = Object.entries(batchGroups)
      .filter(([batchNumber, batches]) => batches.length > 1)
      .map(([batchNumber, batches]) => ({ batchNumber, batches }));

    if (duplicateGroups.length === 0) {

      return { duplicatesFound: 0, removed: 0, kept: allBatches.length };
    }
    ...
    // For each duplicate group, keep the first one and remove the rest
    for (const group of duplicateGroups) {
      const { batchNumber, batches } = group;

      // Sort by createdAt to keep the oldest (or most recent if no createdAt)
      const sortedBatches = batches.sort((a, b) => {
        const aTime = a.createdAt ? new Date(a.createdAt).getTime() : 0;
        const bTime = b.createdAt ? new Date(b.createdAt).getTime() : 0;
        return aTime - bTime; // Keep the oldest first
      });

      // Keep the first one (oldest), remove the rest
      const toKeep = sortedBatches[0];
      const toRemove = sortedBatches.slice(1);

      // Remove duplicates
      for (const batch of toRemove) {
        try {
          await deleteItem(STORES.productBatches, batch.id);
          totalRemoved++;
