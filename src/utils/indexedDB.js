...
export const isIndexedDBAvailable = async () => {
  ...
  try {
    ...
    indexedDBAvailabilityCheckPromise = new Promise((resolve) => {
      ...
      // Timeout after 3 seconds in case the callbacks don't fire (increased from 1 second)
      setTimeout(() => {
        // Only log timeout warning once every 30 seconds to prevent console spam
        const now = Date.now();
        if (now - lastTimeoutWarningTime > 30000) {
          console.warn('⚠️ IndexedDB availability check timed out');
...
const DB_VERSION = 21; // Incremented to support staff data store
...
export const STORES = {
  ...
  staff: 'staff', // Staff members data
  ...
  staffPermissions: 'staffPermissions', // Staff permissions for offline access
};
...
const openDB = () => {
  return new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = (event) => {
      ...
      // Create Staff Store (MongoDB: sellerId, name, email, permissions, isActive, createdAt, lastActivityDate, invitedBy, inviteToken)
      if (!db.objectStoreNames.contains(STORES.staff)) {
        ...
        staffStore.createIndex('sellerId', 'sellerId', { unique: false }); // MongoDB field
      ...
      // Create Staff Permissions Store (frontend-only for offline staff permissions)
      if (!db.objectStoreNames.contains(STORES.staffPermissions)) {
        const staffPermissionsStore = db.createObjectStore(STORES.staffPermissions, { keyPath: 'id', autoIncrement: false });
        staffPermissionsStore.createIndex('staffId', 'staffId', { unique: true }); // Each staff member has one permissions record
        staffPermissionsStore.createIndex('sellerId', 'sellerId', { unique: false }); // Multiple staff can belong to same seller
        staffPermissionsStore.createIndex('updatedAt', 'updatedAt', { unique: false });
      }
    };
 ...
 */
export const saveStaffPermissions = async (staffId, permissions, sellerId) => {
  ...
  return updateItem(STORES.staffPermissions, permissionsData, true); // Skip validation
};
...
export const getStaffPermissions = async (staffId) => {
  try {
    const permissionsRecord = await getItem(STORES.staffPermissions, `staff_${staffId}`);
    return permissionsRecord ? permissionsRecord.permissions : null;
  } catch (error) {
    console.error('Error getting staff permissions from IndexedDB:', error);
    return null;
  }
};
 ...
 */
export const updateStaffPermissions = async (staffId, newPermissions, sellerId) => {
  try {
    const existingPermissions = await getStaffPermissions(staffId) || {};
    const mergedPermissions = { ...existingPermissions, ...newPermissions };
    return saveStaffPermissions(staffId, mergedPermissions, sellerId);
  } catch (error) {
    console.error('Error updating staff permissions in IndexedDB:', error);
    throw error;
  }
};

/**
 * Save staff member data to IndexedDB
 * @param {Object} staffMember - Staff member object
 * @returns {Promise<any>} - The key of the saved staff member
 */
export const saveStaffMember = async (staffMember) => {
  if (!staffMember || !staffMember._id) {
    throw new Error('Staff member must have an _id');
  }

  const staffData = {
    ...staffMember,
    id: staffMember._id, // Use _id as key for IndexedDB
    updatedAt: new Date().toISOString(),
    isSynced: staffMember.isSynced !== false // Default to true unless explicitly set to false
  };

  return updateItem(STORES.staff, staffData);
};

/**
 * Save multiple staff members to IndexedDB
 * @param {Array} staffMembers - Array of staff member objects
 * @returns {Promise<any[]>} - Array of keys of saved staff members
 */
export const saveStaffMembers = async (staffMembers) => {
  if (!Array.isArray(staffMembers)) {
    throw new Error('staffMembers must be an array');
  }

  const promises = staffMembers.map(staffMember => saveStaffMember(staffMember));
  return Promise.all(promises);
};

/**
 * Get staff member from IndexedDB by ID
 * @param {string} staffId - Staff member ID
 * @returns {Promise<Object|null>} - Staff member object or null if not found
 */
export const getStaffMember = async (staffId) => {
  try {
    return await getItem(STORES.staff, staffId);
  } catch (error) {
    console.error('Error getting staff member from IndexedDB:', error);
    return null;
  }
};

/**
 * Get all staff members for a seller from IndexedDB
 * @param {string} sellerId - Seller ID
 * @returns {Promise<Array>} - Array of staff members
 */
export const getStaffMembers = async (sellerId) => {
  try {
    return await getItemsByIndex(STORES.staff, 'sellerId', sellerId);
  } catch (error) {
    console.error('Error getting staff members from IndexedDB:', error);
    return [];
  }
};

/**
 * Update staff member in IndexedDB
 * @param {string} staffId - Staff member ID
 * @param {Object} updates - Updates to apply
 * @returns {Promise<any>} - The key of the updated staff member
 */
export const updateStaffMember = async (staffId, updates) => {
  try {
    const existingStaff = await getStaffMember(staffId);
    if (!existingStaff) {
    ...
    return updateItem(STORES.staff, updatedStaff);
  } catch (error) {
    console.error('Error updating staff member in IndexedDB:', error);
    throw error;
  }
};

/**
 * Delete staff member from IndexedDB
 * @param {string} staffId - Staff member ID
 * @returns {Promise<boolean>} - True if deleted successfully
 */
export const deleteStaffMember = async (staffId) => {
  try {
    await deleteItem(STORES.staff, staffId);
    return true;
  } catch (error) {
    console.error('Error deleting staff member from IndexedDB:', error);
    return false;
  }
};
