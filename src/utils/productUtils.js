...
export const calculateProductAlerts = (products, lowStockThreshold, expiryDaysThreshold) => {
    ...
    // Calculate Low Stock
    const lowStockProducts = products.filter(p => (Number(p.quantity) || Number(p.stock) || 0) <= lowStockThreshold);
    ...
    const expiryAlerts = products.reduce((acc, product) => {
        ...
        const totalStock = Number(product.quantity) || Number(product.stock) || 0;
