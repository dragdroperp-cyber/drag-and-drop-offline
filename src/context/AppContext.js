      ...
      if (result.success && result.data) {
        ...
        if (result.planDetails) {
          ...
          if (Array.isArray(result.planDetails)) {
            ...
            const activePlans = result.planDetails
              .filter(p => (p.status === 'active' || p.status === 'paid') && p.remainingMs > 0)
              .sort((a, b) => (a.remainingMs || 0) - (b.remainingMs || 0));

            selectedPlan = activePlans.length > 0 ? activePlans[0] : (result.planDetails[0] || {});
          }

          // Merge plan details with usage summary using standard helper
          // The helper expects an object with { summary, plans }
      ...
      if (mergeResult) {
        ...
        if (latestData.planDetails) {
          ...
          if (Array.isArray(latestData.planDetails)) {
            // Select best active plan: Active, Payment Completed, Remaining Time > 0
            // Sort by remaining time ascending (burn shortest first)
            const activePlans = latestData.planDetails
              .filter(p => p.status === 'active' && p.remainingMs > 0)
              .sort((a, b) => (a.remainingMs || 0) - (b.remainingMs || 0));

            selectedPlan = activePlans.length > 0 ? activePlans[0] : (latestData.planDetails[0] || {});
          }
