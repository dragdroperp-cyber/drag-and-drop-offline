  ...
  const manualRefresh = useCallback(async () => {
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        // Handle plan info
        if (collections.planOrders?.data) {
          dispatch({ type: ActionTypes.SET_PLAN_ORDERS, payload: collections.planOrders.data.filter(i => !i.isDeleted) });
        }

        if (result.planDetails) {
          dispatch({ type: ActionTypes.SET_CURRENT_PLAN_DETAILS, payload: result.planDetails });
        }
