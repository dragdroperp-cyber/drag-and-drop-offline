...
const appReducer = (state, action) => {
  ...
  switch (action.type) {
    ...
    case ActionTypes.DELETE_D_PRODUCT: {
      const productToDelete = state.dProducts.find(p => p.id === action.payload);
      if (productToDelete) {
        const deletedProduct = {
          ...productToDelete,
          isDeleted: true,
        ...
        updateInIndexedDB(STORES.dProducts, deletedProduct, true)
          .then(() => {
            if (syncService.isOnline()) {
              syncService.scheduleSync(appStoreFunctions);
            }
        ...
        return {
          ...state,
          dProducts: state.dProducts.filter(p => p.id !== action.payload)
        };
      }
      return state;
    }
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        const {
          ...
          productBatches, categories, refunds, expenses, customerTransactions, planOrders, suppliers, supplierTransactions, dProducts
        } = result.data;
        ...
        if (suppliers) dispatch({ type: ActionTypes.SET_SUPPLIERS, payload: suppliers });
        if (supplierTransactions) dispatch({ type: ActionTypes.SET_SUPPLIER_TRANSACTIONS, payload: supplierTransactions });
        if (dProducts) dispatch({ type: ActionTypes.SET_D_PRODUCTS, payload: dProducts });
        ...
        const errorMsg = result.reason || 'Sync failed';
        dispatch({ type: ActionTypes.SET_MANUAL_REFRESH_ERROR, payload: errorMsg });
        dispatch({ type: ActionTypes.SET_MANUAL_REFRESHING, payload: false });
