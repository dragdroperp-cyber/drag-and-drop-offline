...
const recomputeDerivedData = (state) => {
  try {
    ...
    for (const o of orders) {
      ...
      const orderTotal = Math.max(0, Number(o.totalAmount || 0) - refundedAmount);
  ...
  switch (action.type) {
    ...
    case ActionTypes.LOGOUT:
      ...
      try {
        ...
        // Clear HttpOnly cookie from backend
    ...
    case ActionTypes.SET_CURRENT_PLAN_DETAILS:
      ...
      if (action.payload) {
        ...
        if (sellerId) {
          ...
          const usageSummary = action.payload?.planUsageSummary ||
            calculateRealtimeAggregatedUsage({ ...state, currentPlanDetails: action.payload });
          ...
          const record = {
            ...
            planOrders: action.payload?.planUsagePlans || state.planOrders || [],
...
export const AppProvider = ({ children }) => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (!loadEssentialDataOnly) {
        ...
        [indexedDBCustomers, indexedDBOrders, indexedDBTransactions, indexedDBPurchaseOrders, indexedDBPlanOrders, indexedDBExpenses, indexedDBSuppliers, indexedDBSupplierTransactions, indexedDBDProducts] = await Promise.all(additionalPromises);
      ...
      if (combined) {
        ...
        if (hasChanged) {
          ...
          const record = {
            ...
            sellerId: sellerId,
            data: combined,
            planUsageSummary: combined?.planUsageSummary || null, // Top-level storage for parity with reducer
  ...
  const manualRefresh = useCallback(async () => {
    ...
    const isAtCompleteProfile = typeof window !== 'undefined' && window.location.pathname === '/complete-profile';
    if (state.currentUser && state.currentUser.profileCompleted === false && !isAtCompleteProfile) {
      // Mark session as loaded even for incomplete profiles to prevent repeated attempts
      window.loadDataCalledForSession = true;
    ...
    try {
      ...
      if (syncService.isOnline()) {
        try {
          ...
          const [customers, products, orders, transactions, vOrders, productBatches, customerTransactions, dProductsLocal] = await Promise.all([
      ...
      if (result.success && result.data) {
        ...
        if (result.planDetails) {
          ...
          if (combined) {
            ...
            // CRITICAL: Also update the top-level currentPlan identifier to match the selected plan.
            if (combined.planId) {
              dispatch({ type: ActionTypes.SET_CURRENT_PLAN, payload: combined.planId });
            }

            // Cache the updated plan details in IndexedDB to prevent stale mini plans
            // Ensure sellerId is available
            const sellerId = state.currentUser?.sellerId || state.currentUser?.uid;
            if (sellerId) {
              // Import updateItem dynamically to avoid circular deps if any
              import('../utils/indexedDB').then(({ updateItem }) => {
                updateItem(STORES.planDetails, {
                  sellerId,
                  data: combined,
                  timestamp: Date.now()
                }).catch(e => console.error('Failed to update planDetails in IDB during manual refresh:', e));
              });
            }
          }
        } else {
  ...
  }, [dispatch, state.currentUser?.sellerId, state.currentPlanDetails]);
