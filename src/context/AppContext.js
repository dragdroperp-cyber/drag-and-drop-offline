            ...
            try {
              ...
              if (syncResult.success) {
                ...
                if (updatedCollections.length > 0) {
                  ...
                  if (updatedCollections.includes('refunds')) {
                    reloadPromises.push(getAllItems(STORES.refunds).then(items => {
                      reloadData.refunds = items.filter(i => i.isDeleted !== true);
                    }));
                  }
              ...
              // Fallback to full fetch if incremental sync fails
              try {
                ...
                if (result.success && result.data?.data) {
                  ...
                  // ... (keep existing normalization and update logic)
                  ...
                  const normalizedBackendProducts = (products || []).map(product => {
                    const stock = product.stock !== undefined ? product.stock : (product.quantity !== undefined ? product.quantity : 0);
                    const costPrice = product.costPrice !== undefined ? product.costPrice : (product.unitPrice !== undefined ? product.unitPrice : 0);
                    return { ...product, stock, quantity: stock, costPrice, unitPrice: costPrice };
                  });
                  await Promise.all([
