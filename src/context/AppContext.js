...
const getInitialState = () => {
  ...
  let settingsState = {
    ...
    upiId: '',
    darkMode: false,
    currencyFormat: 'compact', // 'plain' or 'compact' (K format)
  };
  ...
  return {
    ...
    systemStatus: 'online',
    dataFreshness: 'cached', // Start as cached to show IndexedDB data immediately, will change to 'loading' when backend fetch starts
    ...
    currencyFormat: settingsState.currencyFormat || 'compact',
  };
};
...
const appReducer = (state, action) => {
  ...
  switch (action.type) {
    ...
    case ActionTypes.SET_CUSTOMERS:
      ...
      let finalCustomers = normalizedCustomers;
      if (action.merge) {
        const existingMap = new Map(state.customers.map(c => [c.id, c]));
        normalizedCustomers.forEach(c => existingMap.set(c.id, c));
        finalCustomers = Array.from(existingMap.values());
      }

      // Only update if customers array actually changed (prevent unnecessary re-renders)
      if (arraysEqual(state.customers, finalCustomers, 'customers')) {
        return state;
      }
      return {
        ...state,
    ...
    case ActionTypes.SET_PRODUCTS:
      ...
      // }
      {
        let nextProducts = action.payload;

        if (action.merge) {
          const existingMap = new Map(state.products.map(p => [p.id, p]));
          nextProducts.forEach(p => existingMap.set(p.id, p));
          nextProducts = Array.from(existingMap.values());
        }
    ...
    case ActionTypes.DELETE_PRODUCT:
      // Soft delete: Mark as deleted with isSynced: false instead of actually deleting
      const productToDeleteId = action.payload;
      const productToDelete = state.products.find(p => p.id === productToDeleteId || p._id === productToDeleteId);

      if (productToDelete) {
        const exactId = productToDelete.id;
        const exactMongoId = productToDelete._id;
        ...
        updateInIndexedDB(STORES.products, deletedProduct, true) // Skip validation for soft delete
          .then(async () => {
            // ALSO Soft Delete associated batches in IndexedDB
            const batchesToDelete = state.productBatches.filter(b =>
              b.productId === exactId ||
              (exactMongoId && b.productId === exactMongoId) ||
              b.productId === productToDeleteId
            );

            if (batchesToDelete.length > 0) {
              const deletedBatches = batchesToDelete.map(batch => ({
                ...batch,
                isDeleted: true,
                deletedAt: now,
                isSynced: false
            ...
            // Show success toast
            if (window.showToast) {
              window.showToast('Product and associated batches deleted.', 'success');
            }
          })
          .catch(err => {
            console.error('Error deleting product/batches:', err);
            if (window.showToast) {
        ...
        // Remove from state (UI) but keep in IndexedDB for sync
        const nextProducts = state.products.filter(p => p.id !== productToDeleteId && p._id !== productToDeleteId);
        const nextBatches = state.productBatches.filter(b =>
          b.productId !== exactId &&
          (!exactMongoId || b.productId !== exactMongoId) &&
          b.productId !== productToDeleteId
        );

        const derived = recomputeDerivedData({ ...state, products: nextProducts, productBatches: nextBatches });
        return {
          ...state,
          products: nextProducts,
          productBatches: nextBatches,
    ...
    case ActionTypes.ADD_EXPENSE:
      console.log('✅ AppContext: Adding expense to state:', action.payload.id);
      return {
        ...state,
    ...
    case ActionTypes.DELETE_EXPENSE:
      return {
        ...state,
        expenses: state.expenses.filter(e => e.id !== action.payload && e._id !== action.payload)
      };
    ...
    case ActionTypes.UPDATE_EXPENSE:
      return {
        ...
        expenses: state.expenses.map(e =>
          (e.id === action.payload.id || (e._id && e._id === action.payload._id))
            ? { ...e, ...action.payload }
            : e
        )
      };
    ...
    case ActionTypes.SET_CURRENCY_FORMAT: {
      // Update currency format preference
...
export const AppProvider = ({ children }) => {
            ...
            if (refreshResult.success && refreshResult.data) {
              ...
              try {
                ...
                if (planOrdersResult.success && planOrdersResult.data) {
                  ...
                  dispatch({ type: ActionTypes.SET_PLAN_ORDERS, payload: planOrdersData });
                } else {
  ...
  useEffect(() => {
    ...
    const loadUserData = async () => {
      try {
        // Step 1: Load from IndexedDB FIRST (immediate display)
        const [indexedDBCustomers, indexedDBProducts, indexedDBProductBatches, indexedDBOrders, indexedDBTransactions, indexedDBPurchaseOrders, indexedDBCategories, indexedDBPlanDetails, activities, indexedDBExpenses, indexedDBPlanOrders] = await Promise.all([
          getAllItems(STORES.customers).catch(() => []),
          getAllItems(STORES.products).catch(() => []),
        ...
        // Mark data as cached so components can stop showing skeletons
        dispatch({ type: ActionTypes.SET_DATA_FRESHNESS, payload: { freshness: 'cached' } });
    ...
    const initializeAppData = async () => {
      ...
      try {
        ...
        if (isMounted && indexedDBData) {
          ...
          dispatch({ type: ActionTypes.SET_PLAN_ORDERS, payload: (indexedDBData.planOrders || []).filter(i => i.isDeleted !== true) });
  ...
  const collectUnsyncedStores = useCallback(async () => {
    const storesToCheck = [
      ...
      { key: 'expenses', label: 'Expenses', storeName: STORES.expenses }
    ];
  ...
  // Function to force full data refresh
  const refreshData = useCallback(async () => {
    try {
      ...
      // Force full sync by passing override options
      const result = await backgroundSyncWithBackend(dispatch, ActionTypes, { forceFullSync: true, showProgress: true });
      ...
      if (result.success) {
        ...
        if (window.showToast) {
          window.showToast('Failed to refresh data: ' + errorMsg, 'error');
        }
      }
      return result;
    } catch (error) {
          ...
          if (dataInfo.data && dataInfo.data.length > 0) {
            ...
            if (actionType) {
              dispatch({
                ...
                merge: true // Enable merging for delta updates
              });
            }
          }
        }
  ...
  const value = useMemo(() => ({
    ...
    toggleDarkMode,
  }), [state, derivedMetrics, enhancedDispatch, refreshCurrentPlanDetails, refreshLatestData, refreshData, manualRefresh, debugPlanSwitching, logoutWithDataProtection, toggleDarkMode]);
