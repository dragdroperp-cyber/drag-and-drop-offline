...
const arraysEqual = (arr1, arr2, dataType = 'unknown') => {
  ...
  if (arr1.length !== arr2.length) {
    console.log(`🔍 arraysEqual: Different lengths (${arr1.length} vs ${arr2.length}) - UPDATE NEEDED`);
    return false;
  }
  ...
  for (let i = 0; i < arr1.length; i++) {
    ...
    const item2 = arr2[i];
    if (!item1 || !item2) {
      console.log(`🔍 arraysEqual: Missing item at index ${i} - UPDATE NEEDED`);
      return false;
    }
    if (item1.id !== item2.id) {
      console.log(`🔍 arraysEqual: Different IDs at index ${i} (${item1.id} vs ${item2.id}) - UPDATE NEEDED`);
      return false;
    }
    ...
    if (dataType === 'customers') {
      if (item1.name !== item2.name ||
        ...
        console.log(`🔍 arraysEqual: Different customer data at index ${i} - UPDATE NEEDED`, {
          name: item1.name !== item2.name,
          dueAmount: item1.dueAmount !== item2.dueAmount,
          balanceDue: item1.balanceDue !== item2.balanceDue,
          email: item1.email !== item2.email,
          mobileNumber: item1.mobileNumber !== item2.mobileNumber
        });
        return false;
      }
      ...
      if (item1.name !== item2.name ||
        ...
        console.log(`🔍 arraysEqual: Different product data at index ${i} - UPDATE NEEDED`);
        return false;
      }
      ...
      if (item1.total !== item2.total ||
        ...
        console.log(`🔍 arraysEqual: Different order data at index ${i} - UPDATE NEEDED`);
        return false;
      }
    }
    ...
    if (item1.updatedAt !== item2.updatedAt ||
      ...
      console.log(`🔍 arraysEqual: Different metadata at index ${i} - UPDATE NEEDED`, {
        updatedAt: item1.updatedAt !== item2.updatedAt,
        isSynced: item1.isSynced !== item2.isSynced,
        isDeleted: item1.isDeleted !== item2.isDeleted
      });
      return false;
    }
  }
  console.log(`🔍 arraysEqual: Arrays are equal - NO UPDATE NEEDED`);
  return true;
};
...
const appReducer = (state, action) => {
  ...
  switch (action.type) {
    case ActionTypes.LOGIN: {
      ...
      if (isStaffUser) {
        console.log('👤 Staff user logged in - data will be loaded by components');
      }
    ...
    case ActionTypes.LOGOUT:
      ...
      console.log('🗑️ Deleting entire IndexedDB database on logout...');
      ...
      deleteDatabase().then(() => {
        console.log('✅ IndexedDB database deleted successfully on logout');
      }).catch(error => {
        console.warn('⚠️ Error deleting IndexedDB database on logout:', error);
      });
    ...
    case ActionTypes.DELETE_CUSTOMER:
      ...
      if (customerToDelete) {
        ...
        console.log('🗑️ Customer data to be synced for deletion:', deletedCustomer);
...
export const AppProvider = ({ children }) => {
  ...
  const refreshCurrentPlanDetails = useCallback(async () => {
    ...
    console.log('✅ refreshCurrentPlanDetails: Seller ID verified:', currentSellerId);
    ...
    try {
      ...
      if (combined) {
        ...
        if (combined && combined.status === 'paused' && combined.remainingMs > 0) {
          console.log('🔄 Current plan is paused but still valid, attempting to reactivate...');
          try {
            ...
            if (reactivateResult.success) {
              console.log('✅ Successfully reactivated paused plan:', reactivateResult.data);
              ...
              if (updatedPlanResult.success && updatedUsageResult.success) {
                ...
                if (updatedCombined) {
                  console.log('🔄 Updated plan details after reactivation');
              ...
              console.log('⚠️ Could not reactivate paused plan:', reactivateResult.message);
            }
        ...
        if (combined && (combined.isExpired || combined.status === 'expired' || (combined.remainingMs !== undefined && combined.remainingMs <= 0))) {
          console.log('🔄 Current plan is expired/invalid, attempting to switch to valid alternative...');
          ...
          try {
            ...
            if (switchResult.success) {
              console.log('✅ Successfully switched to valid alternative plan:', switchResult.data);
              ...
              if (switchedPlanResult.success && switchedUsageResult.success) {
                ...
                if (switchedCombined) {
                  console.log('🔄 Updated plan details after switching to alternative');
              ...
              console.log('⚠️ Could not switch to alternative plan:', switchResult.message);
            }
          } catch (switchError) {
  ...
  useEffect(() => {
    ...
    setOnSyncCompletedCallback((syncResult) => {
      console.log('[SYNC] Sync completed, triggering status update:', syncResult);
  ...
  useEffect(() => {
    console.log('[AUTH LOAD] useEffect triggered, isAuthenticated:', state.isAuthenticated);

    // Only load data if user is authenticated
    if (!state.isAuthenticated) {
      console.log('[AUTH LOAD] User not authenticated, skipping data load setup');
      return;
    }

    // Data loading is now handled by the pageshow event listener
    console.log('[AUTH LOAD] User authenticated - data loading will be triggered by page events');
  ...
  const loadData = useCallback(async () => {
    ...
    console.log('[MANUAL LOAD] Connection detected:', slowConnection ? 'slow' : 'fast');
    ...
    try {
      ...
      if (indexedDBProducts.length > 0) {
        dispatch({ type: ActionTypes.SET_PRODUCTS, payload: indexedDBProducts.filter(i => i.isDeleted !== true) });
      }
      if (indexedDBProductBatches.length > 0) {
      ...
      console.log('[MANUAL LOAD] About to check isOnline status...');
      const isOnlineStatus = await isOnline();
      console.log('[MANUAL LOAD] isOnline check result:', isOnlineStatus);
      dispatch({ type: ActionTypes.SET_SYSTEM_STATUS, payload: isOnlineStatus ? 'online' : 'offline' });

      if (isOnlineStatus) {
        console.log('[MANUAL LOAD] ✅ Online - proceeding with API calls');
        try {
          ...
          try {
            console.log('🔄 Starting auto-refresh of latest data...');
            console.log('🔄 About to call autoRefreshLatestData()');
            const refreshResult = await autoRefreshLatestData();
            console.log('🔄 autoRefreshLatestData() returned:', refreshResult);

            // Check if plan is invalid
            if (refreshResult.planInvalid === true) {
              console.log('🚫 Plan invalid - restricting access and redirecting to upgrade page');
            ...
            if (refreshResult.success && refreshResult.data) {
              console.log('✅ Auto-refresh completed:', refreshResult.message);
              console.log('🔄 UI Update: Processing data for Redux:', Object.keys(refreshResult.data));

              // Mark data as fresh immediately after successful delta sync
              console.log('🔄 DELTA SYNC SUCCESS - Setting dataFreshness to fresh');
              dispatch({ type: ActionTypes.SET_DATA_FRESHNESS, payload: { freshness: 'fresh' } });
              ...
              for (const [dataType, dataInfo] of Object.entries(refreshResult.data)) {
                if (dataInfo.data && Array.isArray(dataInfo.data)) {
                  console.log(`🔄 UI Update: Updating ${dataType} with ${dataInfo.data.length} items`);
                  if (dataInfo.data.length > 0) {
                    console.log(`🔄 UI Update: First ${dataType} item:`, dataInfo.data[0]);
                  } else {
                    console.log(`🔄 UI Update: Clearing ${dataType} (empty array received)`);
                  }

                  const actionType = getActionTypeForDataType(dataType);
                  if (actionType) {
                    const filteredData = dataInfo.data.filter(item => item.isDeleted !== true);
                    console.log(`🔄 UI Update: Dispatching ${actionType} for ${dataType} with ${filteredData.length} items (filtered from ${dataInfo.data.length})`);
                    dispatch({
                      type: actionType,
                      payload: filteredData
                    });
                    console.log(`✅ UI Update: Dispatched ${actionType} for ${dataType}`);
                  } else {
                    console.warn(`⚠️ UI Update: No action type found for ${dataType}`);
                  }
                } else {
                  console.log(`🔄 UI Update: No valid data to update for ${dataType}`);
                }
              }
            }
          } catch (refreshError) {
        ...
        console.log('[MANUAL LOAD] ❌ Offline - skipping API calls, using cached data only');
      }
  ...
  const loadAdditionalData = useCallback(async () => {
    if (state.dataFreshness === 'partial') {
      console.log('[LAZY LOAD] Loading additional data on demand');
      ...
      try {
        ...
        console.log('[LAZY LOAD] Additional data loaded successfully');
      } catch (error) {
  ...
  useEffect(() => {
    ...
    const loadUserData = async () => {
      try {
        // Step 1: Load from IndexedDB FIRST (immediate display)
        const [indexedDBCustomers, indexedDBProducts, indexedDBOrders, indexedDBTransactions, indexedDBPurchaseOrders, indexedDBCategories, indexedDBPlanDetails, activities, indexedDBPlanOrders] = await Promise.all([
          getAllItems(STORES.customers).catch(() => []),
  ...
  const testSwitchToValidPlan = useCallback(async () => {
    console.log('🧪 Testing switch to valid plan...');
    try {
      ...
      if (result.success) {
        ...
        await refreshCurrentPlanDetails();
        console.log('✅ Plan switched and details refreshed');
      }
    } catch (error) {
  ...
  useEffect(() => {
    ...
    const initializeAppData = async () => {
      ...
      console.log('🚀 INITIALIZING APP: Loading data instantly from IndexedDB...');
      ...
      try {
        ...
        if (isMounted && indexedDBData) {
          ...
          dispatch({ type: ActionTypes.SET_PRODUCTS, payload: (indexedDBData.products || []).filter(i => i.isDeleted !== true) });
          dispatch({ type: ActionTypes.SET_PRODUCT_BATCHES, payload: (indexedDBData.productBatches || []).filter(i => i.isDeleted !== true) });
          ...
          console.log('✅ APP INITIALIZED: IndexedDB data loaded instantly', {
        ...
        //   console.log('🚀 APP INIT: Starting background sync in 1 second...');
        //   setTimeout(() => {
        //     console.log('🚀 APP INIT: Executing background sync now...');
        //     backgroundSyncWithBackend(dispatch, ActionTypes).then(syncResult => {
        //       console.log('🔄 APP INIT: Background sync result:', syncResult);
        //       if (syncResult.success && isMounted) {
        //         console.log('🔄 Background sync completed after app initialization');
        ...
        //         console.log('⚠️ APP INIT: Background sync failed, keeping cached data');
        //         dispatch({
        //           type: ActionTypes.SET_DATA_FRESHNESS,
        //           payload: { freshness: 'cached', lastSynced: null }
        //         });
    ...
    const handleOnline = async () => {
      ...
      //     }
      //   } else {
      //     console.log('ℹ️ Background sync skipped or failed:', syncResult.reason);
      //     if (isMounted) {
      //       dispatch({
      //         type: ActionTypes.SET_DATA_FRESHNESS,
      //         payload: { freshness: 'cached', lastSynced: null }
  ...
  const refreshLatestData = useCallback(async (dataTypes = null) => {
    try {
      console.log('🔄 REFRESH: Starting latest data refresh for:', dataTypes || 'all types');
      ...
      if (Object.keys(filteredTimestamps).length === 0) {
        console.log('🔄 REFRESH: No valid timestamps found, skipping refresh');
      ...
      if (Object.keys(latestData).length === 0) {
        console.log('🔄 REFRESH: No new data found');
      ...
      if (mergeResult) {
        ...
        console.log('✅ REFRESH: Successfully refreshed latest data');
        return {
          success: true,
  ...
  const value = useMemo(() => ({
    ...
    manualRefresh,
    debugPlanSwitching,
  }), [state, derivedMetrics, enhancedDispatch, refreshCurrentPlanDetails, refreshLatestData, manualRefresh, debugPlanSwitching]);
