...
import syncManager, { performIncrementalSync, performFullSync, resetSyncMetadata, COLLECTION_MAP } from '../services/syncManager';
      ...
      try {
        ...
        if (isOnlineStatus) {
          try {
            ...
            // Step B: Perform universal incremental sync
            const syncResult = await performIncrementalSync();
            
            if (syncResult.success) {
              console.log('✅ Incremental sync completed successfully:', syncResult.summary);

              // Reload data from IndexedDB to update UI with any changes
              const [
                updatedCustomers,
                updatedProducts,
                updatedOrders,
              ...
              // Update UI state with synced data (exclude soft-deleted items)
              dispatch({ type: ActionTypes.SET_CUSTOMERS, payload: (updatedCustomers || []).filter(i => i.isDeleted !== true) });
              dispatch({ type: ActionTypes.SET_PRODUCTS, payload: (updatedProducts || []).filter(i => i.isDeleted !== true) });
              dispatch({ type: ActionTypes.SET_ORDERS, payload: (updatedOrders || []).filter(i => i.isDeleted !== true) });
              dispatch({ type: ActionTypes.SET_TRANSACTIONS, payload: (updatedTransactions || []).filter(i => i.isDeleted !== true) });
              dispatch({ type: ActionTypes.SET_PURCHASE_ORDERS, payload: (updatedPurchaseOrders || []).filter(i => i.isDeleted !== true) });
              dispatch({ type: ActionTypes.SET_CATEGORIES, payload: (updatedCategories || []).filter(i => i.isDeleted !== true) });
              dispatch({ type: ActionTypes.SET_ACTIVITIES, payload: updatedActivities || [] });
