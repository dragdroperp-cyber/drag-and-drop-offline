...
const calculateRealtimeAggregatedUsage = (params) => {
  ...
  const applyDelta = (entityType, items) => {
    ...
    items.forEach(item => {
      ...
      // Condition: Not synced, Not deleted, and NO backend _id (meaning it's purely local)
      const isNewLocal = !item.isSynced && !item.isDeleted && !item._id;
      ...
      const isDeletedLocal = !item.isSynced && item.isDeleted && item._id;

      if (isNewLocal) delta++;
      if (isDeletedLocal) delta--;
    });
  ...
  switch (action.type) {
    ...
    case ActionTypes.ADD_CUSTOMER: {
      ...
      addToIndexedDB(STORES.customers, newCustomer)
        .then(() => {
          registerBackgroundSync();
          if (syncService.isOnline()) {
            syncService.scheduleSync(getStoreFunctions);
          }
    ...
    case ActionTypes.DELETE_CUSTOMER:
      ...
      if (customerToDelete) {
        ...
        // Remove from state (UI) but keep in IndexedDB for sync
        // const nextPlanDetails = adjustPlanUsage(state.currentPlanDetails, { customers: -1 });
        return {
          ...state,
...
export const AppProvider = ({ children }) => {
  ...
  const loadAdditionalData = useCallback(async () => {
    if (state.dataFreshness === 'partial') {
      ...
      try {
        ...
        } catch (e) { console.error('Error lazy loading expenses:', e); }
  ...
  useEffect(() => {
    ...
    const loadUserData = async () => {
      try {
        // Step 1: Load from IndexedDB FIRST (immediate display)
        const [indexedDBCustomers, indexedDBProducts, indexedDBProductBatches, indexedDBOrders, indexedDBTransactions, indexedDBPurchaseOrders, indexedDBCategories, indexedDBPlanDetails, activities, indexedDBPlanOrders, indexedDBExpenses] = await Promise.all([
          getAllItems(STORES.customers).catch(() => []),
          getAllItems(STORES.products).catch(() => []),
