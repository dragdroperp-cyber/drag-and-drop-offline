    ...
    const filteredOrders = useMemo(() => {
        return filterBySeller(state.orders || [])
            ...
            .map(order => {
                ...
                // Sync with Reports/Financial logic for saleMode
                ...
                // Professional GST Logic: The full invoice value (including delivery) is the "Value of Supply" (Composite Supply)
                // This ensures Taxable Value + GST matches the total money actually collected.
                const finalCalculatedTotal = proportionalFactor * originalGrandTotal;

                const correctedItems = filteredItems.map(item => {
                    const itemOriginalTotal = Number(item.totalSellingPrice ?? item.total ?? item.amount ?? item.sellingPrice ?? 0);
                    // Ratio of this specific item's value against the total items in the order
                    const itemRatio = totalItemsSum > 0 ? (itemOriginalTotal / totalItemsSum) : 0;

                    // The "Full Value" of this item including its share of delivery charges and discounts
                    const itemFullValue = itemRatio * originalGrandTotal;
            ...
            .filter(Boolean);
    }, [state.orders, startDate, endDate, searchTerm, saleMode]);
        ...
        <div className="min-h-screen pb-12 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-6 mb-8">
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                    <div className="flex flex-wrap items-center justify-center gap-1 w-full sm:w-auto sm:inline-flex rounded-xl sm:rounded-full border border-slate-200 dark:border-slate-700 bg-white/80 dark:bg-slate-800/80 p-1 shadow-sm backdrop-blur-sm mr-2">
                        <button
                            type="button"
