...
const DProductBarcodePrintModal = ({ isOpen, onClose, initialProduct }) => {
    ...
    return createPortal(
        <div className="fixed inset-0 z-[1200] flex flex-col bg-white dark:bg-slate-900 animate-in fade-in duration-300 overflow-hidden">
            ...
            {/* Footer */}
            <div className="p-6 border-t border-gray-100 dark:border-slate-800 bg-gray-50 dark:bg-slate-900 shrink-0">
                <div className="max-w-4xl mx-auto flex justify-center">
                    {step === 1 ? (
                        <button
                            onClick={handleConfirm}
                            disabled={items.length === 0}
                            className="bg-slate-900 dark:bg-white text-white dark:text-slate-900 px-12 py-4 rounded-2xl font-bold text-lg shadow-2xl hover:scale-[1.02] active:scale-[0.98] transition-all disabled:opacity-50 flex items-center gap-3"
                        >
                            Next: Preview Labels
                        ...
                        <div className="flex gap-4 w-full">
                            <button
                                onClick={() => setStep(1)}
                                className="flex-1 py-4 px-6 border-2 border-gray-200 dark:border-slate-700 dark:text-white font-bold rounded-2xl hover:bg-gray-100 dark:hover:bg-slate-800 transition-all"
                            >
                                Back to Edit
                            </button>
                            <button
                                onClick={handlePrint}
                                className="flex-[2] bg-slate-900 dark:bg-white text-white dark:text-slate-900 py-4 px-12 rounded-2xl font-bold text-lg shadow-2xl hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-3"
                            >
                                <Printer className="w-6 h-6" />
                                Print {items.reduce((sum, item) => sum + (parseInt(item.quantity) || 0), 0)} Labels
                            </button>
                        </div>
                    )}
                </div>
                                ...
                                .map(p => (
                                    ...
                                    >
                                        ...
                                        <div className="mb-auto">
                                            <h4 className="font-bold text-gray-900 dark:text-white text-lg leading-tight mb-2 line-clamp-2">{p.productName}</h4>
