...
const Reports = () => {
  ...
  const exportReportsPDF = async () => {
    try {
      ...
      const brandPrimary = { r: 47, g: 60, b: 126 }; // Website Theme Purple
      const brandAccent = { r: 0, g: 0, b: 0 }; // Black
      const brandPrimaryLight = { r: 71, g: 85, b: 145 }; // Lighter Theme Purple
      const brandPrimaryUltraLight = { r: 248, g: 249, b: 253 }; // Very light (Subtle Blue-White)
      const textPrimary = { r: 0, g: 0, b: 0 }; // Pure Black
      const textSecondary = { r: 75, g: 85, b: 99 }; // Gray-600
      const borderColor = { r: 229, g: 231, b: 235 }; // Gray-200
      ...
      try {
        // Load logo image via fetch to convert to base64
        // Ensure we use the public URL and handle potential 404s returning HTML (SPA fallback)
        const publicUrl = process.env.PUBLIC_URL || '';
        const logoUrl = `${publicUrl}/assets/drag-and-drop-logo-croped.jpg`;

        const response = await fetch(logoUrl);
        if (!response.ok) throw new Error(`Failed to fetch logo: ${response.status} ${response.statusText}`);

        const contentType = response.headers.get('content-type');
        if (contentType && !contentType.startsWith('image/')) {
          throw new Error(`Invalid content type for logo: ${contentType}. Expected an image.`);
        }
        ...
        await new Promise((resolve, reject) => {
          img.onload = resolve;
          img.onerror = (e) => reject(new Error('Image failed to load from base64 data'));
        });

        // Calculate aspect ratio
        const imgRatio = img.width / img.height;
        let logoW = logoMaxSize;
        let logoH = logoMaxSize;
        ...
        // Center vertically in the logo area
        pdf.addImage(base64, 'JPEG', logoX, logoY + (logoMaxSize - logoH) / 2, logoW, logoH);
      } catch (e) {
        console.warn('Logo could not be loaded for PDF:', e.message);
        // Fallback or silently continue (pdf.addImage might fail if format is wrong)
      }

      // App name next to logo
      const logoSpace = logoMaxSize + 6;
      pdf.setTextColor(255, 255, 255);
      pdf.setFont('helvetica', 'bold');
      pdf.setFontSize(20);
      pdf.text('Drag & Drop', logoX + logoSpace, logoY + 7);

      // Underline accent - White
      pdf.setFillColor(255, 255, 255);
      pdf.rect(logoX + logoSpace, logoY + 9, 50, 1, 'F');
      ...
      const summaryCards = [
        { label: 'Total Sales', value: `Rs. ${totalSales.toFixed(2)}`, accentColor: brandPrimary },
        { label: 'Total Orders', value: totalOrders.toString(), accentColor: brandAccent },
        { label: 'Net Profit', value: `Rs. ${netProfit.toFixed(2)}`, accentColor: brandPrimary },
      ...
      const reportData = [
        ...
        ['Sales Summary', 'Total Sales', `Rs. ${totalSales.toFixed(2)}`],
        ['Sales Summary', 'Total Orders', totalOrders.toString()],
        ['Sales Summary', 'Net Profit', `Rs. ${netProfit.toFixed(2)}`],
        ...
        ['Purchase Summary', 'Total Purchase Amount', `Rs. ${totalPurchaseAmount.toFixed(2)}`],
        ['Purchase Summary', 'Pending Orders', pendingPurchaseOrders.toString()],
        ['Payment Summary', 'Cash Received', `Rs. ${totalCashReceived.toFixed(2)}`],
        ['Payment Summary', 'Online Received', `Rs. ${totalOnlineReceived.toFixed(2)}`],
        ['Payment Summary', 'Outstanding Due', `Rs. ${totalOutstandingDue.toFixed(2)}`],
      ];

      // Column widths
      const colWidths = [contentWidth * 0.35, contentWidth * 0.40, contentWidth * 0.25];
      ...
      reportData.forEach((row, idx) => {
        ...
        if (idx === 0) {
          ...
          row.forEach((text, i) => {
            ...
            pdf.text(text, x, currentY - 2, { align: 'center' });
          });
        } else {
          // Table Rows
          ...
          row.forEach((text, i) => {
            ...
            // For the last column (Value), use bold font if desired, but still center it
            if (i === 2) {
              pdf.setFont('helvetica', 'bold');
            } else {
              pdf.setFont('helvetica', 'normal');
    ...
    <div className="space-y-6 fade-in-up">
            ...
            {showExportMenu && (
              <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-gray-200 dark:border-slate-700 py-2 z-50">
                <button
                  ...
                  className="w-full px-4 py-2.5 text-left text-sm hover:bg-gray-50 dark:hover:bg-slate-700/50 text-gray-700 dark:text-slate-200 transition-colors flex items-center gap-3"
                >
      ...
      <div className="card">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-6">
          <div className="stat-card bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-5 shadow-sm hover:shadow-md transition-all duration-300">
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-2xl font-semibold text-slate-900 dark:text-white">{formatCurrency(totalSales)}</p>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-2xl font-semibold text-slate-900 dark:text-white">{totalOrders}</p>
                </div>
              </div>
            </div>
          </div>
          <div className="stat-card bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-5 shadow-sm hover:shadow-md transition-all duration-300">
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  <p className="text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">Net Profit</p>
                  <p className="text-2xl font-semibold text-slate-900 dark:text-white">{formatCurrency(netProfit)}</p>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-2xl font-semibold text-slate-900 dark:text-white">{totalProducts}</p>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-2xl font-semibold text-slate-900 dark:text-white">{lowStockCount}</p>
                </div>
              </div>
            </div>
          </div>
          <div className="stat-card bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-5 shadow-sm hover:shadow-md transition-all duration-300">
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  <p className="text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">Out of Stock</p>
                  <p className="text-2xl font-semibold text-slate-900 dark:text-white">{outOfStockCount}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="stat-card bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-5 shadow-sm hover:shadow-md transition-all duration-300">
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  <p className="text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">Total Purchase Amount</p>
                  <p className="text-2xl font-semibold text-slate-900 dark:text-white">{formatCurrency(totalPurchaseAmount)}</p>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-2xl font-semibold text-slate-900 dark:text-white">{pendingPurchaseOrders}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="stat-card bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-5 shadow-sm hover:shadow-md transition-all duration-300">
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  <p className="text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">Cash Received</p>
                  <p className="text-2xl font-semibold text-slate-900 dark:text-white">{formatCurrency(totalCashReceived)}</p>
                </div>
              </div>
            </div>
          </div>
          <div className="stat-card bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-5 shadow-sm hover:shadow-md transition-all duration-300">
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  <p className="text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">Online Received</p>
                  <p className="text-2xl font-semibold text-slate-900 dark:text-white">{formatCurrency(totalOnlineReceived)}</p>
                </div>
              </div>
            </div>
          </div>
          <div className="stat-card bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-5 shadow-sm hover:shadow-md transition-all duration-300">
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  <p className="text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">Outstanding Due</p>
                  <p className="text-2xl font-semibold text-slate-900 dark:text-white">{formatCurrency(totalOutstandingDue)}</p>
                </div>
              </div>
            </div>
          </div>
