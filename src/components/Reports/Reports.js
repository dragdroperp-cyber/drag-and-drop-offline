...
const Reports = () => {
  ...
  // Calculate gross profit from confirmed sales (Net Revenue - COGS)
  const grossProfit = totalSales - totalCogs;

  // Net Profit = (Net Revenue - COGS) - Petty Expenses
  // totalSales is already net of discounts and excludes delivery charges.
  // Pending profit is NOT added here; it's tracked separately.
  ...
  const hourlySalesChartData = useMemo(() => {
    ...
    filteredOrders.forEach(order => {
      ...
      if (formatDate(orderDate) === selectedDate) {
        ...
        hourlyProfit[hour] += (calculateOrderRevenue(order) - calculateOrderCost(order));
      }
    });
