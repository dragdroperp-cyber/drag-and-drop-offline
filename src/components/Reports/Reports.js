...
const Reports = () => {
  ...
  const filteredOrders = useMemo(() => {
    ...
    return filterBySeller(state.orders || [])
      ...
      .map(order => {
        ...
        // Subtract refunds relevant to the filtered items
        ...
        if (orderRefunds && orderRefunds.length > 0) {
          ...
          orderRefunds.forEach(refund => {
            // If refund has detailed items, use them to subtract precisely
            if (Array.isArray(refund.items) && refund.items.length > 0) {
              refund.items.forEach(refundItem => {
                ...
                if (filteredItems.some(item => {
                  ...
                  // Calculate amount for this item (qty * rate)
                  const itemAmount = (Number(refundItem.qty) || 0) * (Number(refundItem.rate) || 0);
                  refundedAmountForFilteredItems += itemAmount;
                }
              });
            } else {
              // Fallback for old/simple refunds: Proportional deduction
              // If we can't map items, we assume the refund applies proportionally to the filtered view
        ...
        return {
          ...order,
          items: filteredItems,
          totalAmount: finalCalculatedTotal, // Exclude delivery from "Total Sales"
  ...
  const { totalRefunds, totalRefundedCost } = useMemo(() => {
    ...
    filteredOrders.forEach(order => {
      ...
      associatedRefunds.forEach(refund => {
        // Calculate Amount
        // Note: filteredOrders already has `totalAmount` reduced by this. 
        // We just need to sum it up for display "Total Refunds".
        // Use logic consistent with filteredOrders reduction if possible, 
        // but for "Total Refunds" stat, we usually just want the total value of the refunds linked to these orders.
        // However, if we filtered items (DIRECT mode), we should only count refunds for those items.

        if (Array.isArray(refund.items) && refund.items.length > 0) {
          refund.items.forEach(ri => {
            const riPid = normalizeId(ri.productId || ri.product_id || ri._id || ri.id);
            // Check if item exists in the filtered order items
            // Check if item exists in the filtered order items
            // Fallback to name matching for Direct Products if IDs don't match
            ...
            if (originalItem) {
              ...
              const itemRefundAmount = qty * rate;

              refundAmount += itemRefundAmount;

              // Cost Calculation
              const unitCost = Number(originalItem.costPrice || originalItem.purchasePrice || 0); // Need unit cost
              // If item.totalCostPrice is stored, derive unit cost
          ...
          // Simple refund (no items doc) - assume proportional to Order Total vs Filtered Total?
          // Complex. Fallback:
          // If we are in 'normal' mode (most cases), just take the full refund amount if order is present.
          // If 'direct', and refund has no items, we might over-count. 
          // For now, assume simple refunds apply to the whole order.
          const rAmt = Number(refund.totalRefundAmount || refund.amount || 0);
          // Prorate based on filtered items share?? 
          // In filteredOrders we calculated proportionalFactor. We can't easily access it here.
          // Let's assume 100% for now to imply "Refunded Amount".
          refundAmount += rAmt;
          // Cannot accurately estimate cost for simple refund without items. Assume 0 cost adjustment to be safe (conservative profit).
        }
      });
    });
  ...
  const businessWideRefunds = useMemo(() => {
    return filterBySeller(state.refunds || []).filter(refund => {
      const refundDate = new Date(refund.createdAt || refund.date || 0);
  ...
  // Net COGS = Gross COGS - Cost of Refunded Items
