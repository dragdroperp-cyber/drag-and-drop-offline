                            ...
                            <table className="w-full">
                                ...
                                <tbody className="divide-y divide-gray-200 dark:divide-slate-700">
                                    {plans.map((plan) => {
                                        ...
                                        // Sometimes the plan object itself has the ID if it's not a usagePlan structure but a direct plan structure (fallback)
                                        if (!planId) planId = plan.id || plan._id;

                                        // Find matches in the catalog
                                        ...
                                        const displayPrice = typeof price === 'number' ? `₹${price}` : (price || '₹0');
