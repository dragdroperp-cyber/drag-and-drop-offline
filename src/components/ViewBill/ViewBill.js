...
const ViewBill = () => {
    ...
    const generateA4PDF = async () => {
        ...
        try {
            ...
            pdf.setTextColor(...COLORS.text);
            let displayInvNo = order.invoiceNumber || order.invoiceNo || order.billNumber;
            if (!displayInvNo) {
                displayInvNo = (order.id || order._id || '').slice(-6).toUpperCase();
            }
            ...
            const pMethod = (order.paymentMethod || 'PAID').toUpperCase();
    ...
    const generateThermalPDF = async (size) => {
        ...
        try {
            ...
            const dateObj = order.date || order.createdAt ? new Date(order.date || order.createdAt) : new Date();
            ...
            const drawContent = async (pdf) => {
                ...
                // HEADER
                const logoUrl = order.sellerId?.logoUrl;
                if (logoUrl) {
                    try {
                ...
                // Customer Info
                const displayCustomerName = order.customerName || 'Walk-in Customer';
                pdf.setFontSize(9);
                pdf.setFont('helvetica', 'bold');
                ...
                if (order.customerMobile) {
                    pdf.setFont('helvetica', 'normal');
                    pdf.setFontSize(8);
                    pdf.text(`Mobile no.: ${order.customerMobile}`, margin, y);
                    y += 4;
                }
                ...
                <div className="max-w-4xl mx-auto px-4 py-8">
                    ...
                    <div className="bg-white dark:bg-slate-900 shadow-2xl shadow-slate-200/50 dark:shadow-black/50 border border-slate-100 dark:border-slate-800 rounded-sm overflow-hidden relative">
                        ...
                        <div className="p-6 sm:p-10 space-y-10">
                            ...
                            {order.paymentMethod?.toLowerCase() === 'split' && order.splitPaymentDetails && (
                                <div className="bg-slate-50 dark:bg-slate-800/50 p-6 rounded-sm border border-slate-100 dark:border-slate-700">
                                    ...
                                    <div className="grid grid-cols-3 gap-4">
                                        {order.splitPaymentDetails.cashAmount > 0 && (
                                            <div>
                                                <p className="text-[10px] text-slate-500 uppercase font-bold mb-1">Cash</p>
                                                <p className="text-base font-black text-slate-900 dark:text-white">₹{Number(order.splitPaymentDetails.cashAmount).toFixed(2)}</p>
                                            </div>
                                        )}
                                        {order.splitPaymentDetails.onlineAmount > 0 && (
                                            <div>
                                                <p className="text-[10px] text-slate-500 uppercase font-bold mb-1">Online</p>
                                                <p className="text-base font-black text-slate-900 dark:text-white">₹{Number(order.splitPaymentDetails.onlineAmount).toFixed(2)}</p>
                                            </div>
                                        )}
                                        {order.splitPaymentDetails.dueAmount > 0 && (
                                            <div>
                                                <p className="text-[10px] text-red-500 uppercase font-bold mb-1">Due</p>
                                                <p className="text-base font-black text-red-600">₹{Number(order.splitPaymentDetails.dueAmount).toFixed(2)}</p>
                                            </div>
                                        )}
                                    </div>
                                </div>
                            ...
                            <div>
                                ...
                                <div className="sm:hidden space-y-3">
                                    ...
                                    <div className="mt-4 pt-4 border-t border-slate-200 dark:border-slate-700 flex justify-between items-center">
                                        ...
                                        <span className="font-black text-slate-900 dark:text-white text-xl">₹{Number(order.totalAmount).toFixed(2)}</span>
                                    </div>
                                </div>
                            </div>
