...
import { useApp, ActionTypes, associateBatchesWithProducts, isPlanExpired } from '../../context/AppContext';
import {
  RotateCcw,
  Search,
  X,
...
const Refunds = () => {
  ...
  const handleProcessRefund = async () => {
    ...
    // Validate batch selection (must select a batch if multiple exist)
    const missingBatchSelection = itemsToRefund.find(item =>
      item.batches.length > 0 && !item.selectedBatchId
    );

    if (missingBatchSelection) {
      if (window.showToast) {
        window.showToast(`Please select a batch for ${missingBatchSelection.name}`, 'warning');
      }
      return;
    }
    ...
    try {
      ...
      // STEP 2: Update product stock and batches locally (increase stock for refunded items)
      // This increases stock in both STORES.products and STORES.productBatches
      const inventoryResult = await updateInventoryAfterRefund(refund);

      if (inventoryResult.success && dispatch) {
        // OPTIMISTIC UI: Immediately update state with new stock levels

        // 1. Update product batches state first (with merge)
        ...
        // To ensure consistency, we merge the updated products with existing ones
        // AND then re-associate with all current batches
        if (inventoryResult.updatedProducts?.length > 0) {
          // Merge updated products with current state products
          ...
          // Merge updated batches with current state batches for association
          ...
          // Re-associate to ensure the 'batches' property on products is up to date
  ...
  return (
    <div className="space-y-4 sm:space-y-6 px-2 sm:px-0 pb-6">
      {/* Header */}
      <div className="bg-gradient-to-r from-[#2f3c7e] to-[#18224f] dark:from-slate-800 dark:to-slate-900 rounded-2xl p-4 sm:p-6 shadow-lg">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs sm:text-sm text-blue-100 dark:text-slate-300 mt-1">Process refunds and manage return requests</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex border-b-2 border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 rounded-t-xl overflow-hidden shadow-sm">
        <button
          ...
          className={`flex-1 px-3 sm:px-4 py-3 font-semibold text-xs sm:text-sm transition-all relative ${activeTab === 'search'
            ? 'text-[#2f3c7e] dark:text-blue-400 bg-blue-50 dark:bg-slate-700/50'
            : 'text-gray-600 dark:text-slate-400 hover:text-[#2f3c7e] dark:hover:text-blue-400 hover:bg-gray-50 dark:hover:bg-slate-700'
            }`}
        >
          {activeTab === 'search' && (
            <div className="absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r from-[#2f3c7e] to-[#18224f] dark:from-blue-500 dark:to-indigo-500"></div>
          )}
        ...
        <div className="space-y-4 sm:space-y-6">
          ...
          <div className="bg-white dark:bg-slate-800 rounded-xl shadow-md p-4 sm:p-6 border border-gray-100 dark:border-slate-700">
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-xs sm:text-sm font-semibold text-gray-700 dark:text-slate-300 mb-2">Search By</label>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-3 rounded-xl border-2 border-gray-200 dark:border-slate-600 bg-white dark:bg-slate-700/50 focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-500 dark:focus:border-blue-400 transition-all text-sm sm:text-base text-gray-900 dark:text-white placeholder:text-gray-400 dark:placeholder:text-slate-500"
                />
              </div>

              {/* Refund Window Info */}
              <div className="bg-gradient-to-r from-blue-50 to-slate-50 dark:from-blue-900/20 dark:to-slate-900/20 border-2 border-blue-200 dark:border-blue-800/50 rounded-xl p-3 sm:p-4">
                <p className="text-xs sm:text-sm text-slate-700 dark:text-slate-300 flex items-start gap-2">
                  <AlertCircle className="h-4 w-4 text-slate-600 dark:text-slate-400 flex-shrink-0 mt-0.5" />
          ...
          {!isSearching && eligibleOrders.length > 0 && (
            <div className="bg-white dark:bg-slate-800 rounded-xl shadow-md p-4 sm:p-6 border border-gray-100 dark:border-slate-700">
              <h3 className="text-base sm:text-lg font-bold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
                <CheckCircle className="h-5 w-5 text-[#2f3c7e] dark:text-indigo-400" />
                Eligible Orders <span className="text-indigo-600 dark:text-indigo-400">({eligibleOrders.length})</span>
              </h3>
                    ...
                    <div
                      ...
                      className={`border-2 rounded-xl p-3 sm:p-4 cursor-pointer transition-all ${hasRefund
                        ...
                        : isSelected
                          ...
                          : 'border-gray-200 dark:border-slate-700 hover:border-blue-300 dark:hover:border-blue-500/50 hover:bg-gradient-to-r hover:from-gray-50 hover:to-blue-50/30 dark:hover:bg-slate-700/50'
                        }`}
                    ...
                    >
                      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
                        ...
                        <div className="text-left sm:text-right flex-shrink-0">
                          ...
                          <p className={`text-xs uppercase font-medium ${hasRefund ? 'text-red-600 dark:text-red-400' : 'text-[#2f3c7e] dark:text-blue-400'}`}>
          ...
          {!isSearching && searchTerm.trim() && eligibleOrders.length === 0 && (
            <div className="bg-white dark:bg-slate-800 rounded-xl shadow-md p-6 sm:p-8 border border-gray-100 dark:border-slate-700">
              <div className="text-center py-8">
                ...
                <p className="text-gray-700 dark:text-slate-300 font-medium">No eligible orders found</p>
          ...
          {selectedOrder && (
            ...
            >
              <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl max-w-5xl w-full max-h-[95vh] sm:max-h-[90vh] overflow-hidden flex flex-col border dark:border-slate-700">
                {/* Header */}
                <div className="bg-gradient-to-r from-[#2f3c7e] to-[#18224f] dark:from-slate-800 dark:to-slate-900 px-4 sm:px-6 py-4 flex items-center justify-between border-b border-white/10 dark:border-slate-700">
                ...
                <div className="overflow-y-auto flex-1 p-4 sm:p-6">
                  ...
                  <div className="bg-gradient-to-r from-gray-50 to-slate-50 dark:from-slate-700/50 dark:to-slate-800/50 rounded-xl p-4 mb-4 border border-gray-200 dark:border-slate-700">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 text-sm">
                      <div>
                        <p className="text-xs text-gray-600 dark:text-slate-400 font-medium mb-1">Order ID</p>
                        <p className="font-bold text-gray-900 dark:text-white font-mono">{(selectedOrder.id || selectedOrder._id || '').toString().slice(-8)}</p>
                      </div>
                      <div>
                        <p className="text-xs text-gray-600 dark:text-slate-400 font-medium mb-1">Customer</p>
                        <p className="font-bold text-gray-900 dark:text-white">{selectedOrder.customerName || 'Walk-in Customer'}</p>
                      </div>
                      <div>
                        <p className="text-xs text-gray-600 dark:text-slate-400 font-medium mb-1">Date</p>
                      ...
                      <div>
                        <p className="text-xs text-gray-600 dark:text-slate-400 font-medium mb-1">Total Amount</p>
                        <p className="font-bold text-indigo-600 dark:text-indigo-400 text-lg" title={formatCurrency(selectedOrder.totalAmount || selectedOrder.total)}>
                    ...
                    <div className="mb-4">
                        ...
                        <div className="inline-block min-w-full align-middle">
                          <table className="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
                            <thead className="bg-gradient-to-r from-blue-50 to-slate-50 dark:from-slate-700/50 dark:to-slate-800/50">
                              <tr>
                                <th className="px-4 py-3 text-left text-xs font-bold uppercase tracking-wide text-slate-700 dark:text-slate-300">Product</th>
                                <th className="px-4 py-3 text-center text-xs font-bold uppercase tracking-wide text-slate-700 dark:text-slate-300">Ordered</th>
                                <th className="px-4 py-3 text-center text-xs font-bold uppercase tracking-wide text-slate-700 dark:text-slate-300">Refunded</th>
                                <th className="px-4 py-3 text-center text-xs font-bold uppercase tracking-wide text-slate-700 dark:text-slate-300">Available</th>
                                <th className="px-4 py-3 text-left text-xs font-bold uppercase tracking-wide text-slate-700 dark:text-slate-300">Batch</th>
                                <th className="px-4 py-3 text-right text-xs font-bold uppercase tracking-wide text-slate-700 dark:text-slate-300">Rate</th>
                                <th className="px-4 py-3 text-center text-xs font-bold uppercase tracking-wide text-slate-700 dark:text-slate-300">Refund Qty</th>
                                <th className="px-4 py-3 text-right text-xs font-bold uppercase tracking-wide text-slate-700 dark:text-slate-300">Total</th>
                              </tr>
                            </thead>
                                ...
                                >
                                    ...
                                    {item.batches && item.batches.length > 0 ? (
                                      item.batches.length > 1 ? (
                                        <select
                                          ...
                                          className="w-full min-w-[120px] px-2 py-1.5 text-xs font-semibold rounded-lg border-2 border-indigo-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-gray-900 dark:text-white focus:border-indigo-500 focus:outline-none"
                                        >
                                  ...
                                  <td className="px-4 py-3 text-center">
                                    <input
                                      ...
                                      className={`w-20 px-2 py-1.5 text-sm border-2 rounded-lg text-center font-semibold ${item.availableQty === 0
                                        ? 'bg-gray-100 dark:bg-slate-700 cursor-not-allowed text-gray-400 dark:text-slate-500 border-gray-200 dark:border-slate-600'
                                        : 'border-blue-300 dark:border-blue-500/50 focus:border-blue-500 dark:focus:border-blue-400 focus:ring-2 focus:ring-blue-500/30 text-gray-900 dark:text-white bg-white dark:bg-slate-700'
                                        }`}
                                    />
                                  </td>
                        ...
                        {refundItems.map((item) => (
                          ...
                          >
                            <div className="flex items-start justify-between mb-3">
                              ...
                              <span className="text-xs font-semibold text-[#2f3c7e] dark:text-blue-400 bg-blue-50 dark:bg-slate-700/50 px-2 py-1 rounded" title={formatCurrency(item.rate)}>
                                {formatCurrencySmart(item.rate, state.currencyFormat)}/{item.unit}
                              </span>
                            </div>
                            <div className="grid grid-cols-2 gap-2 mb-3 text-xs">
                              <div>
                                <p className="text-gray-500 dark:text-slate-400">Ordered</p>
                                <p className="font-semibold text-gray-900 dark:text-white">{item.orderedQty} {item.unit}</p>
                              </div>
                              <div>
                                <p className="text-gray-500 dark:text-slate-400">Refunded</p>
                              ...
                              <div>
                                <p className="text-gray-500 dark:text-slate-400">Available</p>
                                <p className="font-bold text-indigo-600 dark:text-indigo-400">{item.availableQty} {item.unit}</p>
                              </div>
                              <div>
                                <p className="text-gray-500 dark:text-slate-400">Line Total</p>
                                <p className="font-bold text-indigo-600 dark:text-indigo-400" title={formatCurrency(item.refundQty * item.rate)}>{formatCurrencySmart(item.refundQty * item.rate, state.currencyFormat)}</p>
                              </div>
                            </div>
                            ...
                            <div className="space-y-3">
                              {item.batches && item.batches.length > 0 && (
                                <div>
                                  <label className="block text-xs font-semibold text-gray-700 dark:text-slate-300 mb-1">Target Batch</label>
                                  {item.batches.length > 1 ? (
                                    <select
                                      value={item.selectedBatchId}
                                      onChange={(e) => handleBatchChange(item.productId, e.target.value)}
                                      className="w-full px-3 py-2 text-sm border-2 rounded-lg font-semibold border-indigo-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-gray-900 dark:text-white"
                                    >
                                      <option value="">Select Batch</option>
                                      {item.batches.map(batch => (
                                        <option key={batch._id || batch.id} value={batch._id || batch.id}>
                              ...
                              <div>
                                <label className="block text-xs font-semibold text-gray-700 dark:text-slate-300 mb-1">Refund Quantity</label>
                                <input
                                  type="number"
                                  min="0"
                                  max={item.availableQty}
                                  ...
                                  onChange={(e) => handleRefundQtyChange(item.productId, e.target.value)}
                                  disabled={item.availableQty === 0}
                                  className={`w-full px-3 py-2 text-sm border-2 rounded-lg text-center font-semibold ${item.availableQty === 0
                                    ? 'bg-gray-100 dark:bg-slate-700 cursor-not-allowed text-gray-400 dark:text-slate-500 border-gray-200 dark:border-slate-600'
                                    : 'border-blue-300 dark:border-blue-500/50 focus:border-blue-500 dark:focus:border-blue-400 focus:ring-2 focus:ring-blue-500/30 text-gray-900 dark:text-white bg-white dark:bg-slate-700'
                                    }`}
                                />
                      ...
                      <div className="mt-4 sm:mt-6 space-y-4">
                        <div className="flex justify-end">
                          <div className="bg-gradient-to-r from-[#2f3c7e] to-[#18224f] dark:from-slate-800 dark:to-slate-900 rounded-xl p-4 sm:p-6 shadow-lg w-full sm:w-auto">
                            <p className="text-xs sm:text-sm text-blue-100 dark:text-slate-300 mb-1 font-medium">Total Refund Amount</p>
                        ...
                        <div>
                          <label className="block text-xs sm:text-sm font-semibold text-gray-700 dark:text-slate-300 mb-2">
                            Refund Reason <span className="text-gray-400 dark:text-slate-500 font-normal">(Optional)</span>
                          </label>
                          <textarea
                            value={refundReason}
                            ...
                            rows={3}
                            className="w-full px-4 py-3 rounded-xl border-2 border-gray-200 dark:border-slate-600 bg-white dark:bg-slate-700/50 focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-500 dark:focus:border-blue-400 transition-all text-sm sm:text-base text-gray-900 dark:text-white placeholder:text-gray-400 dark:placeholder:text-slate-500"
                          />
                        </div>
                        ...
                        <div className="flex flex-col sm:flex-row justify-end gap-3 pt-2">
                          ...
                          <button
                            ...
                            className={`w-full sm:w-auto px-6 py-3 rounded-xl font-bold transition-all ${isProcessing || totalRefundAmount === 0
                              ? 'bg-gray-300 dark:bg-slate-700 text-gray-500 dark:text-slate-500 cursor-not-allowed'
                              : 'bg-gradient-to-r from-[#2f3c7e] to-[#18224f] dark:from-slate-800 dark:to-slate-900 text-white shadow-lg hover:shadow-xl active:scale-95'
                              }`}
                          >
                            {isProcessing ? (
                  ...
                  {refundItems.length === 0 && (
                    <div className="text-center py-12">
                      ...
                      <p className="text-sm text-gray-600 dark:text-slate-400 font-medium">Loading refund items...</p>
                    </div>
                  )}
                </div>
              </div>
        ...
        <div className="space-y-4 sm:space-y-6">
          ...
          <div className="bg-white dark:bg-slate-800 rounded-xl shadow-md p-4 sm:p-6 border border-gray-100 dark:border-slate-700">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
              <div>
                <label className="block text-xs sm:text-sm font-semibold text-gray-700 dark:text-slate-300 mb-2">From Date</label>
                <input
                  type="date"
                  value={refundFilters.from}
                  onChange={(e) => setRefundFilters(prev => ({ ...prev, from: e.target.value }))}
                  className="w-full px-3 sm:px-4 py-2 text-sm rounded-lg border-2 border-gray-200 dark:border-slate-600 bg-white dark:bg-slate-700/50 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-500 dark:focus:border-indigo-400 text-gray-900 dark:text-white placeholder:text-gray-400 dark:placeholder:text-slate-500"
                />
              </div>
              <div>
                <label className="block text-xs sm:text-sm font-semibold text-gray-700 dark:text-slate-300 mb-2">To Date</label>
                <input
                  type="date"
              ...
              <div>
                <label className="block text-xs sm:text-sm font-semibold text-gray-700 dark:text-slate-300 mb-2">Customer Mobile</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-xs sm:text-sm font-semibold text-gray-700 dark:text-slate-300 mb-2">Order ID</label>
                <input
                  type="text"
          ...
          <div className="bg-white dark:bg-slate-800 rounded-xl shadow-md border border-gray-100 dark:border-slate-700 overflow-hidden">
            <div className="p-4 sm:p-6 border-b border-gray-200 dark:border-slate-700 bg-gradient-to-r from-blue-50 to-slate-50 dark:from-slate-700/50 dark:to-slate-800/50">
              <h2 className="text-base sm:text-lg font-bold text-gray-900 dark:text-white flex items-center gap-2">
                <RotateCcw className="h-5 w-5 text-[#2f3c7e] dark:text-indigo-400" />
                All Refunds <span className="text-indigo-600 dark:text-indigo-400">({allRefunds.length})</span>
              </h2>
            </div>
            ...
            <div className="hidden sm:block overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
                <thead className="bg-gradient-to-r from-blue-50 to-slate-50 dark:from-slate-700/50 dark:to-slate-800/50">
                  <tr>
                    <th className="px-4 py-3 text-left text-xs font-bold uppercase tracking-wide text-slate-700 dark:text-slate-300">Refund ID</th>
                    <th className="px-4 py-3 text-left text-xs font-bold uppercase tracking-wide text-slate-700 dark:text-slate-300">Order ID</th>
                    <th className="px-4 py-3 text-left text-xs font-bold uppercase tracking-wide text-slate-700 dark:text-slate-300">Customer</th>
                    <th className="px-4 py-3 text-right text-xs font-bold uppercase tracking-wide text-slate-700 dark:text-slate-300">Amount</th>
                    <th className="px-4 py-3 text-center text-xs font-bold uppercase tracking-wide text-slate-700 dark:text-slate-300">Items</th>
                    <th className="px-4 py-3 text-left text-xs font-bold uppercase tracking-wide text-slate-700 dark:text-slate-300">Date</th>
                  </tr>
                </thead>
                <tbody className="bg-white dark:bg-slate-800 divide-y divide-gray-200 dark:divide-slate-700">
                  {paginatedRefunds.length > 0 ? (
                    paginatedRefunds.map((refund) => (
                      <tr key={refund.id} className="hover:bg-indigo-50/30 dark:hover:bg-slate-700/50 transition-colors">
                        ...
                        <td className="px-4 py-3 text-sm text-gray-700 dark:text-slate-300 font-mono">
                    ...
                    <tr>
                      <td colSpan="6" className="px-4 py-12 text-center">
                        ...
                        <p className="text-gray-600 dark:text-slate-400 font-medium">No refunds found</p>
                      </td>
                    </tr>
                  )}
                </tbody>
                    ...
                    <div className="flex items-start justify-between mb-2">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <span className="text-xs text-gray-500 dark:text-slate-500">Order: {(refund.orderId || '').toString().slice(-8)}</span>
                        </div>
              ...
              <div className="px-4 sm:px-6 py-4 border-t border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-800/50">
                <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                  <div className="text-xs sm:text-sm text-gray-700 dark:text-slate-300 font-medium">
                    Showing <span className="font-bold text-indigo-600 dark:text-indigo-400">{startIndex + 1}</span> to{' '}
                    <span className="font-bold text-indigo-600 dark:text-indigo-400">{Math.min(startIndex + itemsPerPage, allRefunds.length)}</span> of{' '}
                    <span className="font-bold text-indigo-600 dark:text-indigo-400">{allRefunds.length}</span> refunds
                  </div>
      ...
      {showSuccessModal && successData && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl max-w-md w-full overflow-hidden border dark:border-slate-700">
            ...
            <div className="p-6 space-y-4">
              <div className="bg-gray-50 dark:bg-slate-700/50 rounded-xl p-4 space-y-3">
                <div className="flex justify-between items-center">
                  <span className="text-sm text-gray-600 dark:text-slate-400 font-medium">Refund ID</span>
                ...
                <div className="flex justify-between items-center">
                  <span className="text-sm text-gray-600 dark:text-slate-400 font-medium">Total Amount</span>
                  <span className="text-xl font-bold text-indigo-600 dark:text-indigo-400">
                    {formatCurrency(successData.totalRefundAmount)}
                  </span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm text-gray-600 dark:text-slate-400 font-medium">Items Refunded</span>
