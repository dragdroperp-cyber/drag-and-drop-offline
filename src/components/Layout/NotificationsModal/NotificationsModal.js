...
const NotificationsModal = ({ onClose }) => {
  ...
  const notifications = [
    ...
    ...lowStockProducts.filter(p => (Number(p.quantity) || Number(p.stock) || 0) <= 0).map(product => ({
      id: `oos-${product.id}`,
      type: 'out_of_stock',
      severity: 'critical',
  ...
  const modalContent = (
    ...
    >
      ...
      <div
        ...
        className="bg-white dark:bg-gray-800 !rounded-none md:!rounded-2xl shadow-2xl w-full max-w-4xl !h-full md:!h-auto md:max-h-[85vh] flex flex-col overflow-hidden transition-colors duration-200 fixed inset-0 md:relative md:inset-auto m-0"
