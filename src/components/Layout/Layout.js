...
const Layout = React.memo(({ children }) => {
  ...
  return (
    <div className="flex h-screen text-slate-900 dark:text-slate-100 transition-colors duration-300">
      <nav
        className={`xl:flex xl:flex-col xl:sticky xl:top-0 xl:h-screen shadow-[0_28px_90px_-55px_rgba(15,23,42,0.55)] transition-all duration-300 ease-in-out overflow-hidden ${sidebarOpen ? 'w-72 opacity-100' : 'w-0 opacity-0'}`}
      >
        <Sidebar />
      </nav>

      <div className={`fixed inset-0 z-40 xl:hidden transition-all duration-300 ${sidebarOpen ? 'visible pointer-events-auto' : 'invisible pointer-events-none'}`} style={{ height: '100vh', overflow: 'hidden' }}>
        {/* Backdrop */}
        <div
      ...
      <div className={`fixed z-[100] pointer-events-none transition-all duration-300 ${isMobileView
