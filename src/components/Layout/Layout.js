...
const Layout = React.memo(({ children }) => {
  ...
  const ToastItem = React.memo(({ toast, onRemove, isMobile }) => {
    ...
    const handleMouseLeave = useCallback(() => {
      ...
      timerRef.current = setTimeout(updateTimer, 50);
    }, [toast.id, toast.duration, onRemove, timerRef]);

    return (
      <div
        className={`${isMobile
          ? // Mobile styling: full width, top positioned, sharp corners with slide-out animation only
          `w-full rounded-none border-x-0 border-t px-4 py-3 text-sm transform transition-all duration-300 ease-out ${isDismissing ? 'translate-x-full opacity-0' : 'translate-y-0 opacity-100'
          } ${toast.type === 'success'
          ...
          `flex items-end gap-3 rounded-none border px-4 py-3 shadow-[0_16px_40px_-28px_rgba(15,23,42,0.5)] backdrop-blur-md min-w-[280px] max-w-[420px] mb-2 text-sm transform transition-all duration-300 ease-out hover:scale-[1.02] ${isDismissing ? 'translate-x-full opacity-0' : 'translate-y-0 opacity-100'
          } ${toast.type === 'success'
          ...
          {isMobile ? (
            ...
            <>
              ...
              <div className="absolute bottom-0 left-0 right-0 h-1 bg-slate-200 rounded-b-none overflow-hidden">
                <div
                  className={`h-full transition-all duration-100 ease-linear ${toast.type === 'success'
                    ? 'bg-emerald-500'
                    : toast.type === 'error'
                      ? 'bg-rose-500'
                      : toast.type === 'warning'
    ...
    <div className="flex h-screen text-slate-900 dark:text-slate-100 transition-colors duration-300">
      ...
      {sidebarOpen && (
        <div className="fixed inset-0 z-40 xl:hidden" style={{ height: '100vh', overflow: 'hidden' }}>
          ...
          <div className="relative w-full max-w-xs h-full flex flex-col shadow-[0_28px_80px_-50px_rgba(15,23,42,0.55)] bg-white dark:bg-slate-800" style={{ height: '100vh' }}>
