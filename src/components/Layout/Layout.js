...
const Layout = ({ children }) => {
  ...
  useEffect(() => {
    console.log('🔍 isLoadingAccess changed to:', isLoadingAccess);
  }, [isLoadingAccess]);
  ...
  const checkIndexedDBHasData = async () => {
    try {
      console.log('🔍 Checking if IndexedDB has any existing data...');
      ...
      console.log('📊 IndexedDB data check:', {
        hasAnyData,
  ...
  useEffect(() => {
    console.log('🎯 Layout mounted - checking IndexedDB data');
    ...
    const checkDataAndSetLoading = async () => {
      ...
      if (!profileCompleted) {
        console.log('👤 SELLER PROFILE NOT COMPLETED - SKIPPING LOADING SCREEN');
      ...
      if (hasExistingData) {
        ...
        console.log('🔄 EXISTING DATA FOUND - NO LOADING SCREEN');
        setIsLoadingAccess(false);
      } else {
        // First login - show loading until UI data loads
        console.log('🎯 FIRST LOGIN DETECTED - SHOWING LOADING UNTIL UI DATA LOADS');
  ...
  useEffect(() => {
    ...
    console.log('📊 UI Data loading check:', {
      hasProducts,
      hasOrders,
      hasCustomers,
  ...
  useEffect(() => {
    ...
    console.log('🔍 Loading hide conditions:', {
      allDataLoaded,
      deltaSyncSuccessful,
      hasAnyData,
      dataFreshness: state.dataFreshness,
      isLoadingAccess,
      dataStates: dataLoadingStates,
      productsLength: productsArray.length,
      ordersLength: ordersArray.length,
      customersLength: customersArray.length
    });

    if (isLoadingAccess === true && (allDataLoaded || deltaSyncSuccessful || hasAnyData)) {
      console.log('✅ UI DATA LOADED OR DELTA SYNC SUCCESSFUL OR HAS ANY DATA - HIDING LOADING SCREEN IMMEDIATELY');
      console.log(`   📊 Reason: ${allDataLoaded ? 'All data loaded' : deltaSyncSuccessful ? 'Delta sync successful' : 'Has any data in Redux'}`);
      console.log('🚨 SETTING isLoadingAccess to false');
      setIsLoadingAccess(false);
      console.log('✅ isLoadingAccess should now be false');
    }
