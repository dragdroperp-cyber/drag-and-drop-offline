    ...
    <div className="dark-sidebar flex h-full flex-col" style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      <div className="flex items-center justify-between px-5 py-5 flex-shrink-0 border-b border-slate-200/50 dark:border-slate-700/50">
        <div className="flex items-center gap-3 min-w-0">
          <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-transparent overflow-hidden">
            <img
              src={logoSrc}
              alt="Drag & Drop"
          ...
          <div className="min-w-0">
            ...
            {planExpired && (
              <div className="mt-1 flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-rose-50 dark:bg-rose-900/20 border border-rose-100 dark:border-rose-900/30 text-[10px] font-bold text-rose-600 dark:text-rose-400">
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden min-h-0" style={{ WebkitOverflowScrolling: 'touch' }}>
        <ul className="pb-6 pt-2">
          {navigation.map((item) => {
            ...
            // Sidebar options are now always unlocked to allow read-only access to all modules
            // Users can navigate freely, but write operations are restricted if the plan is expired
            const finalIsUnlocked = true;



            const buttonTitle = !finalIsUnlocked
          ...
          <li className="px-3 py-1">
            ...
            >
              ...
              <span className="truncate capitalize">{getTranslation('settings', state.currentLanguage)}</span>
            </button>
          </li>
        </ul>
      </div>
