              ...
              {product.batches && product.batches.length > 0 && (
                <div className="space-y-1.5 md:col-span-2">
                  ...
                  <select
                    ...
                    className="block w-full px-4 py-3 bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-lg text-sm font-bold focus:border-indigo-500 outline-none transition-all"
                  >
                    <option value="">{getTranslation('autoSelect', state.currentLanguage) || 'Auto Select (FIFO/FEFO)'}</option>
                    {product.batches
                      .filter(b => (Number(b.quantity) || 0) > 0)
                      .map(b => (
                        <option key={b.id || b._id} value={b.id || b._id}>
                          {b.batchNumber || 'N/A'} - {formatQuantityWithUnit(b.quantity, product.unit || 'pcs')} - {formatCurrencySmart(saleMode === 'wholesale' ? (b.wholesalePrice || product.wholesalePrice || b.sellingUnitPrice || b.sellingPrice) : (b.sellingUnitPrice || b.sellingPrice), state.currencyFormat)}
                        </option>
                      ))}
                  </select>
                </div>
