...
const QuantityModal = ({ product, onClose, onAdd }) => {
  ...
  return (
    <div className="fixed inset-0 bg-gray-900/60 backdrop-blur-sm flex items-end sm:items-center justify-center z-[1300] sm:p-4">
      <div className="bg-white dark:bg-slate-900 w-full h-[90%] sm:h-auto sm:max-w-md rounded-t-2xl sm:rounded-2xl shadow-2xl p-6 border dark:border-slate-700/60 transition-colors overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl border border-slate-100 dark:border-slate-700/50">
            ...
            <p className="text-sm text-gray-600 dark:text-slate-400">
              {getTranslation('price', state.currentLanguage)}: <span className="font-medium text-gray-900 dark:text-white" title={formatCurrency(product.sellingPrice || product.costPrice || 0)}>{formatCurrencySmart(product.sellingPrice || product.costPrice || 0, state.currencyFormat)}</span> per {product.productUnit || product.quantityUnit || 'pcs'}
            </p>
