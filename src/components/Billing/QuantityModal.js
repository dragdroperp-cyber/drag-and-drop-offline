...
const QuantityModal = ({ product, onClose, onAdd }) => {
  ...
  const getPricePerUnit = (targetUnit) => {
    ...
    const pUnit = (product.quantityUnit || product.unit || 'pcs').toLowerCase();
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl border border-slate-100 dark:border-slate-700/50">
            ...
            <p className="text-sm text-gray-600 dark:text-slate-400">
              {getTranslation('price', state.currentLanguage)}: <span className="font-medium text-gray-900 dark:text-white">{formatCurrency(product.sellingPrice || product.costPrice || 0)}</span> per {product.quantityUnit || 'pcs'}
            </p>
