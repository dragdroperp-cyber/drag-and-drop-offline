...
import { X, User, Phone, Wallet, CreditCard, Receipt, Printer, Download } from 'lucide-react';
      ...
      <div className="fixed inset-0 bg-gray-900/60 backdrop-blur-sm flex items-end sm:items-center justify-center z-50 sm:p-4">
        <div className="bg-white dark:bg-slate-900 w-full h-[95%] sm:h-auto sm:max-w-md rounded-none sm:rounded-2xl shadow-2xl border dark:border-slate-700/60 flex flex-col overflow-hidden">
          ...
          <div className="flex items-center justify-between p-6 pb-4 flex-shrink-0 border-b border-slate-200/60 dark:border-slate-700/60">
            <div>
              <h2 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">Complete Payment</h2>
              <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Total: {formatCurrency(total)}</p>
            </div>
            <button
              onClick={onClose}
            ...
            <form onSubmit={handleSubmit} className="space-y-5" noValidate>
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center gap-2 text-sm">
                  ...
                  />
                  <label htmlFor="customName" className="cursor-pointer text-slate-700 dark:text-slate-300">
                    Use custom name
                  </label>
                </div>
                {useCustomName ? (
                  <div>
                    ...
                    <input
                      ...
                      className={`w-full px-4 py-3 rounded-xl border bg-white dark:bg-slate-800 focus:outline-none focus:ring-2 transition-all text-slate-900 dark:text-white placeholder:text-slate-400 ${customerNameError
                        ? 'border-red-500 focus:ring-red-500/20 focus:border-red-500'
                        : 'border-slate-200/80 dark:border-slate-700 focus:ring-indigo-500/20 focus:border-indigo-500'
                        }`}
                    />
                    {customerNameError && (
                  ...
                  <div>
                    <label htmlFor="selectCustomer" className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">
                      Select Customer
                    </label>
                    <select
                      id="selectCustomer"
                      ...
                      className={`w-full px-4 py-3 rounded-xl border bg-white dark:bg-slate-800 focus:outline-none focus:ring-2 transition-all text-slate-900 dark:text-white ${customerNameError
                        ? 'border-red-500 focus:ring-red-500/20 focus:border-red-500'
                        : 'border-slate-200/80 dark:border-slate-700 focus:ring-indigo-500/20 focus:border-indigo-500'
                        }`}
                    >
                      ...
                      {customers.map(customer => {
                        ...
                        return (
                          <option key={customer.id} value={customer.name}>
                            {customer.name} {mobileNumber ? `(${mobileNumber})` : ''} {(customer.balanceDue || customer.dueAmount) ? `- ${formatCurrency(customer.dueAmount || customer.balanceDue || 0)} due` : ''}
                          </option>
                        );
                      })}
                    </select>
                ...
                <div>
                  <label htmlFor="mobile" className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2 flex items-center gap-2">
                    <Phone className="h-4 w-4" />
                    Mobile Number
                    {!useCustomName && selectedCustomer && (
                      <span className="text-xs text-slate-500 dark:text-slate-400 font-normal">(Auto-filled from customer)</span>
                    )}
                  </label>
                  <input
                    type="tel"
                    ...
                    className={`w-full px-4 py-3 rounded-xl border bg-white dark:bg-slate-800 focus:outline-none focus:ring-2 transition-all text-slate-900 dark:text-white placeholder:text-slate-400 ${!useCustomName && selectedCustomer ? 'bg-slate-50 dark:bg-slate-900/50 cursor-not-allowed opacity-75' : ''
                ...
                <div className="flex items-center gap-2 text-sm">
                  <input
                    ...
                    style={{ accentColor: 'var(--brand-primary)' }}
                  />
                  <label htmlFor="sendWhatsApp" className="cursor-pointer text-slate-700 dark:text-slate-300">
              ...
              <div className="space-y-4 pt-4 border-t border-slate-200/60 dark:border-slate-700/60">
                ...
                <div>
                  <select
                    ...
                    className={`w-full px-4 py-3 rounded-xl border bg-white dark:bg-slate-800 focus:outline-none focus:ring-2 transition-all text-slate-900 dark:text-white ${splitPaymentError || upiIdError
                      ? 'border-red-500 focus:ring-red-500/20 focus:border-red-500'
                      : 'border-slate-200/80 dark:border-slate-700 focus:ring-indigo-500/20 focus:border-indigo-500'
                      }`}
                  >
                ...
                {splitPaymentDetails && !splitPaymentError && (
                  <div className="mt-3 p-3 rounded-lg bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-200 dark:border-indigo-800/30">
                    <p className="text-xs font-medium text-indigo-800 dark:text-indigo-300">
                      Split Payment: {splitPaymentDetails.cashAmount > 0 && `Cash: ${formatCurrency(splitPaymentDetails.cashAmount)} `}
                      {splitPaymentDetails.onlineAmount > 0 && `Online: ${formatCurrency(splitPaymentDetails.onlineAmount)} `}
                      {splitPaymentDetails.dueAmount > 0 && `Due: ${formatCurrency(splitPaymentDetails.dueAmount)}`}
                    </p>
                  </div>
                )}
              </div>
              ...
              {/* Action Buttons */}
              <div className="flex justify-end gap-3 pt-6 pb-2 border-t border-slate-200/60 dark:border-slate-700/60">
                <button
                  type="button"
                  onClick={onClose}
        ...
        >
          <div
            style={{
              ...
              maxHeight: '80vh',
              overflowY: 'auto',
              boxShadow: state.darkMode ? '0 20px 60px -12px rgba(0, 0, 0, 0.5)' : '0 20px 60px -12px rgba(0, 0, 0, 0.25)',
              border: state.darkMode ? '1px solid #334155' : '1px solid #e5e7eb'
            }}
              ...
              {foundCustomers.map((customer, index) => {
                ...
                return (
                  <button
                    ...
                    onMouseEnter={(e) => {
                      e.currentTarget.style.backgroundColor = state.darkMode ? '#475569' : '#f3f4f6';
                      e.currentTarget.style.borderColor = '#3b82f6';
                    }}
                    onMouseLeave={(e) => {
                      e.currentTarget.style.backgroundColor = state.darkMode ? '#334155' : '#f9fafb';
                      e.currentTarget.style.borderColor = state.darkMode ? '#475569' : '#e5e7eb';
                    }}
                  >
                    <div style={{
                      fontWeight: '600',
            ...
            <div style={{
              ...
              <button
                ...
                style={{
                  ...
                  border: state.darkMode ? '1px solid #475569' : '1px solid #d1d5db',
                  borderRadius: '8px',
                  cursor: 'pointer',
                  fontWeight: '500',
                  fontSize: '14px'
