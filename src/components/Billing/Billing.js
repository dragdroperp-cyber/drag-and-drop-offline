...
import { formatNumberOnly, formatNumber } from '../../utils/numberFormat';
...
const Billing = () => {
  ...
  const generatePaperBill = async (pdf, invoiceNumber = null, billData = null) => {
    try {
      ...
      const discount = (billData?.discountPercent || billData?.discountAmount || discount) ?? 0;
      // Handle both percent and amount
      const finalDiscountAmount = billData?.discountAmount ?? (subTotal * (billData?.discountPercent ?? 0) / 100);
