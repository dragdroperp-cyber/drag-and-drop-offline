...
const Billing = () => {
  ...
  const finalizeOrder = ({
    ...
    // CRITICAL: Explicitly sync to MongoDB after order creation
    // We strictly avoid syncing 'products' to prevent heavy API calls, as per optimization request.
    // Instead we only sync batches (where stock is deducted) and the order/customer data.
    // Delay increased to 2000ms to ABSOLUTELY ensure updateInventoryAfterSale has finished marking items as unsynced.
    setTimeout(async () => {
      // Use window.syncService if available (to ensure we have the latest instance)
      // or fallback to the imported one
      const service = window.syncService || syncService;

      if (service && service.isOnline()) {
        try {
          console.log('🔄 Starting optimized post-order sync (delayed)...');

          // 1. Sync Customers (in case new customer or balance update)
          const customerFuncs = getStoreFunctions('customers');
          if (customerFuncs) {
            await service.syncStore('customers', customerFuncs.getAllItems, customerFuncs.updateItem, customerFuncs.deleteItem);
          }

          // 2. Sync Orders (CRITICAL: Sync orders BEFORE batches to prevent double deduction on backend)
          // The backend checks if stockDeducted is true (which it is for these orders).
          // If we synced batches first, backend might deduct (if check fails) and then we'd overwrite.
          // By syncing orders first with stockDeducted=true, backend skips logic, then we assert correct batch state.
          const orderFuncs = getStoreFunctions('orders');
          if (orderFuncs) {
            await service.syncStore('orders', orderFuncs.getAllItems, orderFuncs.updateItem);
          }

          // 3. Sync Product Batches (CRITICAL: instant batch sync)
          const batchFuncs = getStoreFunctions('productBatches');
          if (batchFuncs) {
            await service.syncStore('productBatches', batchFuncs.getAllItems, batchFuncs.updateItem, batchFuncs.deleteItem);
          }

          // 4. Sync Transactions (if any)
          const transactionFuncs = getStoreFunctions('transactions');
          if (transactionFuncs) {
            await service.syncStore('transactions', transactionFuncs.getAllItems, transactionFuncs.updateItem);
          }

          console.log('✅ Optimized post-order sync completed');


          // Force a final status update to ensure UI is 100% correct
          if (triggerSyncStatusUpdate) {
            triggerSyncStatusUpdate();
          }
        } catch (err) {
          console.error('⚠️ Optimized post-order sync failed:', err);
        }
      }
    }, 1000); // Reduced delay to 1000ms to be more responsive but still safe
