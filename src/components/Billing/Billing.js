...
import { sortBatches, getEffectivePrice } from '../../utils/productUtils';
import { addItem, STORES } from '../../utils/indexedDB';
...
const Billing = () => {
    ...
    if (customerName && customerName.trim() !== '' && customerName.trim().toLowerCase() !== 'walk-in customer') {
      ...
      if (customer) {
        ...
        if (hasDueAmount) {
          ...
          // Create Customer Transaction for due amount
          if (dueAmountToAdd > 0) {
            const transaction = {
              ...
              orderId: order.id,
              type: 'due',
              amount: dueAmountToAdd,
              date: new Date().toISOString(),
              description: `Order #${order.invoiceNumber || ''} - Due Amount`,
              isSynced: false,
              isDeleted: false,
  ...
  const generateAndDownloadPDF = async (bill) => {
    try {
      ...
      safeDrawText(pdf, getTranslation('billingSystem', state.currentLanguage), logoX + 22, 19, { fontSize: 9, color: `rgb(${COLORS.gray.join(',')})` });

      /* -------- RIGHT META -------- */
      pdf.setFontSize(12);
      pdf.setFont('helvetica', 'bold');
      pdf.setTextColor(...COLORS.black);
      safeDrawText(pdf, getTranslation('customerInvoice', state.currentLanguage), pageWidth - margin, 14, { align: 'right', fontSize: 12, color: `rgb(${COLORS.black.join(',')})` });

      pdf.setFontSize(9);
      pdf.setFont('helvetica', 'normal');
      pdf.setTextColor(...COLORS.gray);
      safeDrawText(pdf, `${getTranslation('invoiceNumber', state.currentLanguage)}: ${bill.id || `INV-${Date.now()}`}`, pageWidth - margin, 19, { align: 'right', fontSize: 9, color: `rgb(${COLORS.gray.join(',')})` });
      ...
      pdf.setFont('helvetica', 'bold');
      pdf.setTextColor(...COLORS.primary);
      safeDrawText(pdf, getTranslation('billTo', state.currentLanguage), margin + 4, y + 6, { fontSize: 10, color: `rgb(${COLORS.primary.join(',')})` });

      pdf.setFont('helvetica', 'normal');
      pdf.setTextColor(...COLORS.black);
      safeDrawText(pdf, bill.customerName, margin + 4, y + 14, { fontSize: 10, color: `rgb(${COLORS.black.join(',')})` });

      pdf.setFont('helvetica', 'bold');
      pdf.setTextColor(...COLORS.primary);
      safeDrawText(pdf, `${getTranslation('storeName', state.currentLanguage)}:`, pageWidth / 2, y + 6, { fontSize: 10, color: `rgb(${COLORS.primary.join(',')})` });

      pdf.setFont('helvetica', 'normal');
      pdf.setTextColor(...COLORS.black);
      safeDrawText(pdf, state.storeName || 'Store', pageWidth / 2, y + 14, { fontSize: 10, color: `rgb(${COLORS.black.join(',')})` });

      pdf.setFont('helvetica', 'bold');
      pdf.setTextColor(...COLORS.primary);
      safeDrawText(pdf, `${getTranslation('date', state.currentLanguage)}:`, pageWidth - margin - 30, y + 6, { fontSize: 10, color: `rgb(${COLORS.primary.join(',')})` });
      ...
      safeDrawText(pdf, getTranslation('productHeader', state.currentLanguage), margin + 2, y + 6, { fontSize: 11, color: `rgb(${COLORS.primary.join(',')})` });
      safeDrawText(pdf, getTranslation('qtyHeader', state.currentLanguage), pageWidth / 2, y + 6, { align: 'center', fontSize: 11, color: `rgb(${COLORS.primary.join(',')})` });
      safeDrawText(pdf, getTranslation('priceHeader', state.currentLanguage), pageWidth / 2 + 30, y + 6, { align: 'right', fontSize: 11, color: `rgb(${COLORS.primary.join(',')})` });
      safeDrawText(pdf, getTranslation('totalHeader', state.currentLanguage), pageWidth - margin - 2, y + 6, { align: 'right', fontSize: 11, color: `rgb(${COLORS.primary.join(',')})` });
      ...
      bill.items.forEach((item, index) => {
        ...
        safeDrawText(pdf, nameLines, margin + 2, y + 5, { fontSize: 10, color: `rgb(${COLORS.black.join(',')})` });
        safeDrawText(pdf, String(qty), pageWidth / 2, y + 5, { align: 'center' });
        safeDrawText(pdf, formatCurrencyForPrint(price), pageWidth / 2 + 30, y + 5, { align: 'right' });
        safeDrawText(pdf, formatCurrencyForPrint(total), pageWidth - margin - 2, y + 5, { align: 'right' });
      ...
      const totals = [
        { label: getTranslation('subtotal', state.currentLanguage), value: formatCurrencyForPrint(subTotal) },
        { label: getTranslation('disc', state.currentLanguage), value: `-${formatCurrencyForPrint(discount)}` },
        { label: getTranslation('total', state.currentLanguage), value: formatCurrencyForPrint(finalTotal) }
      ];
      ...
      totals.forEach(({ label, value }, idx) => {
        ...
        const color = idx === totals.length - 1 ? `rgb(${COLORS.primary.join(',')})` : `rgb(${COLORS.black.join(',')})`;
        const fontSize = idx === totals.length - 1 ? 12 : 10;
        safeDrawText(pdf, label, pageWidth / 2 + 30, y, { fontSize, color });
        safeDrawText(pdf, value, pageWidth - margin - 2, y, { align: 'right', fontSize, color });
        y += 6;
      });
      ...
      pdf.setFontSize(10);
      pdf.setTextColor(...COLORS.gray);
      safeDrawText(pdf, getTranslation('thankYouShopping', state.currentLanguage), margin, y, { fontSize: 10, color: `rgb(${COLORS.gray.join(',')})` });
  ...
  const generateThermalBill = async (size, invoiceNumber = null, billData = null) => {
    ...
    if (billData?.paymentMethod === 'split' && billData?.splitPaymentDetails) {
      ...
      if (splitDetails.cashAmount > 0) {
        pdf.text(`Cash: ${formatCurrencyForPrint(splitDetails.cashAmount)}`, 2, contentY);
        contentY += 2.5;
      }
      if (splitDetails.onlineAmount > 0) {
        pdf.text(`Online: ${formatCurrencyForPrint(splitDetails.onlineAmount)}`, 2, contentY);
        contentY += 2.5;
      }
      if (splitDetails.dueAmount > 0) {
        pdf.text(`Due: ${formatCurrencyForPrint(splitDetails.dueAmount)}`, 2, contentY);
        contentY += 2.5;
      }
    ...
    billItems.forEach(item => {
      ...
      pdf.text(formatCurrencyForPrint(amount), pageWidth - 2, contentY, { align: 'right' });
      contentY += 3;
    });
    ...
    pdf.setFont('helvetica', 'bold');
    pdf.setFontSize(7);
    pdf.text(`Subtotal: ${formatCurrencyForPrint(subtotal)}`, 2, contentY);
    contentY += 3;
    pdf.text(`Discount: ${formatCurrencyForPrint(discountAmount)}`, 2, contentY);
    contentY += 3;
    pdf.text(`Tax: ${formatCurrencyForPrint(taxAmount)}`, 2, contentY);
    contentY += 3;
    pdf.setFontSize(8);
    pdf.text(`TOTAL: ${formatCurrencyForPrint(total)}`, 2, contentY);
    contentY += 5;
    ...
    contentY += 4;
    pdf.setFontSize(5);
    pdf.text(`Powered by ${state.storeName || 'Drag & Drop Store'}`, pageWidth / 2, contentY, { align: 'center' });
  ...
  const generatePaperBill = async (pdf, invoiceNumber = null, billData = null) => {
    try {
      ...
      /* -------- APP NAME -------- */
      const storeLabel = state.storeName || 'Store';
      pdf.setFont('helvetica', 'bold');
      pdf.setFontSize(18);
      pdf.setTextColor(...COLORS.primary);
      pdf.text(storeLabel, logoX + 22, 14);

      pdf.setFontSize(9);
      pdf.setFont('helvetica', 'normal');
      pdf.setTextColor(...COLORS.gray);
      pdf.text('Tax Invoice', logoX + 22, 19);

      /* -------- RIGHT META -------- */
      pdf.setFontSize(12);
      pdf.setFont('helvetica', 'bold');
      pdf.setTextColor(...COLORS.black);
      pdf.text('INVOICE', pageWidth - margin, 14, { align: 'right' });
      ...
      pdf.setFont('helvetica', 'bold');
      pdf.setTextColor(...COLORS.black);
      pdf.text(billData?.customerName || 'Walk-in Customer', margin, y + 6);

      if (billData?.customerMobile) {
        pdf.setFontSize(9);
        pdf.setFont('helvetica', 'normal');
        pdf.setTextColor(...COLORS.gray);
        pdf.text(billData.customerMobile, margin, y + 11);
      }
      ...
      pdf.setTextColor(...COLORS.black);
      const billDate = billData?.date ? formatDate(billData.date) : formatDate(new Date());
      pdf.text(billDate, rightColX, y + 6);

      pdf.setFontSize(9);
      pdf.setFont('helvetica', 'bold');
      pdf.setTextColor(...COLORS.gray);
      pdf.text('PAYMENT', rightColX, y + 12);

      pdf.setFontSize(10);
      pdf.setFont('helvetica', 'normal');
      pdf.setTextColor(...COLORS.black);
      pdf.text(getPaymentMethodLabel(billData?.paymentMethod), rightColX, y + 18);
      ...
      pdf.text('PRODUCT DESCRIPTION', margin + 3, y + 6.5);
      pdf.text('QTY', pageWidth / 2 + 10, y + 6.5, { align: 'center' });
      pdf.text('PRICE', pageWidth / 2 + 35, y + 6.5, { align: 'right' });
      pdf.text('TOTAL', pageWidth - margin - 3, y + 6.5, { align: 'right' });
      ...
      items.forEach((item, index) => {
        ...
        // Calculate height based on name wrapping
        const nameLines = pdf.splitTextToSize(name, 85);
        ...
        pdf.text(String(qty), pageWidth / 2 + 10, y + 6, { align: 'center' });
        pdf.text(formatCurrencyForPrint(price), pageWidth / 2 + 35, y + 6, { align: 'right' });
        pdf.text(formatCurrencyForPrint(totalAmount), pageWidth - margin - 3, y + 6, { align: 'right' });
      ...
      const totalsData = [
        { label: "Subtotal", value: formatCurrencyForPrint(subTotal) },
        { label: "Discount", value: `-${formatCurrencyForPrint(finalDiscountAmount)}` },
        { label: "Grand Total", value: formatCurrencyForPrint(finalTotal) }
      ];
      ...
      totalsData.forEach(({ label, value }, idx) => {
        ...
        if (isLast) {
          ...
          // Draw a background for Grand Total
          pdf.setFillColor(...COLORS.lightBg);
          pdf.rect(pageWidth / 2 + 25, y - 6, pageWidth - (pageWidth / 2 + 25) - margin, 10, 'F');
        } else {
          pdf.setFont('helvetica', 'normal');
          pdf.setFontSize(10);
          pdf.setTextColor(...COLORS.gray);
      ...
      if (sellerUpiIdValue && sellerUpiIdValue.includes('@')) {
        try {
          ...
          const qrResult = await generateBillPaymentQR({
            ...billData,
            upiId: sellerUpiIdValue,
            storeName: state.storeName || 'Store'
          }, {
            upiId: sellerUpiIdValue,
            merchantName: state.storeName || 'Store'
          });
      ...
      /* ================= FOOTER ================= */
      const pageCountValue = pdf.internal.getNumberOfPages();
      ...
      for (let i = 1; i <= pageCountValue; i++) {
        ...
        pdf.setTextColor(...COLORS.gray);
        if (pageCountValue > 1) {
          pdf.text(`Page ${i} of ${pageCountValue}`, margin, pageHeight - 8);
        }
        pdf.text(
          state.storeName || 'Store',
          pageWidth - margin,
  ...
  const calculateBatchPricing = (product, requestedQuantity, unit) => {
    ...
    if (product.batches && product.batches.length > 0) {
      // Sort batches based on expiry logic (if trackExpiry) or creation date (FIFO)
      const sortedBatches = sortBatches(product.batches, product.trackExpiry);
  ...
  return (
    <div className="min-h-screen pb-8">
      ...
      <div className="mb-8">
        ...
        {(() => {
          const ordUsed = state.orders ? state.orders.filter(o => !o.isDeleted).length : 0;
          const custUsed = state.customers ? state.customers.filter(c => !c.isDeleted).length : 0;
            ...
            <div className="flex gap-3 text-xs mt-2">
              {ordRemaining < 15 && maxOrders !== Infinity && (
                <span className="px-2.5 py-1 rounded-md font-medium bg-red-50 text-red-700 border border-red-100 dark:bg-red-900/20 dark:text-red-400 dark:border-red-900/30">
                  {getTranslation('orders', state.currentLanguage)}: {ordUsed}/{maxOrders} ({ordRemaining} left)
                </span>
              )}
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs mt-0.5 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors" style={{ color: 'var(--text-secondary)' }}>
                          ₹{getEffectivePrice(product).toFixed(2)}/{product.quantityUnit || product.unit || 'pcs'} • Stock: {getTotalStockQuantity(product)}
                        </p>
                      </div>
                ...
                <button
                  onClick={() => {
                    ...
                    if (itemsToAdd.length === 0) {
                      ...
                      showToast(hasIssues ? 'Please fix product/unit issues before adding.' : 'No items found.', 'warning');
                      if (hasIssues) speakFeedback("Please fix the errors in the list or remove those items.");
                      return;
                    }
      ...
      {showInlineScanner && (
        <div className="fixed inset-0 z-[1200] bg-black overflow-y-auto">
          <div className="min-h-screen flex flex-col items-center pb-20">
            {/* Header with Close */}
            <div className="w-full flex items-center justify-between p-4 z-30 sticky top-0 bg-black/80 backdrop-blur-md">
            ...
            {/* Scanner Area */}
            <div className="w-full max-w-md aspect-square bg-black relative shrink-0">
              <BarcodeScanner
                onScan={(barcode) => {
                  setBarcodeInput(barcode);
                  handleBarcodeScan(barcode);
              ...
              />
              {/* Laser Overlay */}
              <div className="absolute inset-0 pointer-events-none border-2 border-indigo-500/50 m-8 rounded-lg">
              ...
              <p className="absolute bottom-4 left-0 right-0 text-center text-white/70 text-sm pointer-events-none font-medium">
                Align barcode within frame
              </p>
            </div>

            {/* Added Items List - Bottom Panel */}
            <div className="w-full max-w-md bg-white dark:bg-slate-900 rounded-t-3xl mt-[-20px] z-10 flex flex-col shadow-[0_-4px_20px_rgba(0,0,0,0.3)] min-h-[40vh] shrink-0">
              <div className="p-4 border-b border-gray-100 dark:border-slate-800 flex items-center justify-between bg-gray-50/90 dark:bg-slate-800/90 backdrop-blur-sm sticky top-[60px] z-20">
                    ...
                    <div key={index} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-slate-800 rounded-xl border border-gray-100 dark:border-slate-700 animate-slideUp">
                      <div className="flex-1 min-w-0 mr-3">
                        ...
                        <p className="text-xs text-gray-500 dark:text-slate-400">
                          {item.quantity} {item.unit} × {formatCurrencySmart(item.price, state.currencyFormat)}
                        </p>
                      </div>
              ...
              <div className="p-4 bg-gray-50 dark:bg-slate-800/50 border-t border-gray-100 dark:border-slate-800 mt-auto">
                <button
                  onClick={() => setShowInlineScanner(false)}
                  className="w-full py-4 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold shadow-lg shadow-indigo-200 dark:shadow-none transition-all active:scale-95 flex items-center justify-center gap-2"
                >
