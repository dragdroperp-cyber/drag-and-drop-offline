...
const Billing = () => {
  ...
  useEffect(() => {
    ...
    console.log('📏 printSize changed to:', printSize);
  }, [printSize]);
  ...
  useEffect(() => {
    const handleScannerInput = (e) => {
      ...
      if (e.key.length === 1 && !e.ctrlKey && !e.metaKey && !e.altKey && !e.shiftKey) {
        ...
        console.log(`Billing Key: "${e.key}", Time since last: ${timeSinceLastKey}ms, Buffer length: ${scannerInputBufferRef.current.length}`);
        ...
        if (timeSinceLastKey < 50 || scannerInputBufferRef.current.length === 0) {
          ...
          scannerInputTimerRef.current = setTimeout(() => {
            ...
            console.log('Billing processing scanned code:', `"${scannedCode}"`, 'Length:', scannedCode.length);
            ...
            if (scannedCode.length >= 3 && scannedCode.length <= 50) {
              ...
              console.log('Billing invalid barcode length, ignoring:', scannedCode.length);
            }
            // Clear buffer
          ...
          if (scannerInputBufferRef.current.length > 0) {
            console.log('Billing resetting buffer due to slow typing, buffer was:', `"${scannerInputBufferRef.current}"`);
            scannerInputBufferRef.current = '';
          }
        }
        ...
        e.preventDefault();
        const scannedCode = scannerInputBufferRef.current.trim();
        console.log('Billing Enter key pressed, processing code:', `"${scannedCode}"`, 'Length:', scannedCode.length);
        ...
        if (scannedCode.length >= 3 && scannedCode.length <= 50) {
          ...
          console.log('Billing invalid barcode length from Enter key, ignoring:', scannedCode.length);
        }
        scannerInputBufferRef.current = '';
      }
    };
  ...
  const finalizeOrder = ({
    ...
    console.log('🔧 Order being added to state:', orderWithStockFlag);
    console.log('🔧 Invoice number in order:', orderWithStockFlag.invoiceNumber);
    ...
    console.log('🧾 Stored order for bill generation:', order.id, 'invoice:', order.invoiceNumber);
  ...
  const generateBill = (modalData = null) => {
    ...
    console.log('🎯 Preparing Order with invoice number:', order.invoiceNumber);
  ...
  const shareBillToWhatsApp = () => {
    ...
    try {
      ...
      console.log('🧾 Bill - GST number:', gstNo, 'from state.gstNumber:', state.gstNumber, 'from currentUser:', state.currentUser?.gstNumber);
      if (gstNo && gstNo !== 'N/A') {
...
console.log('🧾 Bill - GST number:', gstNo, 'from state.gstNumber:', state.gstNumber, 'from currentUser:', state.currentUser?.gstNumber);
    ...
    if (total > 0) {
      ...
      console.log('🖨️ Thermal bill QR - Using UPI ID:', currentUpiId);
  ...
  const generateBillAndPrint = async () => {
    ...
    try {
      ...
      console.error('Error generating order and bill:', error);
      if (window.showToast) {
  ...
  const generatePaperBill = async (pdf, invoiceNumber = null, billData = null) => {
    try {
      ...
      pdf.setFont('helvetica', 'normal');
      const billDate = billData?.date ? new Date(billData.date).toLocaleDateString() : new Date().toLocaleDateString();
      pdf.text(`Date: ${billDate}`, pageWidth - 14, currentY, { align: 'right' });
      currentY += 6;
      ...
      const paymentMethodLabel = billData?.paymentMethod ? getPaymentMethodLabel(billData.paymentMethod) : getPaymentMethodLabel(paymentMethod);
      pdf.text(`Customer: ${customerName}`, 14, currentY);
      pdf.text(`Payment: ${paymentMethodLabel}`, pageWidth - 14, currentY, { align: 'right' });
      currentY += 6;
      ...
      if (billData?.paymentMethod === 'split' && billData?.splitPaymentDetails) {
        ...
        if (splitDetails.cashAmount > 0) {
          pdf.text(`Cash: ₹${splitDetails.cashAmount.toFixed(2)}`, 14, currentY);
          currentY += 4;
        }
        if (splitDetails.onlineAmount > 0) {
          pdf.text(`Online: ₹${splitDetails.onlineAmount.toFixed(2)}`, 14, currentY);
          currentY += 4;
        }
        if (splitDetails.dueAmount > 0) {
          pdf.text(`Due: ₹${splitDetails.dueAmount.toFixed(2)}`, 14, currentY);
          currentY += 4;
        }
        currentY += 2; // Extra spacing after split breakdown
      }
      currentY += 4;
      ...
      // Payment QR Code Section (if space allows)
      if (total > 0 && currentY < pageHeight - 40) { // Only add if there's enough space
        currentY += 8;
        ...
        pdf.setFontSize(10);
        pdf.setTextColor(0);
        pdf.text('Scan QR Code to Pay', pageWidth / 2, currentY, { align: 'center' });
        currentY += 8;
        ...
        const currentUpiId = sellerUpiId || 'merchant@upi';
        console.log('🖨️ Paper bill QR - Using UPI ID:', currentUpiId);
        ...
        try {
          // Generate QR code as base64 image
          const qrCodeDataURL = await QRCode.toDataURL(upiUrl, {
            width: 200, // High resolution for PDF
            margin: 1,
            color: {
              dark: '#000000',
              light: '#FFFFFF'
        ...
        pdf.setFont('helvetica', 'normal');
        pdf.setFontSize(8);
        pdf.text('Scan QR to pay', pageWidth / 2, currentY, { align: 'center' });
        currentY += 5;
      }
      ...
      if (event.keyCode === 115 || event.key === 'F4') {
        ...
        if (billItems.length > 0) {
          if (event.shiftKey) {
            ...
            console.log('🔧 Shift + F4: Auto generating bill and printing...');
            generateBillAndPrint();
          } else {
            // F4: Open complete payment popup
            console.log('🔧 F4: Opening payment popup...');
            setShowPaymentAndCustomerModal(true);
          }
        } else {
