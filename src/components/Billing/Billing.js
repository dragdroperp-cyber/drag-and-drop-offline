...
const Billing = () => {
  ...
  const generateAndDownloadPDF = (bill) => {
    try {
      ...
      // Download or Print
      const fileName = `invoice_${bill.customerName.replace(/\s+/g, '_')}_${bill.id}.pdf`;
  ...
  return (
    <div className="min-h-screen pb-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {showInlineScanner && (
          <div
            className={`fixed z-40 bg-black rounded-lg shadow-lg overflow-hidden transition-all duration-150 ${isDragging ? 'cursor-grabbing shadow-xl' : 'cursor-grab'
              }`}
            style={{
            ...
            <div className="bg-white dark:bg-slate-800 rounded-lg p-6 max-w-md w-full mx-4 border dark:border-slate-700">
              ...
              <div className="text-center">
                <div className="bg-gray-100 dark:bg-slate-700/50 p-4 rounded-lg mb-4">
                  ...
                  <div className="text-sm text-gray-600 dark:text-slate-300 mb-2">Date: {new Date(qrCodeData.date).toLocaleDateString()}</div>
                </div>
