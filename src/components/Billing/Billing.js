...
const Billing = () => {
  ...
  const generateThermalBill = async (size, invoiceNumber = null, billData = null) => {
    ...
    const drawContent = async (pdf) => {
      ...
      /* ================= HEADER ================= */
      const logoUrl = state.storeLogo || state.currentUser?.logoUrl || sellerSettings?.billSettings?.logoUrl || sellerSettings?.logoUrl;
      ...
      const logicTaxPercent = billData?.taxPercent || 0;
      const logicTaxAmount = billData?.taxAmount || ((subTotal - discountAmount) * logicTaxPercent / 100);

      if (logicTaxAmount > 0) {
        pdf.setFontSize(8);
        pdf.setFont('helvetica', 'normal');
        pdf.text(`Tax (${logicTaxPercent}%)`, margin, y);
        pdf.text(`${Number(logicTaxAmount).toFixed(2)}`, width - margin, y, { align: 'right' });
        y += 4;
      }
      ...
      /* ================= FOOTER ================= */
      if (sellerSettings?.billSettings?.showFooter !== false) {
        pdf.setFontSize(10);
        pdf.setFont('helvetica', 'bold');
        ...
        pdf.setFontSize(7);
        pdf.setFont('helvetica', 'normal');
        const termsText = sellerSettings?.billSettings?.termsAndConditions || "";
        if (termsText) {
        ...
        y += 2;
        const footerMsg = sellerSettings?.billSettings?.footerMessage || "Thank you, visit again";
