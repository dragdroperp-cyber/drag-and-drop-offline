...
const Billing = () => {
  ...
  const [scannerPosition, setScannerPosition] = useState({ x: 16, y: 16 }); // Default position (top-right with some margin)
  const [scannerSize, setScannerSize] = useState({ width: 400, height: 300 }); // Default size - larger camera frame
  ...
  const [printSize, setPrintSize] = useState('a4'); // Default print size
  const printSizeRef = useRef('a4'); // Ref to track current printSize

  // Update ref when printSize changes
  useEffect(() => {
  ...
  const handleMouseMove = (e) => {
    ...
    const maxX = Math.max(0, window.innerWidth - scannerSize.width - 20); // Account for dynamic width
    const maxY = Math.max(0, window.innerHeight - scannerSize.height - 100); // Account for dynamic height + header/footer
  ...
  const handleResizeMove = (e) => {
    ...
    let newX = scannerPosition.x;
    let newY = scannerPosition.y;

    // Handle different resize directions - only move the edge being stretched
    switch (resizeStart.direction) {
      case 'se': // southeast - bottom-right corner
        // Stretching right: right edge moves right (width increases)
        // Stretching down: bottom edge moves down (height increases)
        newWidth = Math.max(200, resizeStart.width + deltaX);
        newHeight = Math.max(150, resizeStart.height + deltaY);

        // Constrain to not exceed screen bounds from current position
        const maxWidthSE = window.innerWidth - scannerPosition.x - 20;
        const maxHeightSE = window.innerHeight - scannerPosition.y - 100;
        newWidth = Math.min(newWidth, maxWidthSE, 800);
        newHeight = Math.min(newHeight, maxHeightSE, 600);
        break;

      case 'sw': // southwest - bottom-left corner
        // Stretching left: left edge moves left (width increases)
        // Stretching down: bottom edge moves down (height increases)
        newWidth = Math.max(200, resizeStart.width - deltaX); // Negative deltaX means stretching left
        newHeight = Math.max(150, resizeStart.height + deltaY);

        // Move left edge left when stretching left (width increases)
        if (newWidth > resizeStart.width) {
          newX = scannerPosition.x - (newWidth - resizeStart.width);
          newX = Math.max(0, newX); // Constrain to left edge of screen
        }

        // Constrain size - can expand to right and down
        const maxWidthSW = scannerPosition.x + resizeStart.width + 20;
        const maxHeightSW = window.innerHeight - scannerPosition.y - 100;
        newWidth = Math.min(newWidth, maxWidthSW, 800);
        newHeight = Math.min(newHeight, maxHeightSW, 600);
        break;

      case 'ne': // northeast - top-right corner
        // Stretching right: right edge moves right (width increases)
        // Stretching up: top edge moves up (height increases)
        newWidth = Math.max(200, resizeStart.width + deltaX);
        newHeight = Math.max(150, resizeStart.height - deltaY); // Negative deltaY means stretching up

        // Move top edge up when stretching up (height increases)
        if (newHeight > resizeStart.height) {
          newY = scannerPosition.y - (newHeight - resizeStart.height);
          newY = Math.max(0, newY); // Constrain to top edge of screen
        }

        // Constrain size - can expand to right and up
        const maxWidthNE = window.innerWidth - scannerPosition.x - 20;
        const maxHeightNE = scannerPosition.y + resizeStart.height + 100;
        newWidth = Math.min(newWidth, maxWidthNE, 800);
        newHeight = Math.min(newHeight, maxHeightNE, 600);
        break;
      ...
      case 'nw': // northwest - top-left corner
        ...
        // Constrain size - can expand to right and down
        const maxWidthNW = scannerPosition.x + resizeStart.width + 20;
        const maxHeightNW = scannerPosition.y + resizeStart.height + 100;
        newWidth = Math.min(newWidth, maxWidthNW, 800);
        newHeight = Math.min(newHeight, maxHeightNW, 600);
        break;
    ...
    // Final safety check - ensure scanner stays within viewport
    const finalMaxX = window.innerWidth - newWidth - 20;
    const finalMaxY = window.innerHeight - newHeight - 100;
    newX = Math.max(0, Math.min(newX, finalMaxX));
    newY = Math.max(0, Math.min(newY, finalMaxY));

    setScannerSize({ width: newWidth, height: newHeight });
  ...
  const handleTouchMove = (e) => {
    ...
    // Constrain to viewport bounds (with some margin)
    const maxX = Math.max(0, window.innerWidth - scannerSize.width - 20);
    const maxY = Math.max(0, window.innerHeight - scannerSize.height - 100);
  ...
  const handleTouchResizeStart = (e, direction) => {
    setIsResizing(true);
    const touch = e.touches[0];
    setResizeStart({
      x: touch.clientX,
  ...
  const handleTouchResizeMove = (e) => {
    ...
    // Handle different resize directions - only move the edge being stretched (same as mouse)
    switch (resizeStart.direction) {
      case 'se': // southeast - bottom-right corner
        // Stretching right: right edge moves right (width increases)
        // Stretching down: bottom edge moves down (height increases)
  ...
  // Add global mouse/touch event listeners when dragging or resizing
  useEffect(() => {
    if (isDragging || isResizing) {
      document.addEventListener('mousemove', isDragging ? handleMouseMove : handleResizeMove);
      document.addEventListener('mouseup', handleMouseUp);
      document.addEventListener('touchmove', isDragging ? handleTouchMove : handleTouchResizeMove, { passive: false });
    ...
    const loadCashRegisterSound = async () => {
      try {
        ...
        try {
          ...
          audio.addEventListener('loadeddata', () => {
            //('✅ Cash register sound loaded successfully (direct path)');
            cashRegisterAudioRef.current = audio;
          });
  ...
  const handleBarcodeScan = (barcode) => {
    ...
    //('All products:', state.products.map(p => ({ name: p.name, barcode: p.barcode })));
  ...
  const speakAllItems = () => {
    ...
    const selectIndianVoice = () => {
      ...
      if (indianVoice) {
        utterance.voice = indianVoice;
        //('Selected voice:', indianVoice.name, indianVoice.lang, 'Female:', isFemaleVoice(indianVoice));
      }
    };
  ...
  const processVoiceInput = (text, showToasts = true) => {
    ...
    //(`✅ [processVoiceInput] Found ${productMap.size} product(s) to add:`, Array.from(productMap.values()).map(({ product, quantity, unit }) => `${product.name} ${quantity}${unit}`));
  ...
  const generateBill = (modalData = null) => {
    ...
    //('🎯 Full Order (MongoDB schema):', order);
  ...
  const makePayment = () => {
    ...
    //('makePayment called at:', new Date().toISOString());
    ...
    try {
      ...
      //('Auth state:', localStorage.getItem('auth'));
      ...
      //('Creating order:', JSON.stringify(order, null, 2));
      ...
      //('Order ID:', order.id, '(type:', typeof order.id, ')');
      //('Order sellerId:', order.sellerId, '(type:', typeof order.sellerId, ')');
  ...
  const shareBillToWhatsApp = () => {
    ...
    try {
      ...
      pdf.setFont('helvetica', 'normal');
      pdf.text(`Date: ${new Date().toLocaleDateString()}`, pageWidth - 14, currentY, { align: 'right' });
      currentY += 6;
      ...
      pdf.text(`Rs ${subtotal.toFixed(2)}`, pageWidth - 14, currentY, { align: 'right' });
      currentY += 6;

      // Discount
      if (discountAmount > 0) {
        pdf.text(`Discount:`, totalsX, currentY);
        pdf.text(`- Rs ${discountAmount.toFixed(2)}`, pageWidth - 14, currentY, { align: 'right' });
        currentY += 6;
      }
      ...
      if (taxAmount > 0) {
        ...
        pdf.text(`Rs ${taxAmount.toFixed(2)}`, pageWidth - 14, currentY, { align: 'right' });
        currentY += 6;
      }
      ...
      pdf.text(`Rs ${total.toFixed(2)}`, pageWidth - 14, currentY, { align: 'right' });
      currentY += 10;
  ...
  const generateThermalBill = async (size, invoiceNumber = null, billData = null) => {
    ...
    const billDate = billData?.date ? new Date(billData.date).toLocaleDateString() : new Date().toLocaleDateString();
    ...
    if (billData?.paymentMethod === 'split' && billData?.splitPaymentDetails) {
      ...
      if (splitDetails.cashAmount > 0) {
        pdf.text(`Cash: Rs ${splitDetails.cashAmount.toFixed(2)}`, 2, contentY);
        contentY += 2.5;
      }
      if (splitDetails.onlineAmount > 0) {
        pdf.text(`Online: Rs ${splitDetails.onlineAmount.toFixed(2)}`, 2, contentY);
        contentY += 2.5;
      }
      if (splitDetails.dueAmount > 0) {
        pdf.text(`Due: Rs ${splitDetails.dueAmount.toFixed(2)}`, 2, contentY);
        contentY += 2.5;
      }
    ...
    billItems.forEach(item => {
      ...
      pdf.text(`Rs ${amount}`, pageWidth - 2, contentY, { align: 'right' });
      contentY += 3;
    });
    ...
    pdf.setFont('helvetica', 'bold');
    pdf.setFontSize(7);
    pdf.text(`Subtotal: Rs ${subtotal.toFixed(2)}`, 2, contentY);
    contentY += 3;
    pdf.text(`Discount: Rs ${discountAmount.toFixed(2)}`, 2, contentY);
    contentY += 3;
    pdf.text(`Tax: Rs ${taxAmount.toFixed(2)}`, 2, contentY);
    contentY += 3;
    pdf.setFontSize(8);
    pdf.text(`TOTAL: Rs ${total.toFixed(2)}`, 2, contentY);
    contentY += 5;
    ...
    // Save with thermal format name
    pdf.save(`receipt-${customerName.replace(/\s+/g, '-')}-${Date.now()}.pdf`);

    if (window.showToast) {
      window.showToast(`Thermal receipt (${size}) generated successfully!`, 'success');
    }
  };
  ...
  const generatePaperBill = async (pdf, invoiceNumber = null, billData = null) => {
    try {
      ...
      pdf.setFont('helvetica', 'bold');
      pdf.setTextColor(0);
      pdf.text('TAX INVOICE', pageWidth / 2, 25, { align: 'center' });
      ...
      pdf.setFontSize(18);
      pdf.setFont('helvetica', 'bold');
      pdf.setTextColor(0, 0, 100); // Dark blue for store name
      const storeName = state.storeName || state.username || 'Grocery Store';
      pdf.text(storeName, pageWidth / 2, currentY, { align: 'center' });
      currentY += 10;

      // Store Address with better formatting
      pdf.setFont('helvetica', 'normal');
      pdf.setFontSize(11);
      pdf.setTextColor(0);
      const storeAddress = state.currentUser?.shopAddress || '';
      const storeCity = state.currentUser?.city || '';
      const storeState = state.currentUser?.state || '';
      const storePincode = state.currentUser?.pincode || '';

      // Format address properly
      const addressParts = [storeAddress, storeCity, storeState, storePincode].filter(part => part && part.trim());
      if (addressParts.length > 0) {
        const fullAddress = addressParts.join(', ');
        const addressLines = pdf.splitTextToSize(fullAddress, contentWidth - 30);
        addressLines.forEach(line => {
          pdf.text(line.trim(), pageWidth / 2, currentY, { align: 'center' });
          currentY += 6;
        });
      } else {
        pdf.text('Address Not Set', pageWidth / 2, currentY, { align: 'center' });
        currentY += 6;
      }
      ...
      if (phoneNumber) {
        pdf.setFontSize(10);
        pdf.setTextColor(0, 100, 0); // Dark green for phone
        pdf.text(`📞 ${phoneNumber}`, pageWidth / 2, currentY, { align: 'center' });
        currentY += 6;
      }

      // GST Number (Bold and prominent)
      pdf.setFont('helvetica', 'bold');
      pdf.setFontSize(11);
      pdf.setTextColor(150, 0, 0); // Dark red for GST
      const gstNo = state.gstNumber || state.currentUser?.gstNumber || '';
      if (gstNo && gstNo.trim()) {
        pdf.text(`GSTIN: ${gstNo}`, pageWidth / 2, currentY, { align: 'center' });
        currentY += 10;
      }

      // Invoice Details Section with background color only
      pdf.setFillColor(250, 250, 250);
      pdf.rect(marginLeft, currentY, contentWidth, 28, 'F');

      pdf.setFont('helvetica', 'normal');
      pdf.setFontSize(11);
      pdf.setTextColor(0);

      // Invoice Number and Date with better alignment
      const billId = invoiceNumber || `BILL-${Date.now().toString().slice(-6)}`;
      const billDate = billData?.date ? new Date(billData.date).toLocaleDateString('en-IN') : new Date().toLocaleDateString('en-IN');

      // Left side
      pdf.setFont('helvetica', 'bold');
      pdf.setTextColor(0, 0, 150); // Dark blue for labels
      pdf.text(`Invoice No:`, marginLeft + 8, currentY + 8);
      pdf.setTextColor(0);
      pdf.setFont('helvetica', 'normal');
      pdf.text(billId, marginLeft + 40, currentY + 8);

      pdf.setFont('helvetica', 'bold');
      pdf.setTextColor(0, 0, 150);
      pdf.text(`Date:`, marginLeft + 8, currentY + 16);
      pdf.setTextColor(0);
      pdf.setFont('helvetica', 'normal');
      pdf.text(billDate, marginLeft + 40, currentY + 16);
      ...
      pdf.setFont('helvetica', 'bold');
      pdf.setTextColor(0, 0, 150);
      pdf.text(`Customer:`, marginLeft + 8, currentY + 24);
      pdf.setTextColor(0);
      pdf.setFont('helvetica', 'normal');
      // Truncate long customer names
      const displayCustomerName = customerName.length > 25 ? customerName.substring(0, 22) + '...' : customerName;
      pdf.text(displayCustomerName, marginLeft + 40, currentY + 24);

      // Right side - Payment method
      pdf.setFont('helvetica', 'bold');
      pdf.setTextColor(0, 100, 0); // Dark green for payment
      pdf.text(`Payment:`, pageWidth - marginRight - 45, currentY + 24);
      pdf.setTextColor(0, 100, 0);
      pdf.setFont('helvetica', 'normal');
      pdf.text(paymentMethodLabel, pageWidth - marginRight - 8, currentY + 24, { align: 'right' });
      ...
      if (billData?.paymentMethod === 'split' && billData?.splitPaymentDetails) {
        ...
        // No border for split payment section

        let splitY = currentY + 4;
        if (splitDetails.cashAmount > 0) {
          pdf.text(`Cash: Rs.${splitDetails.cashAmount.toFixed(2)}`, marginLeft + 5, splitY);
          splitY += 5;
        }
        if (splitDetails.onlineAmount > 0) {
          pdf.text(`Online: Rs.${splitDetails.onlineAmount.toFixed(2)}`, marginLeft + 5, splitY);
          splitY += 5;
        }
        if (splitDetails.dueAmount > 0) {
          pdf.text(`Due: Rs.${splitDetails.dueAmount.toFixed(2)}`, marginLeft + 5, splitY);
        }
        currentY += 18;
      }
      ...
      // Table Header with background color only
      pdf.setFillColor(240, 240, 240);
      pdf.rect(marginLeft, currentY, contentWidth, 16, 'F');

      // Header bottom line
      pdf.setDrawColor(0);
      pdf.setLineWidth(0.8);
      pdf.line(marginLeft, currentY + 16, pageWidth - marginRight, currentY + 16);

      pdf.setFontSize(12);
      pdf.setFont('helvetica', 'bold');
      pdf.setTextColor(0, 0, 100); // Dark blue headers

      // Define column positions for perfect alignment
      const col1Start = marginLeft + 8;     // Item Description start
      const col1Width = 75;                 // Item Description width
      const col2Start = col1Start + col1Width + 5; // Quantity start
      const col2Width = 25;                 // Quantity width
      const col3Start = col2Start + col2Width + 5; // Rate start
      const col3Width = 35;                 // Rate width
      const col4Start = col3Start + col3Width + 5; // Amount start
      const col4Width = 40;                 // Amount width

      pdf.text('Item Description', col1Start, currentY + 10);
      pdf.text('Qty', col2Start, currentY + 10);
      pdf.text('Rate', col4Start - 5, currentY + 10, { align: 'right' });
      pdf.text('Amount', pageWidth - marginRight - 8, currentY + 10, { align: 'right' });
      ...
      billItems.forEach(item => {
        ...
        // Row background with subtle alternating
        const rowBgColor = rowCount % 2 === 0 ? [255, 255, 255] : [252, 252, 252];
        pdf.setFillColor(rowBgColor[0], rowBgColor[1], rowBgColor[2]);
        pdf.rect(marginLeft, currentY, contentWidth, 12, 'F');
        ...
        pdf.text(itemName, col1Start, currentY + 7);
        pdf.text(quantity, col2Start, currentY + 7);
        pdf.text(`Rs.${rate}`, col4Start - 5, currentY + 7, { align: 'right' });
        pdf.text(`Rs.${amount}`, pageWidth - marginRight - 8, currentY + 7, { align: 'right' });

        currentY += 12;
        rowCount++;
      });

      // Minimal line before totals section
      pdf.setDrawColor(200, 200, 200);
      pdf.setLineWidth(0.3);
      pdf.line(pageWidth - marginRight - 75, currentY, pageWidth - marginRight, currentY);

      // Totals Section with enhanced professional styling
      currentY += 10;

      // Totals section with background color only
      pdf.setFillColor(250, 250, 255); // Light blue background
      const totalsHeight = 12 + (discountAmount > 0 ? 8 : 0) + (taxAmount > 0 ? 8 : 0) + 16;
      pdf.rect(pageWidth - marginRight - 75, currentY, 75, totalsHeight, 'F');

      pdf.setFont('helvetica', 'normal');
      pdf.setFontSize(11);
      pdf.setTextColor(0);

      const totalsLabelX = pageWidth - marginRight - 72;
      const totalsValueX = pageWidth - marginRight - 6;

      // Subtotal with better formatting
      pdf.setFont('helvetica', 'bold');
      pdf.setTextColor(0, 0, 100);
      pdf.text(`Subtotal:`, totalsLabelX, currentY + 8);
      pdf.setTextColor(0);
      pdf.setFont('helvetica', 'normal');
      pdf.text(`Rs.${subtotal.toFixed(2)}`, totalsValueX, currentY + 8, { align: 'right' });
      currentY += 8;

      // Discount with red color and better formatting
      if (discountAmount > 0) {
        pdf.setFont('helvetica', 'bold');
        pdf.setTextColor(180, 0, 0); // Dark red for discount
        pdf.text(`Discount:`, totalsLabelX, currentY + 6);
        pdf.text(`-Rs.${discountAmount.toFixed(2)}`, totalsValueX, currentY + 6, { align: 'right' });
        pdf.setTextColor(0);
        pdf.setFont('helvetica', 'normal');
        currentY += 8;
      }

      // Tax with green color and better formatting
      if (taxAmount > 0) {
        pdf.setFont('helvetica', 'bold');
        pdf.setTextColor(0, 120, 0); // Dark green for tax
        pdf.text(`Tax:`, totalsLabelX, currentY + 6);
        pdf.text(`Rs.${taxAmount.toFixed(2)}`, totalsValueX, currentY + 6, { align: 'right' });
        pdf.setTextColor(0);
        pdf.setFont('helvetica', 'normal');
        currentY += 8;
      }

      // Space before total (no divider line)
      currentY += 10;

      // TOTAL (Bold and larger with enhanced styling)
      pdf.setFont('helvetica', 'bold');
      pdf.setFontSize(16);
      pdf.setTextColor(0, 0, 180); // Dark blue for total
      pdf.text(`TOTAL:`, totalsLabelX, currentY + 6);
      pdf.setTextColor(0, 0, 150);
      pdf.text(`Rs.${total.toFixed(2)}`, totalsValueX, currentY + 6, { align: 'right' });
      currentY += 15;

      // Payment QR Code Section (if space allows and payment is pending)
      if (total > 0 && currentY < pageHeight - 55 && (paymentMethod === 'online' || paymentMethod === 'split')) {
        currentY += 12;

        // QR Code Section with background color only
        pdf.setFillColor(252, 255, 252); // Light green background
        const qrSectionHeight = 50;
        pdf.rect(marginLeft, currentY, contentWidth, qrSectionHeight, 'F');

        // Payment QR Header with better styling
        pdf.setFont('helvetica', 'bold');
        pdf.setFontSize(14);
        pdf.setTextColor(0, 100, 0);
        pdf.text('💳 Scan to Pay', pageWidth / 2, currentY + 10, { align: 'center' });

        // Generate real scannable QR code with UPI payment URL
        const qrSize = 30; // mm - larger for better scanning
        const qrX = (pageWidth - qrSize) / 2;
        currentY += 14;

        // Create UPI payment URL with seller UPI ID
        const currentUpiId = sellerUpiId || 'merchant@upi';

        const upiUrl = `upi://pay?pa=${currentUpiId}&pn=${encodeURIComponent(state.storeName || 'Store')}&am=${total.toFixed(2)}&cu=INR&tn=Bill%20Payment`;

        try {
          // Generate QR code as base64 image
          const qrCodeDataURL = await QRCode.toDataURL(upiUrl, {
            width: 240, // High resolution for PDF
            margin: 1,
            color: {
              dark: '#000000',
              light: '#FFFFFF'
          ...
          pdf.rect(qrX, currentY, qrSize, qrSize);
          pdf.setFont('helvetica', 'normal');
          pdf.setFontSize(10);
          pdf.setTextColor(200, 0, 0);
          pdf.text('QR Error', qrX + qrSize/2, currentY + qrSize/2, { align: 'center' });
        }

        currentY += qrSize + 6;

        // Payment instructions with better formatting
        pdf.setFont('helvetica', 'normal');
        pdf.setFontSize(10);
        pdf.setTextColor(0, 100, 0);
        pdf.text('Use any UPI app to scan and pay', pageWidth / 2, currentY, { align: 'center' });
        currentY += 6;
        pdf.setFont('helvetica', 'bold');
        pdf.setFontSize(11);
        pdf.setTextColor(0, 0, 150);
        pdf.text(`Amount: Rs.${total.toFixed(2)}`, pageWidth / 2, currentY, { align: 'center' });
      ...
      pdf.text('Generated by Grocery Billing Software', pageWidth / 2, currentY, { align: 'center' });
      currentY += 4;
      pdf.setFontSize(7);
      pdf.setTextColor(120, 120, 120);
      const currentDate = new Date().toLocaleDateString('en-IN') + ' ' + new Date().toLocaleTimeString('en-IN');
  ...
  return (
    <div className="min-h-screen pb-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="relative">
                ...
                <div className="absolute right-2 top-1/2 -translate-y-1/2 flex gap-1">
                  ...
                  <button
                    ...
                    className={`p-1.5 rounded-lg transition-all duration-200 hover:scale-110 active:scale-95 ${
                      showInlineScanner ? 'bg-red-100 border-red-300' : ''
                    }`}
                    style={{ 
                      color: showInlineScanner ? '#dc2626' : 'var(--brand-primary)',
                      background: showInlineScanner ? 'rgba(220, 38, 38, 0.08)' : 'rgba(47, 60, 126, 0.08)',
                      border: showInlineScanner ? '1px solid rgba(220, 38, 38, 0.15)' : '1px solid rgba(47, 60, 126, 0.15)'
                    }}
                    title={showInlineScanner ? "Close Camera Scanner" : "Camera Scanner"}
                  >
              ...
              {filteredProducts.map(product => {
                ...
                return (
                  <div
                    ...
                    className="p-3 rounded-lg hover:bg-blue-50 hover:border-blue-200 transition-all duration-200 border cursor-pointer group relative"
                    style={{ borderColor: 'var(--border-subtle)' }}
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex-1 min-w-0">
                        <h4 className="font-medium truncate group-hover:text-blue-700 transition-colors" style={{ color: 'var(--text-primary)' }} title={product.name}>
                          {product.name}
                        </h4>
                        <p className="text-xs mt-0.5 group-hover:text-blue-600 transition-colors" style={{ color: 'var(--text-secondary)' }}>
                          ₹{(product.sellingPrice || product.costPrice || 0).toFixed(2)}/{product.quantityUnit || product.unit || 'pcs'} • Stock: {getTotalStockQuantity(product)}
                        </p>
                      </div>
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between sm:justify-end gap-2 sm:gap-3 flex-shrink-0">
                      ...
                      <div className="relative flex-shrink-0">
                        <span className={`font-bold text-base sm:text-base sm:w-20 sm:text-right ${item.hasMultipleBatchPrices ? 'cursor-help' : ''}`} style={{ color: 'var(--text-primary)' }}>
        ...
        {showInlineScanner && (
          <div
            className={`fixed z-40 bg-black rounded-lg shadow-lg overflow-hidden transition-all duration-150 ${
              isDragging ? 'cursor-grabbing shadow-xl' : 'cursor-grab'
            }`}
            style={{
              left: `${scannerPosition.x}px`,
              top: `${scannerPosition.y}px`,
          ...
          >
            ...
            >
              <BarcodeScanner
                ...
                inline={true}
                keepOpen={true}
                containerWidth={scannerSize.width}
                containerHeight={scannerSize.height}
              />

              {/* Status indicator - top left */}
              <div className="absolute top-2 left-2 z-20">
                <div className="flex items-center space-x-2 bg-black/60 backdrop-blur-sm rounded-md px-2 py-1 border border-gray-600">
              ...
              <button
                ...
                className="absolute top-2 right-2 z-20 p-1.5 text-white/70 hover:text-white hover:bg-white/10 rounded transition-colors duration-150"
                title="Close scanner"
              >
                <X className="h-4 w-4" />
              </button>
              ...
              <div
                className="absolute bottom-0 right-0 w-3 h-3 cursor-se-resize bg-white/20 hover:bg-white/30 transition-colors duration-150"
                onMouseDown={(e) => {
                  e.stopPropagation();
                ...
                onTouchStart={(e) => {
                  e.stopPropagation();
                  handleTouchResizeStart(e, 'se');
                }}
                title="Resize"
              />
              <div
                className="absolute bottom-0 left-0 w-3 h-3 cursor-sw-resize bg-white/20 hover:bg-white/30 transition-colors duration-150"
                onMouseDown={(e) => {
                  e.stopPropagation();
                ...
                onTouchStart={(e) => {
                  e.stopPropagation();
                  handleTouchResizeStart(e, 'sw');
                }}
                title="Resize"
              />
              <div
                className="absolute top-0 right-0 w-3 h-3 cursor-ne-resize bg-white/20 hover:bg-white/30 transition-colors duration-150"
                onMouseDown={(e) => {
                  e.stopPropagation();
                ...
                onTouchStart={(e) => {
                  e.stopPropagation();
                  handleTouchResizeStart(e, 'ne');
                }}
                title="Resize"
              />
              <div
                className="absolute top-0 left-0 w-3 h-3 cursor-nw-resize bg-white/20 hover:bg-white/30 transition-colors duration-150"
                onMouseDown={(e) => {
                  e.stopPropagation();
