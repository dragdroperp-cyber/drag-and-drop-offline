...
const Billing = () => {
  ...
  const handleReplaceQuantity = (product, quantity, unit) => {
    ...
    setBillItems(prev => {
      ...
      if (existingItemIndex >= 0) {
        ...
        const updatedItem = buildBillItem(product, finalQuantity, finalUnit, stockCheck.baseUnit);
  ...
  const processVoiceInput = (text) => {
    ...
    // First, try to extract all quantity-unit patterns and their positions
    ...
    if (qtyUnitMatches.length > 0) {
      qtyUnitMatches.forEach((qtyUnit, idx) => {
        ...
        const startIdx = idx > 0 ? qtyUnitMatches[idx - 1].index + qtyUnitMatches[idx - 1].length : 0;
        const endIdx = idx < qtyUnitMatches.length - 1 ? qtyUnitMatches[idx + 1].index : normalizedText.length;
        const segment = normalizedText.substring(startIdx, endIdx).trim();
        
        // Remove the quantity-unit pattern to get product name
        const productName = extractProductName(segment.replace(qtyUnit.matchedText, ''));
      ...
      // No quantity found, try to find products by splitting
    ...
    // Use handleAddWithQuantity for all products - it will handle merging automatically
    productMap.forEach(({ product, quantity, unit }) => {
      const added = handleAddWithQuantity(product, quantity, unit);
      if (added) {
        // Check if product was merged or newly added
        const existingItemIndex = billItems.findIndex(item => item.id === product.id);
        if (existingItemIndex >= 0) {
          const existingItem = billItems[existingItemIndex];
          showToast(`Updated: ${product.name} ${formatQuantityWithUnit(existingItem.quantity, existingItem.unit || existingItem.quantityUnit || 'pcs')}`, 'success', 2000);
        } else {
          showToast(`Added: ${product.name} ${formatQuantityWithUnit(quantity, unit)}`, 'success', 2000);
        }
      }
    });
