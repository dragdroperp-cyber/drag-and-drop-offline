...
import { sortBatches, getEffectivePrice, getEffectiveWholesaleMOQ } from '../../utils/productUtils';
...
const Billing = () => {
  ...
  const updateQuantity = (productId, quantity) => {
    ...
    if (saleMode === 'wholesale') {
      ...
      // Only block if trying to go BELOW MOQ (allow existing if already below for some reason, but block further reduction? Or block if target < MOQ)
      // Blocking if target < MOQ ensures we respect the rule.
  ...
  const generateAndDownloadPDF = async (bill) => {
    try {
      ...
      bill.items.forEach((item, idx) => {
        ...
        pdf.setTextColor(...COLORS.text);
        pdf.setFont('helvetica', 'bold');
        safeDrawText(pdf, item.name, margin + 12, y + 6, { fontSize: 9 });
        pdf.setFontSize(7);
        pdf.setTextColor(...COLORS.slate400);
        pdf.setFont('helvetica', 'normal');
      ...
      try {
        ...
        pdf.setTextColor(...COLORS.slate400);
        pdf.setFont('helvetica', 'bold');
        pdf.text('SCAN TO VERIFY\nDIGITAL INVOICE', margin + 25, y + 8);
      } catch (qrErr) {
  ...
  const generateThermalBill = async (size, invoiceNumber = null, billData = null) => {
    ...
    estimatedHeight += items.length * 10; // Items
    estimatedHeight += 50; // GST Table
    estimatedHeight += 50; // Footer

    const pdf = new jsPDF('p', 'mm', [width, estimatedHeight + 50]);
    ...
    // Store Name: Blue and Bold (or Accent Color)
    pdf.setFont('helvetica', 'bold');
    ...
    // Address & Details: centered, black, small
    pdf.setTextColor(0, 0, 0);
    pdf.setFont('helvetica', 'bold'); // "Elangkavu..." seems bold/thick in image
    pdf.setFontSize(8);
    ...
    // Invoice Meta (Inv No ... Date)
    // Image: Inv No GST-... (Left)   Date 29... (Right)
    ...
    // Invoice No Label Red? Image shows "Inv No" in Red, Value Black. "Date" Red, Value Black.
    // Let's replicate strict colors if possible, or stick to black for reliability. 
    // Image has dark red/brown labels.
    ...
    pdf.setTextColor(0, 0, 0); // Black
    pdf.setFont('helvetica', 'bold');
    // If 58mm, might need to wrap if bill No is long.
    if (size === '58mm' && billNo.length > 12) {
      pdf.text(billNo, margin, metaY + 3.5); // Next line
      // Then Date needs to move down? 
      // For simplicity, let's keep inline but concise
    } else {
    ...
    pdf.setTextColor(150, 0, 0); // Red Label
    ...
    /* ================= TABLE HEADER ================= */
    // Columns: Sl.No., Item Name, QTY., Price, Amount
    // 58mm: Sl (4), Item (24), Qty(8), Price(10), Amt(12) -> Compressed
    // 80mm: Sl (8), Item (30), Qty(10), Price(12), Amt(18) -> Spacious

    pdf.setFontSize(size === '58mm' ? 7 : 8); // Slightly smaller for 58mm to fit full headers
    pdf.setFont('helvetica', 'bold');

    // User wants 58mm to look EXACTLY like 80mm including header names.
    // 80mm Headers: "Sl.No.", "Item Name", "QTY.", "Price", "Amount"

    const cols = size === '58mm' ? [
      { name: "Sl.No.", x: margin, align: 'left' }, // "Sl.No."
      { name: "Item Name", x: margin + 8, align: 'left' },
      { name: "QTY.", x: width - margin - 22, align: 'right' }, // "QTY."
    ...
    items.forEach((item, index) => {
      ...
      // Item Name
      const xItem = cols[1].x;
      // Max width for item name
      // 58mm: xItem (9) to xQty (~34) => ~25mm
      const maxItemWidth = size === '58mm' ? 22 : 35;

      const nameLines = pdf.splitTextToSize(item.name, maxItemWidth);
      pdf.text(nameLines, xItem, y);
      ...
      // Amount
      pdf.text(totalAmount.toFixed(2), cols[4].x, y, { align: 'right' });
    ...
    /* ================= GST SUMMARY TABLE ================= */
    // Columns: Tax % | Taxable Val | CGST | SGST | TOTAL G
    // User wants 58mm matches 80mm structure (split CGST/SGST).
    ...
    if (Object.keys(gstSummary).length > 0) {
      // Use very small font for 58mm to fit split columns
      pdf.setFontSize(size === '58mm' ? 6 : 7);
      pdf.setFont('helvetica', 'normal');

      const gCols = size === '58mm' ? [
        { n: "Tax %", x: margin, w: 8 },
        { n: "Taxable Val", x: margin + 11, w: 15 },
        { n: "CGST", x: margin + 30, w: 8 },
        { n: "SGST", x: margin + 38, w: 8 },
        { n: "GST", x: width - margin - 5, w: 12, align: 'center' } // Centered GST
      ] : [
        { n: "Tax %", x: margin, w: 10 },
        { n: "Taxable Val", x: margin + 12, w: 18 },
        { n: "CGST", x: margin + 32, w: 12 },
        { n: "SGST", x: margin + 46, w: 12 },
        { n: "GST", x: width - margin - 12, w: 15, align: 'center' } // Centered GST
      ];
      ...
      Object.keys(gstSummary).forEach(rate => {
        ...
        // Standardize output for both sizes
        if (size === '58mm') {
    ...
    /* ================= FINAL BIG TOTAL ================= */
    const discount = billData?.discountAmount || 0;
    const grandTotal = subTotal - discount;

    pdf.setFontSize(14);
    pdf.setFont('helvetica', 'bold');

    // Add some padding before Total
    y += 2;

    pdf.text("Total", margin, y);
    const grandTotalStr = `${Math.round(grandTotal).toFixed(2)}`;
    pdf.text(grandTotalStr, width - margin, y, { align: 'right' });
    y += 6; // Space below text baseline
    ...
    if (grandTotal > 0) {
      ...
      if (sellerUpiIdValue && sellerUpiIdValue.includes('@')) {
        try {
          ...
          if (qrResult) {
            const qrSize = size === '58mm' ? 25 : 30; // 25mm for 58mm, 30mm for 80mm
    ...
    // handlePDFOutput...
    if (invoiceNumber) {
      handlePDFOutput(pdf, `Receipt-${invoiceNumber}.pdf`);
    } else {
      handlePDFOutput(pdf, `Receipt-Preview.pdf`);
    }
  ...
  const generatePaperBill = async (pdf, invoiceNumber = null, billData = null) => {
    try {
      ...
      const logoShow = settings.showLogo !== undefined ? settings.showLogo : (settings.header?.showLogo ?? true);
      if (logoShow) {
        // Logo logic same as generateAndDownloadPDF
        logoOffset = 20;
      }
      ...
      items.forEach((item, index) => {
        ...
        pdf.text(descLines, cols[1].x + 2, cy);
      ...
      if (billData?.paymentMethod === 'split' && billData?.splitPaymentDetails) {
        pdf.text(`Paid: ${billData.splitPaymentDetails.cashAmount + billData.splitPaymentDetails.onlineAmount}`, tX, ty + 5);
        ty += 5;
        pdf.text(`Balance Due: ${billData.splitPaymentDetails.dueAmount}`, tX, ty + 5);
  ...
  const calculateBatchPricing = (product, requestedQuantity, unit) => {
    ...
    if (product.batches && product.batches.length > 0) {
      // Step 1: Fetch batches with stock > 0. Step 2: Sort - Wholesale uses FEFO (Expiry ASC)
      const availableBatches = product.batches.filter(b => (Number(b.quantity) || 0) > 0);
      const sortedBatches = saleMode === 'wholesale'
