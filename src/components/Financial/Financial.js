...
const Financial = () => {
      ...
      tooltip: {
        callbacks: {
          label: function (context) {
            ...
            return `${label}: ${formatCurrencySmart(value, state.currencyFormat)} (${count} ${count === 1 ? 'order' : 'orders'}, ${percentage}%)`;
          }
        },
  ...
  const exportFinancialPDF = async () => {
    try {
      ...
      pdf.text('Financial Management', logoX + 22, 19);

      /* -------- RIGHT META -------- */
      pdf.setFontSize(12);
      pdf.setFont('helvetica', 'bold');
      pdf.setTextColor(...COLORS.black);
      pdf.text('Financial Report', pageWidth - margin, 14, { align: 'right' });
      ...
      const metrics = [
        { label: 'Revenue', value: formatCurrencySmart(totalRevenue, state.currencyFormat) },
        { label: 'Expenses', value: formatCurrencySmart(totalBusinessExpenses, state.currencyFormat) },
        { label: 'Net Profit', value: formatCurrencySmart(netProfit, state.currencyFormat) }
      ];
      ...
      pdf.setFont('helvetica', 'bold');
      pdf.setTextColor(...COLORS.primary);
      pdf.text('Selected Period Summary', margin, y);
      ...
      pdf.text(`Analysis for period: ${periodLabel}`, margin, y);
      y += 15; // Increased gap after subtitle to 15mm

      const summaryData = [
        ['SALES', 'Total Revenue (Sales)', formatCurrencySmart(totalRevenue, state.currencyFormat)],
        ['SALES', 'Cost of Goods Sold (COGS)', formatCurrencySmart(totalCogs, state.currencyFormat)],
        ['OPEX', 'Operating Expenses (Petty)', formatCurrencySmart(totalPettyExpenses, state.currencyFormat)],
        ['PROFIT', 'Net Profit', formatCurrencySmart(netProfit, state.currencyFormat)],
        ['PROFIT', 'Profit Margin', `${profitMargin.toFixed(2)}%`],
        ['CASH', 'Purchases (Stock Cash Out)', formatCurrencySmart(totalPurchaseExpenses, state.currencyFormat)],
        ['CREDIT', 'Total Receivables', formatCurrencySmart(totalReceivables, state.currencyFormat)],
        ['CREDIT', 'Customers with Debt', customersWithDebt.toString()]
      ];

      const rowH_summary = 12;
      const tableW_summary = pageWidth - margin * 2;
      const colW_summary = [45, 150, tableW_summary - 195];
      const headers_summary = ['Section', 'Metric', 'Value'];
    ...
    <div className="space-y-6 sm:space-y-8 fade-in-up">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-3">
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            ...
            <button
              ...
              className="h-10 flex-1 sm:flex-none flex items-center justify-center gap-2 px-4 bg-rose-50 text-rose-600 hover:bg-rose-100 dark:bg-rose-900/20 dark:text-rose-400 dark:hover:bg-rose-900/30 rounded-lg text-sm font-medium transition-colors shadow-sm whitespace-nowrap"
            >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        {[
          ...
          { title: 'Total Expenses', value: totalBusinessExpenses, color: 'red', icon: <TrendingDown /> },
          ...
          <div key={idx} className="relative bg-white dark:bg-slate-800 p-4 sm:p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 transition-all hover:shadow-md">
            ...
            <div className="mt-2">
              ...
              <p className={`text-2xl font-semibold whitespace-nowrap overflow-x-auto scrollbar-hide ${card.title === 'Total Revenue' ? 'text-emerald-600' :
                card.title === 'Total Expenses' ? 'text-rose-600' :
                  card.title === 'Net Profit' ? (card.value >= 0 ? 'text-emerald-600' : 'text-rose-600') :
                    card.title === 'Receivables' ? 'text-amber-600' :
                      'text-gray-900 dark:text-white'
                }`} title={formatCurrency(card.value)}>
                {formatCurrencySmart(card.value, state.currencyFormat)}
              </p>
              {card.title === 'Total Expenses' && totalBusinessExpenses > 0 && (
                <p className="text-[10px] text-gray-500 mt-2">
                  COGS: <span title={formatCurrency(totalCogs)}>{formatCurrencySmart(totalCogs, state.currencyFormat)}</span> • Petty: <span title={formatCurrency(totalPettyExpenses)}>{formatCurrencySmart(totalPettyExpenses, state.currencyFormat)}</span>
                </p>
              )}
            </div>
          </div>
              ...
              <div className="p-4 bg-amber-50 dark:bg-amber-900/10 rounded-lg border border-amber-100 dark:border-amber-900/30 flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-700 dark:text-slate-200">
                    <span title={formatCurrency(totalReceivables)}>{formatCurrencySmart(totalReceivables, state.currencyFormat)}</span> outstanding from {customersWithDebt} customers.
                  </p>
                </div>
              </div>
            )}
            ...
            {netProfit < 0 && (
              <div className="p-4 bg-red-50 dark:bg-red-900/10 rounded-lg border border-red-100 dark:border-red-900/30 flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-700 dark:text-slate-200">
                    Negative net profit of <span title={formatCurrency(Math.abs(netProfit))}>{formatCurrencySmart(Math.abs(netProfit), state.currencyFormat)}</span>.
                  </p>
                </div>
              </div>
            )}
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          {filteredDailyExpenses.length > 0 && (
            <span className="text-sm text-gray-500 dark:text-slate-400">
              Total: <span title={formatCurrency(totalPettyExpenses)}>{formatCurrencySmart(totalPettyExpenses, state.currencyFormat)}</span>
            </span>
          )}
        </div>
              ...
              <form onSubmit={handleAddExpense} className="p-4 space-y-4">
                ...
                <div className="pt-2 flex gap-3">
                  ...
                  <button
                    ...
                    className="flex-1 py-2.5 bg-gradient-to-r from-rose-500 to-pink-600 text-white rounded-lg font-bold hover:shadow-lg disabled:opacity-50 transition-all flex items-center justify-center gap-2"
                  >
                    {isSubmittingExpense ? (
                      <>
      ...
      {/* Delete Confirmation Modal */}
      {
        showDeleteConfirmModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={() => setShowDeleteConfirmModal(false)}>
            <div
              className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden animate-in fade-in zoom-in-95 duration-200 border border-gray-100 dark:border-slate-700 p-6 text-center"
              onClick={e => e.stopPropagation()}
            >
            ...
            <div className="flex items-center justify-between p-6 border-b border-gray-100 dark:border-slate-700 bg-gray-50/50 dark:bg-slate-800/50">
              <div>
                ...
                <p className="text-sm text-gray-500 dark:text-slate-400 mt-1">
                  Hourly analysis for <span className="font-semibold text-indigo-600 dark:text-indigo-400">{selectedDate}</span>
                </p>
              </div>
              <button
                onClick={handleCloseBreakdown}
            ...
            <div className="p-6">
              ...
              <div className="mt-6 flex justify-end gap-4">
                <div className="bg-emerald-50 dark:bg-emerald-900/20 px-4 py-3 rounded-xl border border-emerald-100 dark:border-emerald-900/30">
                  <p className="text-xs text-emerald-600 dark:text-emerald-400 uppercase tracking-wide font-bold">Total Revenue</p>
                ...
                <div className="bg-rose-50 dark:bg-rose-900/20 px-4 py-3 rounded-xl border border-rose-100 dark:border-rose-900/30">
                  <p className="text-xs text-rose-600 dark:text-rose-400 uppercase tracking-wide font-bold">Total Expenses</p>
