  ...
  const exportFinancialPDF = async () => {
    try {
      ...
      const metrics = [
        ...
        { label: 'Expenses', value: formatCurrency(totalBusinessExpenses) },
      ...
      const summaryData = [
        ['SALES', 'Total Revenue (Sales)', formatCurrency(totalRevenue)],
        ['SALES', 'Cost of Goods Sold (COGS)', formatCurrency(totalCogs)],
        ['OPEX', 'Operating Expenses (Petty)', formatCurrency(totalPettyExpenses)],
        ['PROFIT', 'Net Profit', formatCurrency(netProfit)],
        ['PROFIT', 'Profit Margin', `${profitMargin.toFixed(2)}%`],
        ['CASH', 'Purchases (Stock Cash Out)', formatCurrency(totalPurchaseExpenses)],
        ['CREDIT', 'Total Receivables', formatCurrency(totalReceivables)],
            ...
            <div className="mt-2">
              ...
              {card.title === 'Total Expenses' && totalBusinessExpenses > 0 && (
                <p className="text-[10px] text-gray-500 mt-2">
                  COGS: ₹{totalCogs.toLocaleString('en-IN', { maximumFractionDigits: 0 })} • Petty: ₹{totalPettyExpenses.toLocaleString('en-IN', { maximumFractionDigits: 0 })}
                </p>
              )}
            </div>
          </div>
