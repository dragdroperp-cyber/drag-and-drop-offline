    ...
    return filterBySeller(state.orders)
      ...
      .map(order => {
        ...
        const newTotal = filteredItems.reduce((sum, item) => {
          // Use totalSellingPrice or sellingPrice directly (treating it as line total) to match Profit calculation logc and avoid double-counting quantity
          const itemTotal = Number(item.totalSellingPrice ?? item.total ?? item.amount ?? item.sellingPrice ?? 0);
