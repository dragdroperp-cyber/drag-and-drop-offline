...
const Financial = () => {
  ...
  const getFinancialMonthsData = () => {
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      const monthRev = state.orders.filter(o => {
      ...
      // Expenses for this month (Purchase Orders + Petty Expenses)
      const monthPO = state.purchaseOrders.filter(po => {
      ...
      }).reduce((sum, po) => sum + (resolvePurchaseOrderTotal(po) || 0), 0);

      const monthPetty = (Array.isArray(state.expenses) ? state.expenses : []).filter(exp => {
        const expDate = new Date(exp.date || exp.createdAt);
        return expDate.getMonth() === d.getMonth() && expDate.getFullYear() === d.getFullYear();
      }).reduce((sum, exp) => sum + (Number(exp.amount) || 0), 0);

      expenses.push(monthPO + monthPetty);
    }

    return { labels, revenues, expenses };
  };

  const { labels: chartLabels, revenues: monthlyRevenue, expenses: monthlyExpenses } = useMemo(() => getFinancialMonthsData(), [state.orders, state.purchaseOrders, state.expenses]);
  ...
  return (
    <div className="space-y-6 sm:space-y-8 fade-in-up dark:bg-slate-900">
      {/* Header */}
