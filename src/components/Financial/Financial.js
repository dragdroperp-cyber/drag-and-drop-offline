    ...
    try {
      ...
      const addWatermark = (doc) => {
        ...
        for (let i = 1; i <= totalPages; i++) {
          ...
          doc.text('FINANCIAL REPORT', pageWidth / 2, pageHeight / 2, {
            align: 'center',
            angle: 45
          });
      ...
      pdf.setFillColor(...COLORS.primary);
      pdf.roundedRect(pageWidth - 75, 24, 60, 7, 1.5, 1.5, 'F');
      pdf.setTextColor(...COLORS.white);
      pdf.setFontSize(7.5);
      pdf.setFont('helvetica', 'bold');
      pdf.text(`GEN: ${formatDateTime(new Date())}`, pageWidth - margin - 15, 28.5, { align: 'right' });
      ...
      for (let i = 1; i <= pageCount; i++) {
        ...
        pdf.text(
          state.storeName || state.currentUser?.shopName || 'Store',
          pageWidth - margin,
          pageHeight - 10,
          { align: 'right' }
        );
