          ...
          return (
            <div key={card.id} className="relative bg-white dark:bg-slate-800 p-4 sm:p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 transition-all hover:shadow-md">
              ...
              <div className="mt-2 text-left">
                ...
                {(isLoading || (card.id === 'totalRefunds' && state.dataFreshness === 'loading' && (!state.refunds || state.refunds.length === 0))) ? (
                  <SkeletonCard className={`h-8 w-32 rounded ${card.color === 'emerald' ? 'bg-emerald-200 dark:bg-emerald-900/30' : card.color === 'rose' ? 'bg-rose-200 dark:bg-rose-900/30' : 'bg-gray-200 dark:bg-gray-700'}`} />
                ) : (
