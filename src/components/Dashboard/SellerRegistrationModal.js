...
import {
  ...
  Users, Package, Wallet, TrendingUp, Truck, AlertTriangle, Clock, ChevronRight, Search, Menu, Bell
} from 'lucide-react';
...
const InputWrapper = ({ label, error, children, icon: Icon, required }) => (
  <div className="group relative mb-5">
    <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2 ml-1">{label} {required && <span className="text-red-500">*</span>}</label>
    <div className="relative flex items-center">
      <div className="absolute inset-y-0 left-0 flex items-center pointer-events-none text-slate-400 group-focus-within:text-blue-600 transition-colors"><Icon className="h-5 w-5" /></div>
      {React.cloneElement(children, { className: `block w-full pl-9 pr-4 py-3 bg-transparent border-b border-slate-300 text-slate-900 placeholder:text-slate-300 focus:outline-none focus:border-slate-500 transition-all font-medium rounded-none ${error ? 'border-red-400' : ''}` })}
      {error && <div className="absolute inset-y-0 right-0 flex items-center pointer-events-none"><div className="h-5 w-5 text-red-500 flex items-center justify-center">!</div></div>}
    </div>
    {error && <p className="mt-1.5 text-xs font-medium text-red-500 animate-fade-in">{error}</p>}
  </div>
);
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-6 shrink-0">
        ...
        <div className="inline-flex rounded-full border border-slate-200 bg-white p-1 shadow-sm w-fit">
          {['Today', '7d', '30d', 'Custom'].map((t, i) => (
            <span key={t} className={`px-3 py-1 text-xs font-medium rounded-full ${i === 2 ? 'bg-[#2f3c7e] text-white shadow' : 'text-slate-600'}`}>
              {t}
            </span>
          ))}
        </div>
...
const SellerRegistrationModal = ({ isOpen, onClose }) => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (!response.success) throw new Error(response.error || response.data?.message || 'Failed to complete registration');
  ...
  return createPortal(
    <div className="fixed inset-0 z-[9999] bg-white flex h-screen w-screen overflow-hidden font-sans m-0 p-0">
      ...
      <div className="hidden lg:block lg:w-1/2 xl:w-7/12 bg-blue-600 relative overflow-hidden h-screen flex flex-col items-center justify-center">
      ...
      <div className="w-full lg:w-1/2 xl:w-5/12 h-screen overflow-y-auto custom-scrollbar bg-white px-6 py-8 sm:px-12 sm:py-12 flex flex-col">
        ...
        <form onSubmit={handleSubmit} className="flex-1 max-w-lg space-y-8">
          ...
          <div className="pt-2 pb-10">
            <button type="submit" disabled={isSubmitting} className="w-full h-14 rounded-full bg-blue-600 hover:bg-blue-700 text-white font-bold text-lg shadow-lg hover:shadow-blue-600/30 transition-all flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed transform active:scale-98">
