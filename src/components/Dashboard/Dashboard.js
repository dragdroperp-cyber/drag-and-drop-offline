...
const loadStaffData = async (dispatch, permissions) => {
  try {
    console.log('👤 Loading staff data immediately after login...');

    // Load products data immediately for staff
    const products = await STORES.products.getAll();
    console.log('📦 Loaded products for staff:', products?.length || 0);
    ...
    if (permissions?.orders?.read !== false) {
      ...
      console.log('📋 Loaded orders for staff:', orders?.length || 0);
    ...
    if (permissions?.customers?.read !== false) {
      ...
      console.log('👥 Loaded customers for staff:', customers?.length || 0);
    ...
    console.log('✅ Staff data loading completed');
  } catch (error) {
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">Business Overview</h1>
