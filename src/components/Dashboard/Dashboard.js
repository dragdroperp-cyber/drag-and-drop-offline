...
const Dashboard = () => {
  ...
  const sellerOrders = useMemo(() => {
    ...
    return rawSellerOrders.filter(order => {
        ...
        (orderRefunds || []).forEach(refund => {
          if (Array.isArray(refund.items) && refund.items.length > 0) {
            refund.items.forEach(refundItem => {
              const refundPid = (refundItem.productId || refundItem.product_id || refundItem._id || refundItem.id || '').toString();
              ...
              if (filteredItems.some(item => {
                const iPid = (item.productId || item.product_id || item._id || '').toString();
                const namesMatch = item.name && refundItem.name &&
                  item.name.trim().toLowerCase() === refundItem.name.trim().toLowerCase();
                const iPidMatches = iPid && refundPid && iPid === refundPid && iPid !== '' && iPid !== 'null' && iPid !== 'undefined';

                const isDP = item.isDProduct === true || String(item.isDProduct) === 'true' ||
                  refundItem.isDProduct === true || String(refundItem.isDProduct) === 'true';

                return iPidMatches || (namesMatch && isDP);
              })) {
                const qty = Number(refundItem.qty || 0);
                const rate = Number(refundItem.rate || 0);
                refundedAmountForFilteredItems += (qty * rate);
              }
            });
          } else {
            // Fallback for simple refunds
            const totalRefundAmount = Number(refund.totalRefundAmount || refund.amount || 0);
      ...
      let totalLumpSumRefund = 0;
      if (orderRefunds && Array.isArray(orderRefunds)) {
        orderRefunds.forEach(r => {
          if (!r.items || !Array.isArray(r.items) || r.items.length === 0) {
            totalLumpSumRefund += Number(r.totalRefundAmount || r.amount || 0);
          }
        });
      }
      const allocatedLumpSumRefund = totalLumpSumRefund * proportionalFactor;

      // Return a new order object with filtered items and recalculated totals
      return {
        ...order,
        items: filteredItems,
        totalAmount: finalCalculatedTotal, // Exclude delivery from "Sales" to match Reports
        total: finalCalculatedTotal,
        deliveryCharge: (proportionalFactor * deliveryCharge),
        allocatedDiscount: allocatedDiscount,
        allocatedLumpSumRefund: allocatedLumpSumRefund,
        refundedAmount: refundedAmountForFilteredItems,
  ...
  const calculateProfitFromOrderItems = (orders) => {
    ...
    return orders.reduce((totalProfit, order) => {
      // order.totalAmount is already proportional Net Revenue (after discounts and all refunds)
      const netRevenue = toNumber(order.totalAmount || 0);

      // Calculate Net Cost for these items
      const items = Array.isArray(order.items) ? order.items : [];
      const grossCost = items.reduce((sum, item) => {
        return sum + toNumber(item.totalCostPrice ?? item.costPrice ?? 0);
      }, 0);

      // Handle item-specific cost reduction for refunds
      let refundedCost = 0;
      const orderId = (order._id || order.id || '').toString();
      const orderRefunds = (state.refunds || []).filter(r =>
        (r.orderId || r.order_id || '').toString() === orderId
      );
      ...
      orderRefunds.forEach(refund => {
        if (Array.isArray(refund.items) && refund.items.length > 0) {
          refund.items.forEach(ri => {
            ...
            // Match with items in this order
            const matchedItem = items.find(item => {
              ...
              return riPid === iPid || (namesMatch && isDP);
            });
            ...
            if (matchedItem) {
              ...
              refundedCost += (qty * unitCost);
            }
          });
        }
      });

      const netCost = Math.max(0, grossCost - refundedCost);
      return totalProfit + (netRevenue - netCost);
    }, 0);
  };
  ...
  const calculateProfitForRange = (orders, purchaseOrders, startDate, endDate) => {
    ...
    return calculateProfitFromOrderItems(filteredOrders) - expenseCost;
  };
  ...
  const totalProfit = calculateProfitFromOrderItems(sellerOrders) - (saleMode === 'direct' ? 0 : sellerPurchaseOrders.reduce((sum, po) => sum + (po.total || 0), 0)) - (saleMode === 'direct' ? 0 : totalPettyExpenses);
  ...
  const todayProfit = calculateProfitFromOrderItems(todayOrders) - (saleMode === 'direct' ? 0 : todayPurchaseOrders.reduce((sum, po) => sum + (po.total || 0), 0)) - (saleMode === 'direct' ? 0 : todayExpenses);
  ...
  const getNetProfitForPeriod = () => {
    ...
    // In Direct Sale mode, Net Profit = Gross Profit (Sales Profit)
    // We ignore Purchase Orders and Expenses as they are related to Inventory/Shop operations
    if (saleMode === 'direct') {
      return salesProfit;
    }
    ...
    // Purchase Orders are EXCLUDED from Net Profit (they are assets/inventory), but Expenses are subtracted
    const pettyExpensesCost = periodExpenses.reduce((sum, exp) => sum + (Number(exp.amount) || 0), 0);

    // Net Profit = Gross Profit (Sales Profit) - Operating Expenses (Petty Expenses)
    return salesProfit - pettyExpensesCost;
  };
  ...
  // Monthly Profit = Gross Profit (from sales) (Purchase Orders are excluded)
  const monthlyProfit = calculateProfitFromOrderItems(monthlyOrders);
  ...
  // Calculate total refunds for the selected period across all modes (business-wide total)
  const totalRefundsForPeriod = useMemo(() => {
    ...
    return (state.refunds || []).filter(refund => {
      ...
      if (sellerIdStr) {
        ...
        if (rSellerId && rSellerId !== sellerIdStr) return false;
      }
  ...
  }, [state.refunds, sellerId, timeRange, customDateRange]);
