...
const Dashboard = () => {
  ...
  const stats = [
    ...
    {
      name: getTranslation('salesProfit', state.currentLanguage),
      value: formatNumber(getSalesProfitForPeriod()),
      icon: TrendingUp,
      description: `${getTranslation('salesProfit', state.currentLanguage)} - ${getPeriodLabel(timeRange)}`,
      theme: 'purple',
      onClick: () => goToView('financial')
    },
    {
      name: getTranslation('salesProfitMargin', state.currentLanguage),
      value: `${getSalesProfitMarginForPeriod().toFixed(1)}%`,
      icon: TrendingUp,
      description: `${getTranslation('salesProfitMargin', state.currentLanguage)} - ${getPeriodLabel(timeRange)}`,
      theme: 'teal',
      onClick: () => goToView('financial')
    }
  ];
              ...
              {quickActions.map((action) => {
                ...
                return (
                  <button
                    ...
                    className={`group relative overflow-hidden rounded-2xl border border-white/25 bg-white/10 px-5 py-5 text-left text-white shadow-[0_24px_70px_-40px_rgba(15,23,42,0.6)] transition-all duration-200 focus:outline-none ${isPlanRestricted ? 'opacity-50 cursor-not-allowed grayscale-[0.5]' : 'hover:-translate-y-1 hover:shadow-[0_32px_80px_-42px_rgba(15,23,42,0.65)] active:scale-95'
                      }`}
                    ...
                    <div className="flex h-full flex-col justify-between gap-4">
                      ...
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          ...
                          {isPlanRestricted && <Lock className="h-3.5 w-3.5 text-white/60" />}
                        </div>
