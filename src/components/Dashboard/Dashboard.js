...
const Dashboard = () => {
  ...
  const [isLoading, setIsLoading] = useState(() => {
    ...
    const hasData = state.products?.length > 0 || state.customers?.length > 0 || state.orders?.length > 0 || state.refunds?.length > 0;
  ...
  useEffect(() => {
    ...
    const hasData = state.products && state.customers && state.orders && state.transactions && state.refunds;
    if (hasData) {
      setIsLoading(false);
    } else {
  ...
  }, [state.products, state.customers, state.orders, state.transactions, state.refunds]);
  ...
  const stats = [
    ...
    {
      name: 'Refunds',
      value: (isLoading || (state.dataFreshness === 'loading' && (!state.refunds || state.refunds.length === 0))) ? <SkeletonCard className="h-8 w-24 bg-rose-200 dark:bg-rose-900/30 rounded" /> : formatNumber(totalRefundsForPeriod),
      icon: XCircle,
