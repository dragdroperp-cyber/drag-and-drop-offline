      ...
      <div className="w-full lg:w-1/2 xl:w-5/12 h-screen overflow-y-auto custom-scrollbar bg-white px-6 py-8 sm:px-12 sm:py-12 flex flex-col">
        ...
        <form onSubmit={handleSubmit} className="flex-1 max-w-lg space-y-8">
          ...
          <div className="pt-2 pb-10">
            ...
            {formMessage.text && (
              <div className={`mt-4 p-3 rounded-lg text-sm font-semibold flex items-center gap-2 animate-in fade-in slide-in-from-top-2 duration-300 ${formMessage.type === 'success' ? 'bg-emerald-50 text-emerald-700 border border-emerald-100' :
                formMessage.type === 'warning' ? 'bg-amber-50 text-amber-700 border border-amber-100' :
                  'bg-red-50 text-red-700 border border-red-100'
                }`}>
                <div className={`h-5 w-5 rounded-full flex items-center justify-center text-white shrink-0 ${formMessage.type === 'success' ? 'bg-emerald-500' :
