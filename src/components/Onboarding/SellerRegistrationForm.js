...
const SellerRegistrationForm = () => {
  ...
  const validate = () => {
    ...
    if (form.shopAddress && form.shopAddress.trim().length < 5) {
      nextErrors.shopAddress = 'Address must be at least 5 characters long';
    }
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      // If backend returned specific field errors, map them to the form
      if (error.details && Array.isArray(error.details)) {
        const backendErrors = {};
        error.details.forEach(detail => {
          // Map backend field names to frontend form state names if they match
          if (detail.field) {
        ...
        if (Object.keys(backendErrors).length > 0) {
          ...
          return; // Stop here, don't show generic error toast
        }
      }
