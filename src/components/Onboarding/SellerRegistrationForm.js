...
const InputWrapper = ({ label, error, children, icon: Icon, required }) => (
  <div className="space-y-2 mb-6 group">
    <div className="flex justify-between items-center px-1">
      <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">{label} {required && <span className="text-blue-500">*</span>}</label>
      {error && (
    ...
    <div className="relative">
      <div className={`absolute left-4 top-1/2 -translate-y-1/2 transition-all duration-300 ${error ? 'text-rose-500' : 'text-slate-300 group-focus-within:text-slate-900'}`}>
        <Icon size={18} strokeWidth={2.5} />
      </div>
      {React.cloneElement(children, {
        className: `block w-full pl-12 pr-4 py-4 bg-slate-50/50 border border-slate-200/60 transition-all duration-300 rounded-[1.25rem] text-sm font-bold text-slate-900 placeholder:text-slate-300 placeholder:font-medium outline-none shadow-sm ${error
          ? 'border-rose-200 bg-rose-50/20'
          : 'hover:border-slate-300 hover:bg-white focus:bg-white focus:border-slate-900 focus:shadow-[0_8px_30px_rgb(0,0,0,0.04)]'
          }`
      })}
    </div>
  </div>
...
const CustomSelect = ({ label, value, onChange, options, placeholder, icon: Icon, required, error }) => {
  ...
  return (
    <div className={`space-y-2 mb-6 group relative ${isOpen ? 'z-[10002]' : 'z-10'}`} ref={containerRef}>
      <div className="flex justify-between items-center px-1">
        <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">{label} {required && <span className="text-blue-500">*</span>}</label>
        {error && (
          <span className="text-[10px] text-rose-500 font-bold flex items-center gap-1 animate-pulse">
      ...
      <div className="relative">
        <button
          ...
          className={`group flex items-center w-full pl-12 pr-4 py-4 bg-slate-50/50 border transition-all duration-300 rounded-[1.25rem] text-sm font-bold shadow-sm text-left outline-none ${error ? 'border-rose-200 bg-rose-50/20 text-rose-900' : (isOpen ? 'border-slate-900 bg-white shadow-[0_8px_30px_rgb(0,0,0,0.04)]' : 'border-slate-200/60 hover:border-slate-300 hover:bg-white text-slate-900')
            }`}
        >
          <div className={`absolute left-4 top-1/2 -translate-y-1/2 transition-all duration-200 ${error ? 'text-rose-500' : (isOpen ? 'text-slate-900' : 'text-slate-300 group-hover:text-slate-900')}`}>
            <Icon size={18} strokeWidth={2.5} />
          </div>
          <span className={`truncate mr-4 ${value ? 'text-slate-900' : 'text-slate-300 font-medium'}`}>
            {value || placeholder}
          </span>
          <ChevronDown size={18} className={`ml-auto shrink-0 transition-transform duration-300 ${isOpen ? 'rotate-180 text-slate-900' : 'text-slate-400'}`} strokeWidth={2.5} />
        </button>

        {isOpen && (
          <div className="absolute z-[10003] top-full mt-2 w-full bg-white border border-slate-100 rounded-[1.25rem] shadow-[0_30px_60px_rgba(0,0,0,0.15)] py-2 animate-in fade-in zoom-in-95 slide-in-from-top-2 duration-300 overflow-hidden max-h-[300px] overflow-y-auto custom-scrollbar">
            {options.map((opt) => (
              <button
                key={opt}
                type="button"
                ...
                className={`w-full px-6 py-4 text-left text-sm font-bold transition-all flex items-center justify-between group/item ${value === opt ? 'bg-slate-50 text-slate-900' : 'text-slate-500 hover:bg-slate-50 hover:pl-8'
                  }`}
              >
                {opt}
                {value === opt && (
                  <div className="h-6 w-6 rounded-full bg-slate-900 flex items-center justify-center animate-in zoom-in duration-300 shadow-lg">
          ...
          <form onSubmit={handleSubmit} className="space-y-16">
            ...
            <div className="relative">
              <div className="flex items-center gap-3 mb-8">
                ...
                <h3 className="text-xs font-black text-slate-900 uppercase tracking-[0.2em]">Business Identity</h3>
              </div>
            ...
            <div>
              <div className="flex items-center gap-3 mb-8">
                ...
                <h3 className="text-xs font-black text-slate-900 uppercase tracking-[0.2em]">Location & Reach</h3>
              </div>
              <div className="space-y-8">
                ...
                <InputWrapper label="Operating Address" required error={errors.shopAddress} icon={MapPin}><input type="text" value={form.shopAddress} onChange={handleChange('shopAddress')} placeholder="Street, Area, Landmark" maxLength={200} /></InputWrapper>
            ...
            <div>
              <div className="flex items-center gap-3 mb-8">
                ...
                <h3 className="text-xs font-black text-slate-900 uppercase tracking-[0.2em]">Financial & Personal</h3>
              </div>
              <div className="space-y-8">
                <div className="relative">
                ...
                <InputWrapper label="WhatsApp Support Link (Optional)" icon={MessageCircle}><input type="text" value={form.whatsappLink} onChange={handleChange('whatsappLink')} placeholder="https://chat.whatsapp.com/..." /></InputWrapper>
              </div>
            </div>
              ...
              >
                {isSubmitting ? (
                  <>
                    ...
                    <span className="tracking-tight text-white/90 font-bold uppercase tracking-widest text-xs">Initializing...</span>
                  </>
                ) : (
                  <>
                    <span className="tracking-tight text-lg">Complete Setup</span>
