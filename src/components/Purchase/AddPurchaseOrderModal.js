...
const AddPurchaseOrderModal = ({ isOpen, onClose, onSave }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-2 sm:p-4">
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-7xl max-h-[95vh] overflow-y-auto border dark:border-slate-700">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
                    ...
                    <tr key={index} className="hover:bg-gray-50 dark:hover:bg-slate-700/30 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        {entry.trackExpiry ? (
                          ...
                          <span className="text-gray-400 dark:text-slate-500 text-xs">No Expiry</span>
                        )}
                      </td>
                      <td className="px-6 py-4">
                        {entry.trackExpiry ? (
                      ...
                      <td className="px-6 py-4">
                        <div className={`font-bold text-sm ${(Number(entry.sellingUnitPrice) || 0) - (Number(entry.costPrice) || 0) >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
          ...
          <div className="bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-slate-700/50 dark:to-indigo-900/20 border border-blue-200 dark:border-slate-700 rounded-xl p-6 shadow-inner">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
              <div className="bg-white/50 dark:bg-slate-800/40 p-3 rounded-lg border border-blue-100 dark:border-slate-700">
                <p className="text-xs font-bold text-blue-700 dark:text-slate-400 uppercase tracking-wider mb-1">Total Quantity</p>
                <p className="text-xl font-black text-blue-900 dark:text-white">{totalQuantity.toFixed(2)}</p>
              </div>
              <div className="bg-white/50 dark:bg-slate-800/40 p-3 rounded-lg border border-blue-100 dark:border-slate-700">
                <p className="text-xs font-bold text-blue-700 dark:text-slate-400 uppercase tracking-wider mb-1">Total Cost</p>
                <p className="text-xl font-black text-red-600 dark:text-red-400">₹{totalCostValue.toFixed(2)}</p>
              </div>
              <div className="bg-white/50 dark:bg-slate-800/40 p-3 rounded-lg border border-blue-100 dark:border-slate-700">
                <p className="text-xs font-bold text-blue-700 dark:text-slate-400 uppercase tracking-wider mb-1">Total Selling</p>
                <p className="text-xl font-black text-green-600 dark:text-green-400">₹{totalSellingValue.toFixed(2)}</p>
              </div>
              <div className="bg-white/50 dark:bg-slate-800/40 p-3 rounded-lg border border-blue-100 dark:border-slate-700">
                <p className="text-xs font-bold text-blue-700 dark:text-slate-400 uppercase tracking-wider mb-1">Exp. Profit</p>
                <p className={`text-xl font-black ${totalProfit >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                  ₹{totalProfit.toFixed(2)}
                </p>
              </div>
              <div className="bg-white/50 dark:bg-slate-800/40 p-3 rounded-lg border border-blue-100 dark:border-slate-700">
                <p className="text-xs font-bold text-blue-700 dark:text-slate-400 uppercase tracking-wider mb-1">Active Batch</p>
                <p className="text-xl font-black text-blue-900 dark:text-white">{batchEntries.filter(e => e.productName && e.quantity).length}</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex gap-4 pt-4 border-t border-gray-200 dark:border-slate-700">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 px-6 py-3 border border-gray-300 dark:border-slate-600 rounded-xl text-gray-700 dark:text-slate-300 hover:bg-gray-50 dark:hover:bg-slate-700 transition-all font-bold active:scale-95"
            >
              Cancel
            </button>
            <button
              type="submit"
              disabled={loading}
              className="flex-1 px-6 py-3 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 font-bold active:scale-95 shadow-lg shadow-indigo-500/20"
            >
              {loading ? (
                <>
