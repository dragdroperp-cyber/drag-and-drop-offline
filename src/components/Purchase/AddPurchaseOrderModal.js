...
const AddPurchaseOrderModal = ({ isOpen, onClose, onSave }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center sm:p-4">
      <div className="bg-white dark:bg-slate-800 w-full h-[95%] sm:h-auto sm:max-w-7xl rounded-none sm:rounded-2xl shadow-2xl overflow-hidden flex flex-col border dark:border-slate-700">
                ...
                <tbody className="bg-white dark:bg-slate-800 divide-y divide-gray-200 dark:divide-slate-700">
                  {batchEntries.map((entry, index) => (
                    <tr key={index} className="hover:bg-gray-50 dark:hover:bg-slate-700/30 transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <div className={`font-bold text-sm break-all ${(Number(entry.sellingUnitPrice) || 0) - (Number(entry.costPrice) || 0) >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                          {formatCurrency(((Number(entry.quantity) || 0) * ((Number(entry.sellingUnitPrice) || 0) - (Number(entry.costPrice) || 0))))}
                        </div>
                      </td>
              ...
              {batchEntries.map((entry, index) => (
                <div key={index} className="p-4 space-y-4 bg-white dark:bg-slate-800">
                  ...
                  <div className="flex justify-between items-center py-2 px-3 bg-gray-50 dark:bg-slate-700/50 rounded-lg border border-gray-100 dark:border-slate-700">
                    ...
                    <span className={`font-black text-sm break-all ${(Number(entry.sellingUnitPrice) || 0) - (Number(entry.costPrice) || 0) >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                      {formatCurrency(((Number(entry.quantity) || 0) * ((Number(entry.sellingUnitPrice) || 0) - (Number(entry.costPrice) || 0))))}
                    </span>
                  </div>
                </div>
