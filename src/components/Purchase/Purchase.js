    ...
    <div className="space-y-6 fade-in-up">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="relative bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 transition-all hover:shadow-md">
          ...
          <div className="mt-2">
            <p className="text-sm font-medium text-slate-500 dark:text-slate-400 mb-1 tracking-wide uppercase">Fulfilled</p>
        ...
        <div className="relative bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 transition-all hover:shadow-md">
          ...
          <div className="mt-2">
            <p className="text-sm font-medium text-slate-500 dark:text-slate-400 mb-1 tracking-wide uppercase">Pending</p>
        ...
        <div className="relative bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 transition-all hover:shadow-md">
          ...
          <div className="mt-2">
            <p className="text-sm font-medium text-slate-500 dark:text-slate-400 mb-1 tracking-wide uppercase">Completed</p>
        ...
        <div className="relative bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 transition-all hover:shadow-md">
          ...
          <div className="mt-2">
            <p className="text-sm font-medium text-slate-500 dark:text-slate-400 mb-1 tracking-wide uppercase">Total Value</p>
                  ...
                  <td className="px-4 py-4 align-top">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        <p className="font-medium text-slate-900 dark:text-white break-all" title={order.supplierName || 'Unknown Supplier'}>{order.supplierName || 'Unknown Supplier'}</p>
                        <p className="text-xs text-slate-500 dark:text-slate-400 whitespace-nowrap overflow-x-auto scrollbar-hide" title={`PO Value • ${formatCurrency(order.totalValue || order.total || 0)}`}>
                          PO Value • {formatCurrencySmart(order.totalValue || order.total || 0, state.currencyFormat)}
                        </p>
                      </div>
                    </div>
                  </td>
            ...
            <div className="mb-6 p-4 rounded-xl bg-gray-50 dark:bg-slate-700/50">
              ...
              {deleteConfirm.orderInfo?.total > 0 && (
                <p className="text-xs mt-2 text-gray-500 dark:text-slate-400">
                  Order Total: <span className="font-semibold text-gray-900 dark:text-white break-all" title={formatCurrency(deleteConfirm.orderInfo.total)}>{formatCurrencySmart(deleteConfirm.orderInfo.total, state.currencyFormat)}</span>
                </p>
              )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div className="bg-gray-50 dark:bg-slate-700/30 p-4 rounded-xl border dark:border-slate-700">
                  ...
                  <div className="space-y-3 text-sm">
                    ...
                    <p className="flex justify-between border-b dark:border-slate-700 pb-2"><span className="text-gray-500 dark:text-slate-400">Total Value:</span> <span className="font-bold text-indigo-600 dark:text-indigo-400" title={formatCurrency(viewOrderDetails.totalValue || viewOrderDetails.total || 0)}>{formatCurrencySmart(viewOrderDetails.totalValue || viewOrderDetails.total || 0, state.currencyFormat)}</span></p>
                          ...
                          {isBatchFormat ? (
                            ...
                            <>
                              <div className="grid grid-cols-2 gap-4 text-sm">
                                <div className="text-gray-500 dark:text-slate-400">Cost Price: <span className="font-semibold text-gray-900 dark:text-white" title={formatCurrency(item.costPrice || 0)}>{formatCurrencySmart(item.costPrice || 0, state.currencyFormat)}</span></div>
                                <div className="text-gray-500 dark:text-slate-400 text-right">Selling Price: <span className="font-semibold text-gray-900 dark:text-white" title={formatCurrency(item.sellingUnitPrice || item.sellingPrice || 0)}>{formatCurrencySmart(item.sellingUnitPrice || item.sellingPrice || 0, state.currencyFormat)}</span></div>
                              </div>
                              {(item.mfg || item.expiry || item.manufactureDate || item.expiryDate) && (
                                <div className="grid grid-cols-2 gap-4 text-xs text-gray-500 dark:text-slate-500 mt-2 pt-2 border-t dark:border-slate-700">
                                  <div>MFG: {item.mfg || item.manufactureDate ? formatDate(item.mfg || item.manufactureDate) : 'N/A'}</div>
                                  <div className="text-right">Expiry: {item.expiry || item.expiryDate ? formatDate(item.expiry || item.expiryDate) : 'N/A'}</div>
                                </div>
                              )}
                              <div className="mt-3 text-sm font-bold text-green-600 dark:text-green-400 flex justify-end">
                                Estimated Profit: <span title={formatCurrency((quantity) * ((item.sellingUnitPrice || item.sellingPrice || 0) - (item.costPrice || 0)))}>{formatCurrencySmart((quantity) * ((item.sellingUnitPrice || item.sellingPrice || 0) - (item.costPrice || 0)), state.currencyFormat)}</span>
                              </div>
                            </>
                          ) : (
                            ...
                            <>
                              <div className="grid grid-cols-2 gap-4 text-sm">
                                <div className="text-gray-500 dark:text-slate-400">Unit Price: <span className="font-semibold text-gray-900 dark:text-white" title={formatCurrency(item.price || 0)}>{formatCurrencySmart(item.price || 0, state.currencyFormat)}</span></div>
                                <div className="text-gray-500 dark:text-slate-400 text-right">Subtotal: <span className="font-bold text-indigo-600 dark:text-indigo-400" title={formatCurrency(item.subtotal || (item.price * quantity) || 0)}>{formatCurrencySmart(item.subtotal || (item.price * quantity) || 0, state.currencyFormat)}</span></div>
                              </div>
