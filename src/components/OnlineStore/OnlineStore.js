...
const StoreDashboard = ({ storeData, stats, onOrderClick }) => (
    <div className="space-y-6">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-0 md:gap-6 divide-y md:divide-y-0 divide-slate-100 dark:divide-slate-700">
            ...
            <StatsCard title="Online Revenue" value={`₹${stats?.totalRevenue || 0}`} change="+0%" icon={BarChart3} color="purple" />
        </div>
...
const StoreOrders = ({ initialOrder, onClearInitial }) => {
    ...
    const generateA4Bill = async (billData) => {
        ...
        // Add Watermark
        const hasWatermark = await addWatermarkToPDF(pdf);
    ...
    const handleShareInvoice = (order) => {
        ...
        const billUrl = `${window.location.origin}/view-bill/${order._id}`;
                        ...
                        {filteredOrders.length === 0 ? (
                            ...
                            filteredOrders.map((order) => (
                                ...
                                >
                                    ...
                                    <td className="p-3 md:p-4 text-slate-800 dark:text-slate-200 font-semibold text-xs md:text-sm">₹{order.totalAmount}</td>
                        ...
                        {filteredOrders.map((order) => (
                            ...
                            >
                                ...
                                <div className="flex justify-between items-end">
                                    ...
                                    <div className="text-right">
                                        <p className="text-slate-900 dark:text-white font-bold text-lg mb-1">₹{order.totalAmount}</p>
                                    ...
                                    <div className="bg-slate-50 dark:bg-slate-900/50 rounded-2xl p-5 border border-slate-100 dark:border-slate-700">
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            ...
                                            <div className="md:text-right flex flex-col md:items-end justify-center">
                                                ...
                                                <p className="font-black text-3xl text-slate-900 dark:text-white">₹{latestOrder.totalAmount}</p>
                                    ...
                                    <div>
                                        ...
                                        <div className="mt-4 px-4 py-3 bg-slate-50 dark:bg-slate-900/30 rounded-xl border border-slate-100 dark:border-slate-700 space-y-2">
                                            <div className="flex justify-between text-xs md:text-sm">
                                                ...
                                                <span className="text-slate-700 dark:text-slate-300 font-bold">₹{latestOrder.derivedSubtotal}</span>
                                            </div>
                                            <div className="flex justify-between text-xs md:text-sm">
                                                ...
                                                <span className={`font-bold ${latestOrder.derivedDeliveryCharge === 0 ? 'text-green-600' : 'text-slate-700 dark:text-slate-300'}`}>
                                                    {latestOrder.derivedDeliveryCharge === 0 ? 'FREE' : `₹${latestOrder.derivedDeliveryCharge}`}
                                                </span>
                                            </div>
                                            <div className="flex justify-between pt-2 border-t border-slate-200 dark:border-slate-700">
                                                <span className="text-slate-900 dark:text-white font-bold">Total Bill</span>
                                                <span className="text-indigo-600 dark:text-indigo-400 font-black text-base">₹{latestOrder.totalAmount}</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
