...
const BarcodePrintModal = ({ isOpen, onClose }) => {
    ...
    const updateQuantity = (pid, val) => {
        // Store empty string to allow clearing the input
        // Otherwise store the number (including 0)
                                        ...
                                        >
                                            {p.imageUrl || (p.images && p.images.length > 0) ? (
                                                <img
                                                    ...
                                                    alt={p.name}
                                                    className="h-12 w-12 rounded-xl object-cover border border-gray-200 dark:border-slate-700 shadow-sm"
                                                    onError={(e) => {
                                            ...
                                            <div className={`p-3 rounded-xl shrink-0 transition-colors ${isSelected
                                                ...
                                                } ${(p.imageUrl || (p.images && p.images.length > 0)) ? 'hidden' : 'flex'}`}>
                                            ...
                                            {isSelected ? (
                                                <div className="shrink-0" onClick={e => e.stopPropagation()}>
                                                    <input
                                                        ...
                                                        className="w-16 px-2 py-1 text-center font-bold text-sm bg-white dark:bg-slate-800 border-2 border-indigo-500 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500/50"
                                                    />
                                                </div>
                                            ) : (
                                                <div className={`flex items-center justify-center h-6 w-6 rounded-full border-2 transition-all shrink-0 border-gray-200 dark:border-slate-800 bg-transparent`}>
                                                </div>
                                            )}
                                        </div>
                                    );
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 preview-barcode-grid" id="barcode-print-grid">
                            {selectedProductIds.map(pid => {
                                ...
                                return Array.from({ length: qty }).map((_, i) => (
                                    <div key={`${pid}-${i}`} className="barcode-label">
