...
const BulkAddProductsModal = ({
  ...
  const handleSubmit = async (e) => {
    ...
    if (isPlanExpired(state)) {
      setLimitError('Your plan has expired. Please upgrade your plan to add products.');
      return;
    }
    // Validate products
  ...
  return (
    <div className="fixed inset-0 bg-gray-900/60 backdrop-blur-sm flex items-end sm:items-center justify-center z-[1050] p-0 sm:p-4">
      <div
        ref={containerRef}
        className="bg-white dark:bg-slate-800 rounded-none sm:rounded-2xl shadow-2xl w-full max-w-7xl h-full sm:h-[90vh] flex flex-col overflow-hidden transition-colors"
        role="dialog"
        aria-modal="true"
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200 dark:border-slate-700 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-slate-700 dark:to-slate-800 flex-shrink-0">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              {getTranslation('saveAndClose', state.currentLanguage)}
            </button>
            <button
              onClick={() => {
                localStorage.removeItem('bulkAddProducts_saved');
        ...
        <div className="px-6 py-3 bg-gray-50 dark:bg-slate-700/50 border-b border-gray-200 dark:border-slate-700 flex-shrink-0">
          <div className="flex items-center justify-between text-sm">
            <span className="text-gray-600 dark:text-slate-400">
              ...
              {!canAdd && ` (${getTranslation('limitReached', state.currentLanguage)})`}
            </span>
                ...
                >
                  ...
                  <div className="flex items-center justify-between mb-4 pb-2 border-b border-gray-100 dark:border-slate-700">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-medium text-gray-900 dark:text-white">{getTranslation('product', state.currentLanguage)} {index + 1}</span>
                      {product.name && product.name.trim() && (
                        <span className="text-xs bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300 px-2 py-1 rounded-full">
                          ✓ {getTranslation('ready', state.currentLanguage)}
                        </span>
                      )}
                    </div>
                    {products.length > 1 && (
              ...
              >
                {saving ? (
                  ...
                  <>
                    ...
                    {getTranslation('saveProducts', state.currentLanguage).replace('{count}', products.filter(p => p.name && p.name.trim()).length)}
                  </>
                )}
              </button>
            </div>
