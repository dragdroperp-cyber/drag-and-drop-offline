...
const EditProductModal = ({ product, onClose, onSave }) => {
  ...
  const handleSubmit = (e) => {
    ...
    // Validate dates
    if (formData.trackExpiry && formData.mfgDate && formData.expiryDate) {
    ...
    console.log('🔄 Calling onSave with productData...');
    onSave(productData);
        ...
        if (timeSinceLastKey < 50 || scannerInputBufferRef.current.length === 0) {
          ...
          scannerInputTimerRef.current = setTimeout(() => {
            ...
            if (scannedCode.length > 0) {
              console.log('Barcode scanned on Edit Product modal:', scannedCode);
  ...
  if (!product) {
    console.error('EditProductModal: No product data provided');
    return null;
  }
      ...
      >
        <div className="flex items-center justify-between p-4 sm:p-6 border-b border-gray-200 sticky top-0 bg-white rounded-t-xl">
          <div className="flex items-center">
            ...
            <h2 id="edit-product-title" className="text-lg sm:text-xl font-semibold text-gray-900">Edit Product</h2>
          </div>
          <button
            onClick={onClose}
            data-modal-close
