...
import { X, Package, Plus, AlertTriangle, QrCode, Minimize2, Save } from 'lucide-react';
...
const AddProductModal = ({
  ...
  const handleSubmit = async (e) => {
    ...
    const canAdd = await canAddData(totalProducts, 'products', state.aggregatedUsage);
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-end sm:items-center justify-center z-50 p-0 sm:p-4 transition-opacity">
      <div
        ref={containerRef}
        className="bg-white dark:bg-slate-800 rounded-none sm:rounded-xl shadow-xl w-full max-w-2xl h-full sm:h-auto sm:max-h-[95vh] overflow-y-auto transition-all"
        role="dialog"
        aria-modal="true"
      ...
      >
        <div className="flex items-center justify-between p-4 sm:p-6 border-b border-gray-200 dark:border-slate-700 sticky top-0 bg-white dark:bg-slate-800 rounded-t-xl z-10 transition-colors">
          <div className="flex items-center">
            ...
            <h2 id="add-product-title" className="text-lg sm:text-xl font-semibold text-gray-900 dark:text-white">Add New Product</h2>
          </div>
          <div className="flex items-center gap-2">
            <button
              onClick={handleMinimize}
              className="px-3 py-1.5 text-xs bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 hover:bg-blue-200 dark:hover:bg-blue-900/50 rounded-lg transition-colors flex items-center gap-1"
        ...
        <form onSubmit={handleSubmit} className="p-4 sm:p-6 space-y-4 sm:space-y-6">
          ...
          {(() => {
            ...
            const canAdd = totalProducts < maxProducts;
              ...
              <div className={`rounded-lg border p-3 ${canAdd ? 'border-blue-200 bg-blue-50 dark:border-blue-800 dark:bg-blue-900/20' : 'border-red-200 bg-red-50 dark:border-red-800 dark:bg-red-900/20'}`}>
                <div className="flex items-center justify-between text-xs">
                  ...
                  <span className={`font-semibold ${canAdd ? 'text-blue-900 dark:text-blue-100' : 'text-red-900 dark:text-red-100'}`}>
                    {totalProducts} / {maxProducts === Infinity ? '∞' : maxProducts}
                    {!canAdd && ' (Full)'}
                  </span>
                </div>
