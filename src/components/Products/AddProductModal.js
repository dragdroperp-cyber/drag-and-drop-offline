...
import { X, Package, Plus, AlertTriangle, QrCode, Minimize2, Save, Minus, ScanLine, RefreshCw } from 'lucide-react';
...
const AddProductModal = ({
  ...
  useEffect(() => {
    const handleScannerInput = (e) => {
      // Move hooks to top level, removed from inside useEffect
      // handleScannerInput logic remains...
  ...
  return (
    ...
    >
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="p-4 sm:p-6 space-y-4 sm:space-y-6">
          ...
          <div className="flex flex-col sm:flex-row justify-end space-y-2 sm:space-y-0 sm:space-x-3 pt-4 sm:pt-6 border-t border-gray-200 dark:border-slate-700">
            ...
            <button
              ...
              className={`btn-primary w-full sm:w-auto order-1 sm:order-2 flex items-center justify-center gap-2 ${isSubmitting ? 'opacity-70 cursor-not-allowed' : ''}`}
            >
              {isSubmitting ? (
                <>
        ...
        {/* Full Screen Scanner Modal - Billing Style */}
        {/* Full Screen Scanner Modal - Billing Style */}
        {/* Full Screen Scanner Modal - Billing Style (Rendered via Portal to escape modal stacking) */}
