  ...
  const handleBatchSubmit = async () => {
    ...
    try {
      ...
      try {
        ...
        const { isOnline, backgroundSyncWithBackend } = await import('../../utils/dataFetcher');
        if (await isOnline()) {
          backgroundSyncWithBackend(dispatch, {}).catch(console.error);
        }
      } catch (syncError) {
        ...
        filteredProducts.forEach(p => {
          if (p.batches && p.batches.length > 0) {
            ...
            itemsToRender.push({
              ...
              price: formatCurrencySmart(p.sellingPrice || p.price || 0, state.currencyFormat),
              barcode: p.barcode || '-',
                                      ...
                                      <div className="grid grid-cols-2 gap-3">
                                        ...
                                        <div>
                                          ...
                                          <input
                                            type="number"
                                            value={editingBatchData.sellingUnitPrice}
                                            onChange={(e) => handleBatchInputChange('sellingUnitPrice', e.target.value)}
                                            className="w-full px-3 py-2 border border-gray-300 dark:border-slate-600 dark:bg-slate-800 dark:text-white rounded-lg text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"
                                            placeholder="0.00"
                                          />
                                        </div>
                                      ...
                                      <div className="grid grid-cols-2 gap-3">
                                        <div>
                                          ...
                                          <input
                                            type="number"
                                            value={editingBatchData.costPrice}
                                            onChange={(e) => handleBatchInputChange('costPrice', e.target.value)}
                                            className="w-full px-3 py-2 border border-gray-300 dark:border-slate-600 dark:bg-slate-800 dark:text-white rounded-lg text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"
                                            placeholder="0.00"
                                          />
                                        </div>
