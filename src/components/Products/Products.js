...
const Products = () => {
  ...
  const [deletedProductsBackup, setDeletedProductsBackup] = useState([]);
  ...
  // Delete all products with progress tracking and transactional approach
  const handleDeleteAllProducts = async () => {
    const activeProducts = state.products.filter(p => !p.isDeleted);
    
    if (activeProducts.length === 0) {
      if (window.showToast) {
        window.showToast('No products to delete', 'info');
      }
    ...
    // Store backup for potential restoration (with full product data)
    const backup = activeProducts.map(p => ({ ...p }));
    setDeletedProductsBackup(backup);
    setIsDeletingAll(true);
    setDeleteProgress({ current: 0, total: activeProducts.length });

    const deletedSoFar = [];
    const sellerId = getSellerIdFromAuth();

    try {
      // Delete products one by one with delay to show progress
      // Mark as deleted in IndexedDB WITHOUT syncing to backend
      for (let i = 0; i < activeProducts.length; i++) {
        // Check if cancellation was requested
        if (cancelDeleteRef.current) {
          // Restore all deleted products in IndexedDB
          await restoreDeletedProductsInDB(deletedSoFar);
          // Restore in state
          restoreDeletedProducts(deletedSoFar);
          setIsDeletingAll(false);
          setDeleteProgress({ current: 0, total: 0 });
          setDeletedProductsBackup([]);
          cancelDeleteRef.current = false;
          if (window.showToast) {
            window.showToast('Deletion cancelled. All products restored.', 'success');
          }
          return;
        }
        ...
        // Mark as deleted in IndexedDB WITHOUT syncing (isSynced stays true to prevent sync)
        const deletedProduct = {
          ...product,
          isDeleted: true,
          deletedAt: new Date().toISOString(),
          isSynced: true // Keep as synced to prevent automatic sync - we'll sync manually after all are done
        };

        // Update in IndexedDB directly (skip validation, prevent sync)
        await updateItem(STORES.products, deletedProduct, true);
        
        // Update state (remove from UI) - use suppressProductSync to prevent sync trigger
        dispatch({ 
          type: 'DELETE_PRODUCT', 
          payload: product.id,
          meta: { suppressProductSync: true } // Prevent automatic sync in reducer
        });
        
        deletedSoFar.push(product);
        ...
        // Update progress
        setDeleteProgress({ current: i + 1, total: activeProducts.length });

        // Small delay to show progress (50ms per product)
        await new Promise(resolve => setTimeout(resolve, 50));
      }

      // All products deleted successfully in IndexedDB
      // Now sync deletions to backend in batch (transactional - all or nothing)
      const deletedProductsForSync = activeProducts.map(p => ({
        ...p,
        isDeleted: true,
        deletedAt: new Date().toISOString(),
        isSynced: false // Mark as unsynced for backend sync
      }));

      // Sync all deletions to backend at once
      try {
        const syncResult = await syncData('products', deletedProductsForSync, sellerId);
        
        if (syncResult.success) {
          // Mark all as synced after successful batch sync
          for (const product of activeProducts) {
            ...
            await updateItem(STORES.products, syncedDeleted, true);
          }
          console.log('✅ All product deletions synced to backend');
        } else {
          throw new Error(syncResult.error || 'Sync failed');
        }
      } catch (syncError) {
        console.error('❌ Error syncing deletions to backend:', syncError);
        // If sync fails, restore all products (transactional rollback)
        await restoreDeletedProductsInDB(activeProducts);
        restoreDeletedProducts(activeProducts);
        setIsDeletingAll(false);
        setDeleteProgress({ current: 0, total: 0 });
        setDeletedProductsBackup([]);
        cancelDeleteRef.current = false;
        if (window.showToast) {
          window.showToast('Failed to sync deletions to backend. All products restored.', 'error');
        }
        return;
      }
      ...
      if (window.showToast) {
        window.showToast(`Successfully deleted ${activeProducts.length} product(s)`, 'success');
      }
      ...
      cancelDeleteRef.current = false;
      if (window.showToast) {
        window.showToast('Error deleting products. All products restored.', 'error');
      }
    }
  };

  // Restore deleted products in IndexedDB and sync to backend if needed
  const restoreDeletedProductsInDB = async (productsToRestore) => {
    const products = productsToRestore || deletedProductsBackup;
    ...
    for (const product of products) {
      try {
        // Restore product in IndexedDB by removing deletion flag
        const restoredProduct = {
          ...product,
          isDeleted: false,
          deletedAt: undefined,
          isSynced: product._id ? false : false // Mark as unsynced if it had _id (needs backend sync)
        };
        await updateItem(STORES.products, restoredProduct, true);
        
        // If product had _id, it existed in backend, so we need to sync restoration
        if (product._id) {
          productsToSync.push(restoredProduct);
        }
      } catch (error) {
        console.error(`Error restoring product ${product.id} in IndexedDB:`, error);
      }
    }

    // Sync restored products to backend if any had _id
    if (productsToSync.length > 0) {
      try {
        await syncData('products', productsToSync, sellerId);
        console.log(`✅ Restored ${productsToSync.length} product(s) in backend`);
        ...
        for (const product of productsToSync) {
          ...
          await updateItem(STORES.products, syncedProduct, true);
        }
      } catch (syncError) {
        console.error('❌ Error syncing restored products to backend:', syncError);
        // Products are restored in IndexedDB, but may need to sync later
      }
    }
  };

  // Restore deleted products in state
  const restoreDeletedProducts = (productsToRestore) => {
    const products = productsToRestore || deletedProductsBackup;
    if (products.length === 0) return;

    products.forEach(product => {
      // Restore product by dispatching ADD_PRODUCT with original data
      dispatch({ 
        type: 'ADD_PRODUCT', 
        payload: {
          ...product,
        ...
        meta: { suppressProductSync: true } // Prevent sync during restoration
      });
    });
  };

  // Cancel deletion and restore products
  const handleCancelDeleteAll = () => {
    if (isDeletingAll) {
      // Set cancellation flag
      cancelDeleteRef.current = true;
      // The restore will happen in the delete loop
    } else {
      // If not deleting, just close modal
      setShowDeleteAllModal(false);
    }
  };
      ...
      <div className="flex flex-col space-y-3 sm:flex-row sm:items-center sm:justify-between sm:space-y-0">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <button
            ...
            title={activeProducts.length === 0 ? 'No products to delete' : 'Delete all products'}
          >
              ...
              {!isDeletingAll ? (
                <>
                  ...
                  <div className="mb-6">
                    <p className="text-sm text-gray-700 mb-2">
                      Are you sure you want to delete <span className="font-semibold text-red-600">{activeProducts.length}</span> product(s)?
                    </p>
                    <p className="text-xs text-gray-500">
                      All product data including stock, prices, and details will be permanently deleted.
                    </p>
                  </div>
                ...
                <>
                  <div className="flex items-center gap-4 mb-4">
                    ...
                    <div>
                      <h3 className="text-lg font-semibold text-gray-900">Deleting Products...</h3>
                  ...
                  <div className="mb-4">
                    ...
                    <div className="w-full bg-gray-200 rounded-full h-3">
                      <div
                        className="bg-red-500 h-3 rounded-full transition-all duration-300 flex items-center justify-end pr-1"
                        style={{
                  ...
                  <div className="mb-6 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                    <div className="flex items-start gap-2">
                      ...
                      <div className="text-xs text-yellow-800">
                        ...
                        <p className="mb-1">
                          Do <span className="font-bold">NOT</span> refresh the page during deletion.
                        </p>
                        <p>
                          If you refresh, your data may become inconsistent and we are <span className="font-semibold">not responsible</span> for any data loss or corruption.
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-3">
                    ...
                    <button
                      disabled
                      className="flex-1 px-4 py-2.5 bg-red-500 text-white rounded-lg font-medium opacity-50 cursor-not-allowed flex items-center justify-center gap-2"
                    >
