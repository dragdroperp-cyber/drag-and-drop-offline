...
const Products = () => {
  ...
  const handleBatchSubmit = async () => {
    try {
      ...
      // STEP 5: Add to sync queue for background sync
      const { addToSyncQueue } = await import('../../utils/dataFetcher');
      await addToSyncQueue('batch_create_manual', {
        batchId: newBatch.id,
        productId: productId,
        batchData: newBatch,
        timestamp: new Date().toISOString()
      ...
      // STEP 6: Attempt background sync if online
      const { isOnline, backgroundSyncWithBackend } = await import('../../utils/dataFetcher');
      if (isOnline()) {
        backgroundSyncWithBackend(dispatch, {}).catch(syncError => {
          //('⚠️ Background sync failed, but batch is saved locally');
        });
      }
  ...
  const exportProductsPDF = () => {
    try {
      ...
      pdf.text(`Total Value: Rs. ${totalValueMetric.toFixed(2)}`, 180, 40);

      const headers = ['Name', 'Category', 'Stock', 'Cost', 'Price', 'Barcode', 'Expiry'];
      const colWidths = [65, 40, 30, 30, 30, 40, 35];
      ...
      const drawTableHeader = (yPos) => {
        ...
        headers.forEach((header, idx) => {
          const textX = colPositions[idx] + colWidths[idx] / 2;
          pdf.text(header, textX, yPos - 3, { align: 'center' });
        });
        pdf.setDrawColor(210);
        headers.forEach((_, idx) => {
      ...
      filteredProducts.forEach((product, index) => {
        ...
        cellLines.forEach((lines, idx) => {
          // Vertical centering:
          // Row height includes padding. Text block height is lines.length * lineHeight.
          // Top of text block should be: y - lineHeight + 1 + (rowHeight - textHeight)/2 + fontCorrection (approx)
          // Actually, pdf.text position is baseline.
          // Let's use simple logic: boxTop + (boxHeight - textHeight) / 2 + textAscent
          // boxTop is y - lineHeight + 1.
          // simpler: center of row is y - lineHeight + 1 + rowHeight/2.
          // text block center is lines.length * lineHeight / 2.
          // So baseline for first line... this is getting complicated with jsPDF.
          // Let's try: startY = y + (rowHeight - lines.length * lineHeight) / 2 - lineHeight;
          // Wait, 'y' in the loop is the BOTTOM of the row in previous logic?
          // No, y is incremented by rowHeight at the end.
          // In previous code: pdf.rect(leftMargin, y - lineHeight + 1, tableWidth, rowHeight, 'F');
          // So 'y - lineHeight + 1' is the TOP of the row.

          const rowTop = y - lineHeight + 1;
          const textHeight = lines.length * lineHeight;
          const startY = rowTop + (rowHeight - textHeight) / 2 + lineHeight / 2 + 1; // +1 for visual adjustment

          lines.forEach((line, lineIdx) => {
            const offsetY = startY + lineIdx * lineHeight;
            const textX = colPositions[idx] + colWidths[idx] / 2;
            pdf.text(line, textX, offsetY, { align: 'center' });
          });
        });
        ...
        <div className="flex flex-col space-y-3 sm:flex-row sm:items-center sm:justify-between sm:space-y-0">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <div className="relative" ref={exportMenuRef}>
              ...
              {showExportMenu && (
                <div className="absolute left-0 sm:left-auto sm:right-0 mt-2 w-48 max-w-[calc(100vw-2rem)] bg-white dark:bg-slate-800 rounded-xl shadow-lg border border-gray-200 dark:border-slate-700 z-50 py-2">
                  <button
                    onClick={exportProductsPDF}
                    className="w-full px-4 py-2.5 text-left text-sm text-gray-700 dark:text-slate-300 hover:bg-gray-50 dark:hover:bg-slate-700 active:bg-gray-100 dark:active:bg-slate-600 transition-colors flex items-center gap-3"
                  >
                  ...
                  <button
                    ...
                    className="w-full px-4 py-2.5 text-left text-sm text-gray-700 dark:text-slate-300 hover:bg-gray-50 dark:hover:bg-slate-700 active:bg-gray-100 dark:active:bg-slate-600 transition-colors flex items-center gap-3"
                  >
                    <FileJson className="h-4 w-4 text-yellow-600 dark:text-yellow-400 flex-shrink-0" />
              ...
              <table className="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
                <thead className="bg-gray-50 dark:bg-slate-700/50">
                  <tr>
                    ...
                    <th className="px-4 sm:px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-slate-400 uppercase tracking-wider">Category</th>
                    ...
                    return (
                      <tr key={product.id} className="hover:bg-gray-50 dark:hover:bg-slate-700/50">
                        ...
                        <td className="px-4 sm:px-6 py-4 whitespace-nowrap">
                          <span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-300 max-w-[150px] truncate" title={product.category || 'Uncategorized'}>
                  ...
                  <div className="flex items-start space-x-3 flex-1 min-w-0">
                    ...
                    <div className="flex-1 min-w-0 overflow-hidden">
                      ...
                      <div className="flex flex-wrap gap-2 mt-2">
                        <span className="px-2 py-1 text-xs font-semibold rounded-full bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-300 truncate max-w-[150px]" title={product.category || 'Uncategorized'}>
                          {product.category || 'Uncategorized'}
                        </span>
