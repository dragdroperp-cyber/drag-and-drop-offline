...
const Products = () => {
  ...
  const productLimitLabel = maxProducts === Infinity ? getTranslation('unlimited', state.currentLanguage) : maxProducts;
  ...
  const handleBatchSubmit = async () => {
    ...
    try {
      ...
      try {
        const { addToSyncQueue } = await import('../../utils/dataFetcher');
        await addToSyncQueue('batch_create', {
          batchId: newBatch.id,
          productId: productId,
          batchData: newBatch,
          timestamp: new Date().toISOString()
        ...
        const { isOnline, backgroundSyncWithBackend } = await import('../../utils/dataFetcher');
        if (await isOnline()) {
          backgroundSyncWithBackend(dispatch, ActionTypes, { skipDataFetch: true }).catch(console.error);
        }
      } catch (syncError) {
        console.error('Error queuing batch sync:', syncError);
      }
      ...
      if (localSaveSuccess) {
        ...
        try {
          ...
          if (isOnline()) {
            ...
            backgroundSyncWithBackend(dispatch, ActionTypes, { skipDataFetch: true }).catch(syncError => {
  ...
  const exportProductsPDF = async (withBatches = false) => {
    try {
      ...
      pdf.text(`${getTranslation('productReport', state.currentLanguage)} (${withBatches ? getTranslation('detailedExport', state.currentLanguage) : getTranslation('summaryExport', state.currentLanguage)})`, pageWidth - margin, 14, { align: 'right' });
      ...
      pdf.setFont('helvetica', 'bold');
      pdf.setTextColor(...COLORS.primary);
      pdf.text(`${getTranslation('productInventory', state.currentLanguage)} - ${withBatches ? getTranslation('detailedExport', state.currentLanguage) : getTranslation('summaryExport', state.currentLanguage)}`, margin, y);
          ...
          <div>
            ...
            <p className="text-sm text-gray-600 dark:text-slate-400 mt-1">
              ...
              <span className="ml-2 text-blue-600 dark:text-blue-400 font-medium">
                ({getTranslation('productLimitLeft', state.currentLanguage)}: {productLimitLabel === getTranslation('unlimited', state.currentLanguage) ? getTranslation('unlimited', state.currentLanguage) : Math.max(0, maxProducts - totalProducts)})
              </span>
            </p>
          </div>
                ...
                >
                    ...
                    <div className="p-2 rounded-xl bg-gray-50 dark:bg-slate-700/30 border border-gray-100 dark:border-slate-700/50">
                      ...
                      <div className="flex flex-col">
                        <span className="text-sm font-bold text-gray-900 dark:text-white">
                          {formatCurrencySmart(product.sellingUnitPrice || product.sellingPrice || 0, state.currencyFormat)}
                        </span>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="h-10 w-10 flex items-center justify-center rounded-xl bg-gray-50 dark:bg-slate-700 text-gray-600 dark:text-slate-300 hover:bg-gray-100 dark:hover:bg-slate-600 transition-colors border border-gray-200 dark:border-slate-600"
                    >
                      <Edit className="h-4 w-4" />
                    </button>
                    ...
                    <button
                      ...
                      className="h-10 w-10 flex items-center justify-center rounded-xl bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 hover:bg-red-100 dark:hover:bg-red-900/30 transition-colors border border-red-100 dark:border-red-900/30"
                    >
                      <Trash2 className="h-4 w-4" />
                    </button>
                  </div>
