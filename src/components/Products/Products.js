    ...
    try {
      ...
      const addWatermark = (pdfDoc) => {
        ...
        for (let i = 1; i <= totalPages; i++) {
          ...
          pdfDoc.text('INVENTORY REPORT', pageWidth / 2, pageHeight / 2, {
            align: 'center',
            angle: 45
          });
      ...
      for (let i = 1; i <= totalPages; i++) {
        ...
        doc.text(`${state.storeName || 'Store'} - Inventory Report`, pageWidth - margin, pageHeight - 10, { align: 'right' });
      }

      addWatermark(doc);

      doc.save(`products-report-${formatDate(new Date()).replace(/\//g, '-')}.pdf`);
      if (window.showToast) {
