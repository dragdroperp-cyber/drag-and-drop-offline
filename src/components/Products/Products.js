...
const Products = () => {
  ...
  const expiredProducts = state.products.filter(product => {
    if (!product.expiryDate) return false;
    const expiryDate = new Date(product.expiryDate);
    const today = new Date();
  ...
  const handleBatchSubmit = async () => {
    ...
    try {
      ...
      const { quantity, costPrice, sellingUnitPrice, mfg, expiry, batchNumber } = newBatchData;
      ...
      if (qtyVal <= 0 || costVal < 0 || sellVal < 0) {
        if (window.showToast) {
  ...
  const handleConfirmBatchEdit = async () => {
    try {
      const updateData = {
        ...
        quantity: Number(editingBatchData.quantity.toString().replace(/,/g, '')),
        costPrice: Number(editingBatchData.costPrice.toString().replace(/,/g, '')),
        sellingUnitPrice: Number(editingBatchData.sellingUnitPrice.toString().replace(/,/g, '')),
  ...
  const exportProductsPDF = async (withBatches = false) => {
    try {
      ...
      const safeDrawText = (doc, text, x, y, options = {}) => {
        const isHindi = /[\u0900-\u097F\u20B9]/.test(text); // Added Rupee symbol
        if (isHindi) {
          ...
          const w = metrics.width / 3.78; // Convert px to mm (approx)
      ...
      safeDrawText(pdf, 'Drag & Drop', logoX + 22, 15, { color: '#2F3C7E', fontSize: 16 });

      pdf.setFontSize(9);
      pdf.setFont('helvetica', 'normal');
      pdf.setTextColor(...COLORS.gray);
      safeDrawText(pdf, getTranslation('inventoryManagement', state.currentLanguage), logoX + 22, 19, { color: '#787878', fontSize: 9 });

      /* -------- RIGHT META -------- */
      pdf.setFontSize(12);
      pdf.setFont('helvetica', 'bold');
      pdf.setTextColor(...COLORS.black);
      safeDrawText(pdf, `${getTranslation('productReport', state.currentLanguage)} (${withBatches ? getTranslation('detailedExport', state.currentLanguage) : getTranslation('summaryExport', state.currentLanguage)})`, pageWidth - margin, 14, { align: 'right', color: '#000000', fontSize: 12 });
      ...
      safeDrawText(pdf,
        ...
        pageWidth - margin,
        19,
        { align: 'right', color: '#787878', fontSize: 9 }
      );
      ...
      const metrics = [
        { label: getTranslation('totalProducts', state.currentLanguage), value: filteredProducts.length },
        {
        ...
        { label: getTranslation('totalValue', state.currentLanguage), value: formatCurrencySmart(totalValue, state.currencyFormat) }
      ];
      ...
      metrics.forEach((m, i) => {
        ...
        pdf.setFontSize(9);
        pdf.setTextColor(...COLORS.gray);
        safeDrawText(pdf, m.label.toUpperCase(), x + 4, startY + 7, { color: '#787878', fontSize: 9 });

        pdf.setFontSize(16);
        pdf.setFont('helvetica', 'bold');
        pdf.setTextColor(...COLORS.primary);
        safeDrawText(pdf, String(m.value), x + 4, startY + 14, { color: '#2F3C7E', fontSize: 16 });
      });
      ...
      pdf.setFont('helvetica', 'bold');
      pdf.setTextColor(...COLORS.primary);
      safeDrawText(pdf, `${getTranslation('productInventory', state.currentLanguage)} - ${withBatches ? getTranslation('detailedExport', state.currentLanguage) : getTranslation('summaryExport', state.currentLanguage)}`, margin, y, { color: '#2F3C7E', fontSize: 15 });

      /* ================= TABLE ================= */
      const headers = [
        getTranslation('name', state.currentLanguage),
        getTranslation('categoryHeader', state.currentLanguage),
        getTranslation('stockHeader', state.currentLanguage),
      ...
      headers.forEach((h, i) => {
        ...
        safeDrawText(pdf, h, x + colW[i] / 2, y + 6, { align: 'center', color: '#2F3C7E', fontSize: 11 });
      });
          ...
          if (p.batches && p.batches.length > 0) {
            p.batches.forEach(b => {
              itemsToRender.push({
                ...
                cost: formatCurrencySmart(b.costPrice || p.costPrice || 0, state.currencyFormat),
                price: formatCurrencySmart(b.sellingUnitPrice || p.sellingPrice || p.price || 0, state.currencyFormat),
                barcode: p.barcode || '-',
            ...
            itemsToRender.push({
              ...
              cost: formatCurrencySmart(p.costPrice || 0, state.currencyFormat),
              price: formatCurrencySmart(getEffectivePrice(p), state.currencyFormat),
              barcode: p.barcode || '-',
      ...
      itemsToRender.forEach((item, i) => {
        ...
        if (y + rowH > pageHeight - 20) {
          ...
          headers.forEach((h, j) => {
            ...
            safeDrawText(pdf, h, x + colW[j] / 2, y + 6, { align: 'center', color: '#2F3C7E', fontSize: 11 });
          });
          y += 9;
          pdf.setFontSize(10);
          pdf.setTextColor(...COLORS.black);
        ...
        rowValues.forEach((val, j) => {
          const x = margin + colW.slice(0, j).reduce((a, b) => a + b, 0);
          safeDrawText(pdf, String(val), x + colW[j] / 2, y + 5.5, { align: 'center', color: '#000000', fontSize: 10 });
        });
      ...
      if (pageCount > 1) { // Conditional Footer
        for (let i = 1; i <= pageCount; i++) {
          pdf.setPage(i);
          pdf.setFontSize(8);
          pdf.setTextColor(...COLORS.gray);
          safeDrawText(pdf, `${getTranslation('page', state.currentLanguage)} ${i} ${getTranslation('ofPage', state.currentLanguage)} ${pageCount}`, margin, pageHeight - 10, { color: '#787878', fontSize: 8 });
          safeDrawText(pdf,
            state.currentUser?.shopName || getTranslation('storeName', state.currentLanguage),
            pageWidth - margin,
            pageHeight - 10,
            { align: 'right', color: '#787878', fontSize: 8 }
          );
        }
      }
        ...
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div className="relative bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 transition-all hover:shadow-md">
              ...
              <div className="mt-2">
                <p className="text-sm font-medium text-slate-500 dark:text-slate-400 mb-1 tracking-wide uppercase">{getTranslation('lowStock', state.currentLanguage)}</p>
            ...
            <div className="relative bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 transition-all hover:shadow-md">
              ...
              <div className="mt-2">
                <p className="text-sm font-medium text-slate-500 dark:text-slate-400 mb-1 tracking-wide uppercase">{getTranslation('expiringSoon', state.currentLanguage)}</p>
                <p className="text-2xl font-semibold text-gray-900 dark:text-white">{expiringProducts}</p>
              </div>
            </div>
            ...
            <div className="relative bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 transition-all hover:shadow-md">
              ...
              <div className="mt-2">
                <p className="text-sm font-medium text-slate-500 dark:text-slate-400 mb-1 tracking-wide uppercase">{getTranslation('expired', state.currentLanguage)}</p>
                <p className="text-2xl font-semibold text-gray-900 dark:text-white">{expiredProducts}</p>
              </div>
            </div>
                ...
                >
                  ...
                  <div className="flex gap-4">
                    ...
                    <div className="flex-shrink-0">
                      ...
                      <div
                        className={`h-16 w-16 rounded-xl bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 flex items-center justify-center border border-blue-100 dark:border-blue-800/30 ${product.imageUrl ? 'hidden' : 'flex'}`}
                      >
                    ...
                    <div className="space-y-2">
                      ...
                      {importProgress.success > 0 && (
                        <p className="text-sm text-green-600 dark:text-green-400">
                          ✓ {getTranslation('importSuccessMsg', state.currentLanguage)}: {importProgress.success}
                        </p>
                      )}
          ...
          {showAddBatchModal && (
            <div
              className={`fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 transition-opacity duration-300 ${isClosingBatchModal ? 'opacity-0' : 'animate-fadeIn'}`}
            ...
            >
              ...
              <div
                ...
                style={{ animation: `${isClosingBatchModal ? 'slideDown' : 'slideUp'} 0.4s ease-out forwards` }}
                className="bg-white dark:bg-slate-800 rounded-none sm:rounded-2xl shadow-xl max-w-2xl w-full h-auto max-h-[95vh] sm:h-auto sm:max-h-[95vh] overflow-y-auto relative"
                data-batch-modal
                onClick={e => e.stopPropagation()}
              >
                <div className="sticky top-0 bg-white dark:bg-slate-800 border-b border-gray-200 dark:border-slate-700 px-6 py-4 rounded-t-2xl flex items-center justify-between z-10">
                  <h3 className="text-xl font-semibold text-gray-900 dark:text-white">{getTranslation('addProductBatch', state.currentLanguage)}</h3>
                  <button
                    onClick={handleCloseBatchModal}
                    className="p-2 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 hover:bg-gray-100 dark:hover:bg-slate-700 rounded-lg transition-colors"
                  >
                ...
                <div className="p-6 space-y-6">
                  ...
                  {!selectedProductForBatch && (
                    <div>
                      <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">
                        {getTranslation('selectProductLabel', state.currentLanguage)}
                      </label>
                      <div className="space-y-3">
                                ...
                                <div className="flex items-center justify-between">
                                  <div>
                                    ...
                                    <p className="text-sm text-gray-600 dark:text-slate-400">
                                      ...
                                      {getTranslation('barcodeHeader', state.currentLanguage)}: {product.barcode || getTranslation('na', state.currentLanguage)}
                                    </p>
                                  </div>
                                </div>
                              </button>
                        ...
                        {batchSearchTerm.trim() && batchSearchResults.length === 0 && (
                          <div className="text-center py-4 border border-gray-200 dark:border-slate-700 rounded-lg">
                            <p className="text-gray-600 dark:text-slate-400 mb-3">{getTranslation('productNotFound', state.currentLanguage)}</p>
                            <button
                  ...
                  {selectedProductForBatch && (
                    <div>
                      <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">
                        {getTranslation('selectedProduct', state.currentLanguage)}
                      </label>
                        ...
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <p className="text-sm text-blue-700 dark:text-blue-300">
                              {getTranslation('stockHeader', state.currentLanguage)}: {(selectedProductForBatch.batches?.reduce((sum, b) => sum + (b.quantity || 0), 0) || 0) || selectedProductForBatch.quantity || selectedProductForBatch.stock || 0} {selectedProductForBatch.unit || 'pcs'} •
                              {getTranslation('barcodeHeader', state.currentLanguage)}: {selectedProductForBatch.barcode || getTranslation('na', state.currentLanguage)}
                            </p>
                            <p className="text-xs text-blue-600 dark:text-blue-400 mt-1">
                              {selectedProductForBatch.trackExpiry ? getTranslation('tracksExpiry', state.currentLanguage) : getTranslation('doesNotTrackExpiry', state.currentLanguage)}
                            </p>
                          </div>
                  ...
                  {selectedProductForBatch && (
                    <div className="border-t border-gray-200 dark:border-slate-700 pt-6">
                      <h4 className="text-lg font-medium text-gray-900 dark:text-white mb-4">{getTranslation('batchDetails', state.currentLanguage)}</h4>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                          <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">
                            {getTranslation('quantityLabel', state.currentLanguage)}
                          </label>
                          <input
                            type="text"
                            placeholder={getTranslation('enterQuantityPlaceholder', state.currentLanguage)}
                        ...
                        {selectedProductForBatch.trackExpiry && (
                          <div>
                            <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">
                              {getTranslation('mfgDateLabel', state.currentLanguage)}
                            </label>
                            <input
                              type="date"
                        ...
                        {selectedProductForBatch.trackExpiry && (
                          <div>
                            <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">
                              {getTranslation('expiryDateLabel', state.currentLanguage)}
                            </label>
                            <input
                              type="date"
                        ...
                        <div>
                          <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">
                            {getTranslation('costPriceRsLabel', state.currentLanguage)}
                          </label>
                          <input
                            type="text"
                            placeholder={getTranslation('enterCostPricePlaceholder', state.currentLanguage)}
                        ...
                        <div>
                          <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">
                            {getTranslation('sellingPriceRsLabel', state.currentLanguage)}
                          </label>
                          <input
                            type="text"
                            placeholder={getTranslation('enterSellingPricePlaceholder', state.currentLanguage)}
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white">
                                      {isEditing ? (
                                        <input
                                          ...
                                          disabled
                                          className="w-full px-2 py-1 border border-gray-300 dark:border-slate-600 bg-gray-100 dark:bg-slate-600/50 text-gray-500 dark:text-slate-400 rounded text-sm focus:outline-none cursor-not-allowed"
                                      ...
                                      {isEditing ? (
                                        <div className="flex gap-2">
                                          <button
                                            ...
                                            title="Confirm Edit"
                                          >
                                            ✓ {getTranslation('confirm', state.currentLanguage)}
                                          </button>
                                          <button
                            ...
                            return (
                              <div key={batch.id || index} className="bg-gray-50 dark:bg-slate-700/30 border border-gray-200 dark:border-slate-700 rounded-xl p-4 shadow-sm">
                                {isEditing ? (
                                  <div className="space-y-4">
                                    <div className="flex justify-between items-center">
                                      <h5 className="font-semibold text-gray-900 dark:text-white">{getTranslation('editingBatch', state.currentLanguage)}</h5>
                                    </div>

                                    <div className="grid grid-cols-1 gap-3">
                                      <div>
                                        <label className="text-xs text-gray-500 dark:text-slate-400 block mb-1">{getTranslation('batchNumHeader', state.currentLanguage)}</label>
                                        <input
                                          type="text"
                                          value={editingBatchData.batchNumber}
                                          disabled
                                          className="w-full px-3 py-2 border border-gray-300 dark:border-slate-600 bg-gray-100 dark:bg-slate-600/50 text-gray-500 dark:text-slate-400 rounded-lg text-sm focus:outline-none cursor-not-allowed"
                                          placeholder="Batch number"
                                        />
                                      </div>
                                      ...
                                      <div className="grid grid-cols-2 gap-3">
                                        <div>
                                          <label className="text-xs text-gray-500 dark:text-slate-400 block mb-1">{getTranslation('quantityHeader', state.currentLanguage)}</label>
                                          <input
                                            type="text"
                                            inputMode="decimal"
                                            value={editingBatchData.quantity}
                                            ...
                                            className="w-full px-3 py-2 border border-gray-300 dark:border-slate-600 dark:bg-slate-800 dark:text-white rounded-lg text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"
                                            placeholder="0"
                                          />
                                        </div>
                                        <div>
                                          <label className="text-xs text-gray-500 dark:text-slate-400 block mb-1">{getTranslation('priceHeader', state.currentLanguage)}</label>
                                          <input
                                            type="text"
                                            inputMode="decimal"
                                            value={editingBatchData.sellingUnitPrice}
                                      ...
                                      <div className="grid grid-cols-2 gap-3">
                                        <div>
                                          <label className="text-xs text-gray-500 dark:text-slate-400 block mb-1">{getTranslation('costHeader', state.currentLanguage)}</label>
                                          <input
                                            type="text"
                                            inputMode="decimal"
                                            value={editingBatchData.costPrice}
                                      ...
                                      {selectedProduct.trackExpiry && (
                                        <div className="grid grid-cols-2 gap-3">
                                          <div>
                                            <label className="text-xs text-gray-500 dark:text-slate-400 block mb-1">{getTranslation('expiryHeader', state.currentLanguage)}</label>
                                            <input
                                              type="date"
                                              value={editingBatchData.expiry}
                                              onChange={(e) => handleBatchInputChange('expiry', e.target.value)}
                                          ...
                                          <div>
                                            <label className="text-xs text-gray-500 dark:text-slate-400 block mb-1">{getTranslation('mfgDateHeader', state.currentLanguage)}</label>
                                            <input
                                              type="date"
                                              value={editingBatchData.mfg}
                                              onChange={(e) => handleBatchInputChange('mfg', e.target.value)}
                                      ...
                                      <div className="flex gap-2 pt-2">
                                        ...
                                        >
                                          {getTranslation('saveChanges', state.currentLanguage)}
                                        </button>
                                        <button
                                          onClick={() => handleEditBatch(batch)}
