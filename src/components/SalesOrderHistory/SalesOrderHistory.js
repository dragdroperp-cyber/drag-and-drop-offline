...
const SalesOrderHistory = () => {
  ...
  const generateThermalBill = async (size, invoiceNumber, billData) => {
    ...
    // Fallback: If delivery charge is missing but there is a gap between total and (items - discount), infer it.
  ...
  const generateA4Bill = async (billData) => {
    ...
    if (logoShow) {
      try {
        ...
        const sellerLogo = state.storeLogo || state.currentUser?.logoUrl || settings.logoUrl;
    ...
    pdf.setFont('helvetica', 'bold');
    pdf.setFontSize(22);
    pdf.setTextColor(...COLORS.text); // User requested black
    const storeName = state.storeName || state.currentUser?.shopName || 'Grocery Store';
    safeDrawText(pdf, storeName.toUpperCase(), margin + logoOffset, y + 6, { fontSize: 22, color: `rgb(${COLORS.text.join(',')})` });

    pdf.setFontSize(8);
    pdf.setFontSize(8);
    pdf.setTextColor(...COLORS.text); // User requested black for top part
    ...
    // Shop Address
    // Shop Address (Matched to ViewBill spacing)
    pdf.setDrawColor(0, 0, 0); // Black vertical line
    pdf.setLineWidth(1); // Thicker line
    ...
    pdf.setTextColor(30, 41, 59); // Black/Dark to match image
  ...
  const pendingSales = filteredOrders
    .filter(order => order.orderSource === 'online' && order.orderStatus !== 'Delivered' && order.orderStatus !== 'Cancelled')
    .reduce((sum, order) => {
      const refund = refundsMap[order._id || order.id] || 0;
      const gross = Number(order.totalAmount) || Number(order.total) || 0;
      return sum + (gross - refund);
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 auto-rows-[1fr]">
        ...
        <div className="relative bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 transition-all hover:shadow-md h-full">
          ...
          <div className="mt-2">
            ...
            <p className="text-2xl font-semibold text-purple-600" title={formatCurrency(totalDeliveryCharges)}>
        ...
        >
          <div className="absolute top-4 right-4 p-2.5 rounded-xl bg-rose-50 dark:bg-rose-900/20 text-rose-600 dark:text-rose-400 group-hover:bg-rose-100 dark:group-hover:bg-rose-900/40 transition-colors">
          ...
          <div className="mt-2">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] font-bold bg-rose-50 dark:bg-rose-900/20 text-rose-600 px-1.5 py-0.5 rounded uppercase tracking-wider">
                {periodRefunds.length} {periodRefunds.length === 1 ? 'Refund' : 'Refunds'}
              </span>
            </div>
            <p className="text-[10px] text-slate-400 mt-2 font-medium group-hover:text-rose-500 transition-colors flex items-center gap-1">
              Click to view details <Eye className="h-2.5 w-2.5" />
            </p>
          </div>
        </div>
        ...
        <div className="relative bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 transition-all hover:shadow-md h-full">
          <div className="absolute top-4 right-4 p-2.5 rounded-xl bg-cyan-50 dark:bg-cyan-900/20 text-cyan-600 dark:text-cyan-400">
            <IndianRupee className="h-5 w-5" />
          </div>
          <div className="mt-2">
