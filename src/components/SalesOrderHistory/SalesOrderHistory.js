...
const SalesOrderHistory = () => {
  ...
  const executePrint = async (format) => {
    ...
    try {
      ...
      // Attempt to fetch fresh data (specifically Invoice Number) from server if missing locally
      // This handles cases where local state hasn't synced the invoiceNumber back from the server yet
      if ((!freshOrder.invoiceNumber || String(freshOrder.invoiceNumber).startsWith('ord-')) && freshOrder.customerMobile) {
        try {
  ...
  const handleShareInvoice = (order) => {
    ...
    const message = encodeURIComponent(billUrl);
    // Remove space at end
