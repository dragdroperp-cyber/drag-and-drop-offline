...
const SalesOrderHistory = () => {
  ...
  const getOrderDisplayTotals = (order) => {
    ...
    if (items.length > 0) {
      ...
      // Note: Tax might be aggregated if not explicit
      const discount = Number(order.discountAmount || order.discount || 0);
      const delivery = Number(order.deliveryCharge || 0);
      // We assume taxAmount is available if Tax was applied
      const tax = Number(order.taxAmount || 0);

      itemsSubtotal = finalTotal + discount - delivery - tax;
      // Safety check: if calculation goes negative (bad data), default to final
      if (itemsSubtotal < 0) itemsSubtotal = finalTotal;
    }

    // Determine if we should show the original (strikethrough) amount
    const diff = Math.abs(itemsSubtotal - finalTotal);
    const hasDiff = diff > 0.05; // Tolerance for float errors

    return {
      finalTotal,
      originalTotal: itemsSubtotal,
      hasDiscount: hasDiff, // Reuse this flag name to drive the UI logic (show/hide strikethrough)
      discount: finalTotal < itemsSubtotal ? (itemsSubtotal - finalTotal) : 0
    };
  };
