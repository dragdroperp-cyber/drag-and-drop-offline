...
const SalesOrderHistory = () => {
  ...
  const generateThermalBill = async (size, invoiceNumber, billData) => {
    ...
    // Setup Colors (Default Blue if not set)
    const accentHex = sellerSettings?.billSettings?.accentColor || '#2f3c7e';
    const hexToRgb = (hex) => {
    ...
    const drawContent = async (pdf) => {
      ...
      // HEADER
      const logoUrl = state.storeLogo || state.currentUser?.logoUrl || sellerSettings?.billSettings?.logoUrl;
      // Note: Image loading skipped for brevity/reliability in history view, unless cached/available.
      // Implementing detailed logo loading might be complex if CORS issues arise, but basic attempt:
      if (logoUrl) {
        try {
          // Basic placeholder for logo logic if needed, or skip to ensure speed
        } catch (e) { }
      }
      ...
      pdf.setTextColor(0, 0, 0);
      pdf.setFont('helvetica', 'bold');
      const invNo = invoiceNumber || billData.id;
      // Adjust if too long
      const finalInvNo = (size === '58mm' && invNo.length > 10) ? invNo.slice(-10) : invNo;
      pdf.text(finalInvNo, margin + 10, metaY); // Approximate layout

      const dateStr = formatDateTime(billData.date).split(',')[0];
      ...
      const cols = size === '58mm' ? [
        { name: "Item", x: margin, align: 'left' },
        { name: "Qty", x: width - margin - 22, align: 'right' },
        { name: "Amt", x: width - margin, align: 'right' }
      ] : [
        { name: "Item Name", x: margin, align: 'left' },
        { name: "Qty", x: width - margin - 25, align: 'right' },
      ...
      items.forEach((item) => {
        ...
        const nameMaxW = size === '58mm' ? 25 : 40;
        const nameLines = pdf.splitTextToSize(item.name || 'Item', nameMaxW);

        safeDrawText(pdf, nameLines[0], cols[0].x, y, { fontSize: size === '58mm' ? 8 : 9 });
        if (nameLines.length > 1) {
          // Print rest of lines? Skip for compactness or handle?
          // Simple fallback: just first line or multiple
        }

        if (size === '58mm') {
          pdf.text(qty.toString(), cols[1].x, y, { align: 'right' });
          pdf.text(total.toFixed(2), cols[2].x, y, { align: 'right' });
        } else {
          pdf.text(qty.toString(), cols[1].x, y, { align: 'right' });
          pdf.text(rate.toFixed(2), cols[2].x, y, { align: 'right' });
          pdf.text(total.toFixed(2), cols[3].x, y, { align: 'right' });
        }

        y += (Math.max(1, nameLines.length) * 3.5);
      });
      ...
      // TOTALS
      pdf.setFontSize(size === '58mm' ? 9 : 10);
      pdf.setFont('helvetica', 'bold');
      pdf.text(`Total Qty: ${totalQty}`, margin, y);
      pdf.text(Number(billData.total).toFixed(2), width - margin, y, { align: 'right' });
      y += 6;
      drawDashedLine(y);
      y += 4;
      ...
      pdf.setFontSize(8);
      pdf.setFont('helvetica', 'bold');
      centerText(pdf, "Thank You", y, 8, 'bold');
      y += 4;

      // QR CODE (Optional)
      if (billData.upiId && Number(billData.total) > 0) {
        try {
          // We need await here, so drawContent is async
          const qrUrl = `upi://pay?pa=${billData.upiId}&am=${Number(billData.total).toFixed(2)}&cu=INR&tn=Bill%20Payment`;
          const qrImg = await QRCode.toDataURL(qrUrl, { margin: 1 });
    ...
    // Helper to center text in drawContent (needs pdf context)
    const centerText = (doc, txt, yPos, fSize, fStyle) => {
      doc.setFontSize(fSize);
      doc.setFont('helvetica', fStyle);
      doc.text(txt, centerX, yPos, { align: 'center' });
    };
  ...
  const generateA4Bill = async (billData) => {
    ...
    safeDrawText(pdf, storeName.toUpperCase(), margin, y + 6, { fontSize: 22, color: `rgb(${accentColor.join(',')})` });

    pdf.setFontSize(8);
    pdf.setTextColor(148, 163, 184);
    pdf.text('RETAIL INVOICE', margin, y + 11);
    ...
    const address = state.currentUser?.shopAddress || state.storeAddress || '';
    if (address) pdf.text(address, margin + 4, y + 3);

    const phone = state.currentUser?.phoneNumber || state.currentUser?.phone || '';
    if (phone) pdf.text(`Phone: ${phone}`, margin + 4, y + 7);

    const gstin = state.currentUser?.gstNumber || '';
    pdf.setFont('helvetica', 'bold');
    if (gstin) pdf.text(`GSTIN: ${gstin}`, margin + 4, y + 11);
    ...
    pdf.text('INVOICE DETAILS', pageWidth - margin, y, { align: 'right' });
    y += 5;
    ...
    pdf.text(`No: ${billData.id}`, pageWidth - margin, y, { align: 'right' });
    y += 5;
    pdf.text(`Date: ${formatDateTime(billData.date)}`, pageWidth - margin, y, { align: 'right' });

    y += 8;

    // TABLE
    // Cols: Sr, Item, Qty, Rate, Total
    const cols = [
      { name: "Sr", w: 10, align: 'center' },
      { name: "Item Name", w: 80, align: 'left' },
      { name: "Rate", w: 30, align: 'right' },
      { name: "Qty", w: 25, align: 'center' },
      { name: "Total", w: 45, align: 'right' }
    ];

    // Table Header
    pdf.setFillColor(...accentColor);
    pdf.roundedRect(margin, y, pageWidth - (margin * 2), 8, 1, 1, 'F');
    ...
    cols.forEach(c => {
      let tx = cx + (c.w / 2);
      if (c.align === 'left') tx = cx + 2;
      if (c.align === 'right') tx = cx + c.w - 2;
      pdf.text(c.name, tx, y + 5, { align: c.align });
      cx += c.w;
    });
    ...
    items.forEach((item, i) => {
      ...
      cx = margin;
      // Sr
      pdf.text(String(i + 1), cx + cols[0].w / 2, y + 3, { align: 'center' }); cx += cols[0].w;
      // Name
      safeDrawText(pdf, item.name, cx + 2, y + 3, { fontSize: 9 }); cx += cols[1].w;
      // Rate
      pdf.text(rate.toFixed(2), cx + cols[2].w - 2, y + 3, { align: 'right' }); cx += cols[2].w;
      // Qty
      pdf.text(qty.toString(), cx + cols[3].w / 2, y + 3, { align: 'center' }); cx += cols[3].w;
      // Total
      pdf.text(total.toFixed(2), cx + cols[4].w - 2, y + 3, { align: 'right' });
    ...
    pdf.setFont('helvetica', 'bold');
    pdf.setFontSize(12);
    pdf.text("GRAND TOTAL", rightX, y + 5);
    pdf.setTextColor(...accentColor);
    pdf.text(Math.round(grandTotal).toFixed(2), valX, y + 5, { align: 'right' });

    // QR Code
    if (billData.upiId && grandTotal > 0) {
      try {
        const qrUrl = `upi://pay?pa=${billData.upiId}&am=${grandTotal.toFixed(2)}&cu=INR&tn=Bill%20Payment`;
        const qrImg = await QRCode.toDataURL(qrUrl, { margin: 1 });
        const qrSize = 25;
        const qy = pageHeight - 35;
        pdf.addImage(qrImg, 'PNG', pageWidth - margin - qrSize, qy, qrSize, qrSize);
      } catch (e) { }
    }

    // Footer Msg
    const footerMsg = sellerSettings?.billSettings?.footerMessage || 'Thank you for your business!';
    pdf.setFontSize(9);
    pdf.setTextColor(148, 163, 184);
    pdf.text(footerMsg, pageWidth / 2, pageHeight - 15, { align: 'center' });
  ...
  const printOrder = async (order) => {
    try {
      ...
      const printSize = localStorage.getItem('printSize') || sellerSettings?.billSettings?.billFormat || 'a4';

      const billData = {
        id: order.id || `INV-${Date.now()}`,
        customerName: order.customerName,
        customerMobile: order.customerMobile,
        items: order.items,
        total: order.totalAmount || order.total,
        date: order.createdAt || order.date,
        upiId: order.upiId || state.currentUser?.upiId || state.upiId || sellerSettings?.billSettings?.upiId,
      ...
      if (printSize === '58mm' || printSize === '80mm') {
        await generateThermalBill(printSize, billData.id, billData);
      } else {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="relative bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 transition-all hover:shadow-md mb-6">
          ...
          <div className="mt-2">
            ...
            <p className="text-2xl font-semibold text-red-600" title={formatCurrency(dueSales)}>
