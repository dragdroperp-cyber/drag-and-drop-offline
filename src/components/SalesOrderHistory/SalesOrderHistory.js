...
const SalesOrderHistory = () => {
  ...
  const generateThermalBill = async (size, invoiceNumber, billData) => {
    ...
    const drawContent = async (pdf) => {
      ...
      // HEADER
      const logoUrl = state.storeLogo || state.currentUser?.logoUrl;
      if (logoUrl) {
        try {
          // Simplified logo loading for history to prevent cross-origin issues if not cached
          // If needed, same image loading logic as Billing.js can be added here
        } catch (e) { }
      }
  ...
  const generateA4Bill = async (billData) => {
    ...
    // 2. Header
    // Logo logic simplified similar to thermal

    pdf.setFont('helvetica', 'bold');
    pdf.setFontSize(22);
    pdf.setTextColor(...COLORS.text); // User requested black
    const storeName = state.storeName || state.currentUser?.shopName || 'Grocery Store';
    safeDrawText(pdf, storeName.toUpperCase(), margin, y + 6, { fontSize: 22, color: `rgb(${COLORS.text.join(',')})` });
