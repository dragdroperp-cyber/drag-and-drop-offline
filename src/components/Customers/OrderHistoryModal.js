        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 border-b border-gray-200 dark:border-slate-700 px-4 sm:px-6 py-4 flex-shrink-0">
          <div className="flex-1 min-w-0">
            ...
            <div className="flex flex-wrap gap-2 text-xs text-gray-500">
              <span className="inline-flex items-center gap-1 rounded-full bg-gray-100 dark:bg-slate-700 px-2 py-1 text-gray-600 dark:text-slate-300">
                ...
                >
                  <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 px-4 sm:px-5 py-4">
                    ...
                    <div className="flex items-center justify-between sm:block sm:text-right">
                      <div>
                        <p className="text-lg sm:text-xl font-semibold text-primary-600 dark:text-primary-400">{formatCurrency(breakdown.netTotal)}</p>
                    ...
                    if (paymentMethod === 'split') {
                      ...
                      return (
                        <div className="px-4 sm:px-5 pb-3">
                          <p className="text-xs sm:text-sm text-gray-600 dark:text-slate-300 mb-2 font-medium">Split Payment Breakdown</p>
                          <div className="grid grid-cols-3 gap-2 sm:gap-3">
                            <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-900/30 rounded-lg p-2 sm:p-3">
                              <p className="text-[10px] sm:text-xs text-green-700 dark:text-green-400 font-medium mb-0.5 sm:mb-1">Cash</p>
                              <p className="text-sm sm:text-lg font-bold text-green-900 dark:text-green-100">{formatCurrency(cashAmount)}</p>
                            </div>
                            <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-900/30 rounded-lg p-2 sm:p-3">
                              <p className="text-[10px] sm:text-xs text-blue-700 dark:text-blue-400 font-medium mb-0.5 sm:mb-1">Online</p>
                              <p className="text-sm sm:text-lg font-bold text-blue-900 dark:text-blue-100">{formatCurrency(onlineAmount)}</p>
                            </div>
                            <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-900/30 rounded-lg p-2 sm:p-3">
                              <p className="text-[10px] sm:text-xs text-red-700 dark:text-red-400 font-medium mb-0.5 sm:mb-1">Due</p>
                              <div className="text-sm sm:text-lg font-bold text-red-900 dark:text-red-100 flex items-center gap-2">
                                {(order.allPaymentClear && dueAmount > 0) ? (
                                  <>
                                    <span className="line-through text-red-900/50 dark:text-red-100/50 text-xs">{formatCurrency(dueAmount)}</span>
                                    <span>{formatCurrency(0)}</span>
                                  </>
                                ) : (
                                  <span>{formatCurrency(order.allPaymentClear ? 0 : dueAmount)}</span>
                                )}
                              </div>
                            </div>
                          </div>
                  ...
                  {orderItems.length > 0 && (
                    <div className="px-4 sm:px-5 pb-4 space-y-3">
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3">
                        <div className="rounded-lg bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-600 px-2.5 sm:px-3 py-2 sm:py-2.5">
                          <p className="text-[10px] sm:text-xs text-gray-500 dark:text-slate-400 uppercase tracking-wide mb-1">Subtotal</p>
                          <p className="text-xs sm:text-sm font-semibold text-gray-900 dark:text-white break-words">{formatCurrency(breakdown.subtotal)}</p>
                        </div>
                        <div className="rounded-lg bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-600 px-2.5 sm:px-3 py-2 sm:py-2.5">
                          <p className="text-[10px] sm:text-xs text-gray-500 dark:text-slate-400 uppercase tracking-wide mb-1">Discount ({(breakdown.discountPercent || 0).toFixed(2)}%)</p>
                          <p className="text-xs sm:text-sm font-semibold text-emerald-600 dark:text-emerald-400 break-words">
                            - {formatCurrency(breakdown.discountAmount)}
                          </p>
                        </div>
                        <div className="rounded-lg bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-600 px-2.5 sm:px-3 py-2 sm:py-2.5">
                          <p className="text-[10px] sm:text-xs text-gray-500 dark:text-slate-400 uppercase tracking-wide mb-1">Tax ({(breakdown.taxPercent || 0).toFixed(2)}%)</p>
                          <p className="text-xs sm:text-sm font-semibold text-purple-600 dark:text-purple-400 break-words">
                            + {formatCurrency(breakdown.taxAmount)}
                          </p>
                        </div>
                        <div className="rounded-lg bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-600 px-2.5 sm:px-3 py-2 sm:py-2.5">
                          <p className="text-[10px] sm:text-xs text-gray-500 dark:text-slate-400 uppercase tracking-wide mb-1">Net Total</p>
                          <p className="text-xs sm:text-sm font-semibold text-primary-600 dark:text-primary-400 break-words">{formatCurrency(breakdown.netTotal)}</p>
                        </div>
                      </div>
