...
const OrderHistoryModal = ({ customer, orders, onClose }) => {
  ...
  const getPaymentMethodLabel = (method, splitDetails) => {
    ...
    if (m === 'split' && splitDetails) {
      return getTranslation('split', state.currentLanguage);
    }
  ...
  const handleMarkPaid = useCallback(async (order) => {
    if (isPlanExpired(state)) {
      if (window.showToast) {
        window.showToast(getTranslation('planExpiredMarkPaid', state.currentLanguage), 'warning', 8000);
      }
      return;
    }
    if (!window.confirm(getTranslation('markPaidConfirm', state.currentLanguage))) return;
    ...
    try {
      ...
      showToast(getTranslation('paymentSynced', state.currentLanguage), 'success');
    } catch (error) {
      console.error("Sync failed", error);
      showToast(getTranslation('syncedLocallyOnly', state.currentLanguage), "warning");
    }
  ...
  return (
    ...
    >
      ...
      <div
        ...
        className="bg-white dark:bg-slate-800 !rounded-none md:!rounded-2xl shadow-2xl w-full !h-full md:h-auto md:max-w-3xl md:max-h-[90vh] flex flex-col overflow-hidden transition-colors fixed inset-0 md:relative md:inset-auto m-0"
      ...
      >
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 border-b border-gray-200 dark:border-slate-700 px-4 sm:px-6 py-4 flex-shrink-0">
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm sm:text-base text-gray-500 dark:text-slate-400 mb-3 break-words line-clamp-1" title={`${customer?.name || 'Customer'} • ${filteredOrders.length} ${getTranslation('orders', state.currentLanguage)} • ${getTranslation('netPaid', state.currentLanguage)} ${formatCurrency(totalSpend)}`}>
              {customer?.name || 'Customer'} • {filteredOrders.length} {getTranslation('orders', state.currentLanguage)} • {getTranslation('netPaid', state.currentLanguage)} <span className="text-emerald-600 font-bold">{formatCurrencySmart(totalSpend, state.currencyFormat)}</span>
            </p>
            <div className="grid grid-cols-2 lg:flex lg:flex-wrap gap-2 text-sm sm:text-base text-gray-500">
              <span className="inline-flex items-center gap-1.5 rounded-lg bg-gray-50 dark:bg-slate-700/50 px-3 py-2 text-gray-600 dark:text-slate-300 border border-gray-100 dark:border-slate-700 min-w-0">
                <span className="h-2 w-2 rounded-full bg-blue-500 flex-shrink-0" />
                <span className="truncate">{getTranslation('subtotal', state.currentLanguage)} <span className="text-emerald-600 font-bold" title={formatCurrency(totalSubtotal)}>{formatCurrencySmart(totalSubtotal, state.currencyFormat)}</span></span>
              </span>
              <span className="inline-flex items-center gap-1.5 rounded-lg bg-gray-50 dark:bg-slate-700/50 px-3 py-2 text-gray-600 dark:text-slate-300 border border-gray-100 dark:border-slate-700 min-w-0">
                <span className="h-2 w-2 rounded-full bg-emerald-500 flex-shrink-0" />
                <span className="truncate">{getTranslation('disc', state.currentLanguage)} <span className="text-rose-600 font-bold" title={formatCurrency(roundAmount(totalDiscount))}>{formatCurrencySmart(roundAmount(totalDiscount), state.currencyFormat)}</span></span>
              </span>
              <span className="inline-flex items-center gap-1.5 rounded-lg bg-gray-50 dark:bg-slate-700/50 px-3 py-2 text-gray-600 dark:text-slate-300 border border-gray-100 dark:border-slate-700 min-w-0">
                <span className="h-2 w-2 rounded-full bg-purple-500 flex-shrink-0" />
                <span className="truncate">{getTranslation('tax', state.currentLanguage)} <span title={formatCurrency(roundAmount(totalTax))}>{formatCurrencySmart(roundAmount(totalTax), state.currencyFormat)}</span></span>
              </span>
              <span className="inline-flex items-center gap-1.5 rounded-lg bg-emerald-50 dark:bg-emerald-900/20 px-3 py-2 text-emerald-700 dark:text-emerald-300 border border-emerald-100 dark:border-emerald-900/30 min-w-0">
                <span className="h-2 w-2 rounded-full bg-emerald-500 flex-shrink-0" />
                <span className="truncate font-bold">{getTranslation('total', state.currentLanguage)} <span title={formatCurrency(roundAmount(totalSpend))}>{formatCurrencySmart(roundAmount(totalSpend), state.currencyFormat)}</span></span>
              </span>
            </div>
          </div>
        ...
        <div className="px-4 pt-3 pb-3 sm:hidden border-b border-gray-200 dark:border-slate-700 flex-shrink-0">
          <div className="flex flex-col gap-2">
            <label className="text-xs font-medium text-gray-600 dark:text-slate-400">{getTranslation('filterOrders', state.currentLanguage)}</label>
                ...
                >
                  <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 px-4 sm:px-5 py-4">
                    ...
                    <div className="flex items-center justify-between sm:block sm:text-right">
                      <div>
                        <p className="text-lg sm:text-xl font-semibold text-emerald-600" title={formatCurrency(breakdown.netTotal)}>{formatCurrencySmart(breakdown.netTotal, state.currencyFormat)}</p>
                          ...
                          if (isDue || isSplitDue) {
                            return (
                              ...
                              >
                                ...
                                <span className="hidden sm:inline">{getTranslation('markPaid', state.currentLanguage)}</span>
                                <span className="sm:hidden">{getTranslation('payNow', state.currentLanguage)}</span>
                              </button>
                            );
                          }
                          return null;
                    ...
                    if (paymentMethod === 'split') {
                      ...
                      return (
                        <div className="px-4 sm:px-5 pb-3">
                          <p className="text-xs sm:text-base text-gray-500 dark:text-slate-400 mb-2 font-bold uppercase tracking-wider">{getTranslation('paymentBreakdown', state.currentLanguage)}</p>
                          <div className="grid grid-cols-3 gap-2 sm:gap-3">
                            <div className="bg-green-50 dark:bg-green-900/10 border border-green-100 dark:border-green-900/30 rounded-xl p-2 sm:p-3 min-w-0">
                              <p className="text-xs sm:text-sm text-green-600 dark:text-green-400 font-bold uppercase mb-0.5 truncate text-center">{getTranslation('cash', state.currentLanguage)}</p>
                              <p className="text-base sm:text-xl font-black text-green-700 dark:text-green-300 whitespace-nowrap overflow-x-auto scrollbar-hide text-center" title={formatCurrency(cashAmount)}>
                            ...
                            <div className="bg-blue-50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-900/30 rounded-xl p-2 sm:p-3 min-w-0">
                              <p className="text-xs sm:text-sm text-blue-600 dark:text-blue-400 font-bold uppercase mb-0.5 truncate text-center">{getTranslation('online', state.currentLanguage)}</p>
                              <p className="text-base sm:text-xl font-black text-blue-700 dark:text-blue-300 whitespace-nowrap overflow-x-auto scrollbar-hide text-center" title={formatCurrency(onlineAmount)}>
                            ...
                            <div className="bg-red-50 dark:bg-red-900/10 border border-red-100 dark:border-red-900/30 rounded-xl p-2 sm:p-3 min-w-0">
                              <p className="text-xs sm:text-sm text-red-600 dark:text-red-400 font-bold uppercase mb-0.5 truncate text-center">{getTranslation('due', state.currentLanguage)}</p>
                              <div className="text-base sm:text-xl font-black text-red-700 dark:text-red-300 flex items-center justify-center gap-1.5 truncate">
                                {(order.allPaymentClear && dueAmount > 0) ? (
                                  <>
                                    <span className="line-through text-red-900/30 dark:text-red-100/30 text-[10px] sm:text-sm" title={formatCurrency(dueAmount)}>{formatCurrencySmart(dueAmount, state.currencyFormat)}</span>
                    ...
                    <div className="px-4 sm:px-5 pb-4 space-y-3">
                      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-2 sm:gap-3">
                        <div className="rounded-xl bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 px-3 py-2.5 min-w-0">
                          <p className="text-sm sm:text-base text-gray-500 dark:text-slate-400 uppercase font-black tracking-widest mb-1 truncate">{getTranslation('subtotal', state.currentLanguage)}</p>
                          <p className="text-base sm:text-lg font-black text-emerald-600 whitespace-nowrap overflow-x-auto scrollbar-hide" title={formatCurrency(breakdown.subtotal)}>
                            {formatCurrencySmart(breakdown.subtotal, state.currencyFormat)}
                          </p>
                        </div>
                        <div className="rounded-xl bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 px-3 py-2.5 min-w-0">
                          <p className="text-sm sm:text-base text-rose-600 dark:text-rose-400 uppercase font-black tracking-widest mb-1 truncate">{getTranslation('disc', state.currentLanguage)} ({(breakdown.discountPercent || 0).toFixed(0)}%)</p>
                          <p className="text-base sm:text-lg font-black text-rose-600 dark:text-rose-400 whitespace-nowrap overflow-x-auto scrollbar-hide font-black" title={`- ${formatCurrency(breakdown.discountAmount)}`}>
                            - {formatCurrencySmart(breakdown.discountAmount, state.currencyFormat)}
                          </p>
                        </div>
                        <div className="rounded-xl bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 px-3 py-2.5 min-w-0">
                          <p className="text-sm sm:text-base text-slate-900 dark:text-slate-100 uppercase font-black tracking-widest mb-1 truncate">{getTranslation('tax', state.currentLanguage)} ({(breakdown.taxPercent || 0).toFixed(0)}%)</p>
                          <p className="text-base sm:text-lg font-black text-slate-900 dark:text-slate-100 whitespace-nowrap overflow-x-auto scrollbar-hide font-black" title={`+ ${formatCurrency(breakdown.taxAmount)}`}>
                        ...
                        <div className="rounded-xl bg-emerald-50 dark:bg-emerald-900/10 border border-emerald-100 dark:border-emerald-900/30 px-3 py-2.5 min-w-0">
                          <p className="text-sm sm:text-base text-emerald-700 dark:text-emerald-300 uppercase font-black tracking-widest mb-1 truncate">{getTranslation('total', state.currentLanguage)}</p>
                          <p className="text-base sm:text-lg font-black text-emerald-700 dark:text-emerald-300 whitespace-nowrap overflow-x-auto scrollbar-hide font-black" title={formatCurrency(breakdown.netTotal)}>
                      ...
                      <div className="bg-white dark:bg-slate-800 rounded-lg sm:rounded-xl border border-gray-200 dark:border-slate-700 overflow-x-auto">
                        <table className="min-w-full divide-y divide-gray-200 dark:divide-slate-700 text-base sm:text-lg">
                          <thead className="bg-gray-100 dark:bg-slate-700">
                            <tr>
                              <th className="px-3 sm:px-4 py-2 text-left font-medium text-gray-600 dark:text-slate-300 whitespace-nowrap">{getTranslation('itemHeader', state.currentLanguage)}</th>
                              <th className="px-3 sm:px-4 py-2 text-center font-medium text-gray-600 dark:text-slate-300 whitespace-nowrap">{getTranslation('qtyHeader', state.currentLanguage)}</th>
                              <th className="px-3 sm:px-4 py-2 text-right font-medium text-gray-600 dark:text-slate-300 whitespace-nowrap">{getTranslation('rateHeader', state.currentLanguage)}</th>
                              <th className="px-3 sm:px-4 py-2 text-right font-medium text-gray-600 dark:text-slate-300 whitespace-nowrap">{getTranslation('totalHeader', state.currentLanguage)}</th>
                            </tr>
                          </thead>
                            ...
                            {orderItems.map((item, idx) => {
                              ...
                              return (
                                <tr key={idx}>
                                  ...
                                  <td className="px-3 sm:px-4 py-2 text-right font-medium text-gray-700 dark:text-slate-300 whitespace-nowrap" title={formatCurrency(lineTotal)}>{formatCurrencySmart(lineTotal, state.currencyFormat)}</td>
                                </tr>
                              );
                            })}
