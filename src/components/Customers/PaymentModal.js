...
import { X, ArrowDownCircle, ArrowUpCircle, DollarSign, RotateCcw } from 'lucide-react';
...
const PaymentModal = ({ customer, onClose, onSubmit }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 overflow-y-auto">
      <div className="bg-white rounded-2xl shadow-[0_25px_80px_-12px_rgba(0,0,0,0.3)] w-full max-w-lg border border-gray-200/50 my-auto max-h-[95vh] flex flex-col">
        {/* Header */}
        <div className="flex items-center justify-between p-6 pb-4 border-b border-gray-100">
          <div>
            <h2 className="text-xl font-bold text-gray-900">Record Payment</h2>
            <p className="text-sm text-gray-600 mt-1">Manage customer balance</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="overflow-y-auto flex-1 p-6">
          {/* Customer Info Card */}
          <div className="mb-6 p-5 rounded-xl bg-gradient-to-br from-gray-50 to-white border border-gray-200/60">
            <div className="flex items-center justify-between mb-3">
              <p className="text-sm font-medium text-gray-600">Customer</p>
              <div className={`px-3 py-1 rounded-full text-xs font-semibold ${
                currentBalance > 0 ? 'bg-red-100 text-red-700' :
            ...
            <p className="text-lg font-bold text-gray-900 mb-4">{customer.name}</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <button
                  ...
                  className={`flex flex-col items-center justify-center gap-3 p-5 rounded-xl border-2 transition-all duration-200 ${
                    paymentType === 'receive'
                ...
                >
                  <ArrowUpCircle className={`h-7 w-7 ${paymentType === 'give' ? 'text-orange-600' : 'text-gray-400'}`} />
                  <div className="text-center">
                    <span className="font-semibold text-sm">Give Payment</span>
                    <p className="text-xs text-gray-500 mt-1">You pay/refund</p>
                  </div>
                </button>
              </div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {getQuickAmounts().map((option, index) => (
                    <button
                      ...
                      className="p-3 rounded-lg bg-blue-50 hover:bg-blue-100 text-blue-700 font-medium text-sm transition-colors border border-blue-200 hover:border-blue-300"
                    >
                      {option.label}
                      <br />
            ...
            <div>
              ...
              <input
                ...
                step="0.01"
                min="0.01"
                  className="w-full pl-10 pr-4 py-3 rounded-xl border border-gray-200 bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all text-gray-900 placeholder:text-gray-400 text-lg font-medium"
