...
const WhatsAppBillModal = ({ customer, orders, onClose }) => {
            ...
            selectedOrdersList.forEach((order, index) => {
                ...
                if (includeDetails && order.items && order.items.length > 0) {
                    ...
                    order.items.forEach(i => {
                        ...
                        message += `\n  • ${i.name}: ${qty} x ${formatCurrency(rate)}/unit = ${formatCurrency(amount)}`;
                    });
                }
    ...
    return (
        <div
            className={`fixed inset-0 bg-gray-900 bg-opacity-50 flex items-end sm:items-center justify-center z-[60] transition-opacity duration-300 ${isClosing ? 'opacity-0' : 'animate-fadeIn'}`}
            onClick={handleClose}
        >
            <style>{`
                @keyframes slideUp {
            ...
            <div
                ...
                className="bg-white dark:bg-slate-800 rounded-none sm:rounded-xl shadow-2xl w-full max-w-lg h-auto max-h-[95vh] sm:h-auto sm:max-h-[90vh] flex flex-col transition-colors relative overflow-hidden"
