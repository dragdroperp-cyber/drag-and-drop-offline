...
const WhatsAppBillModal = ({ customer, orders, onClose }) => {
    ...
    const handleSend = () => {
        ...
        let message = `Hello ${customer.name}, ${getTranslation('greetingsFrom', state.currentLanguage)} ${storeName}.\n\n`;
        ...
        if (selectedOrdersList.length > 0) {
            ...
            selectedOrdersList.forEach((order, index) => {
                ...
                if (due < total && due > 0) {
                    message += `\n${getTranslation('total', state.currentLanguage)}: ₹${total.toFixed(2)} | *${getTranslation('due', state.currentLanguage)}: ₹${due.toFixed(2)}*\n`;
                } else {
                    message += `\n*${getTranslation('due', state.currentLanguage)}: ₹${due.toFixed(2)}*\n`;
                }
            });

            message += `\n*${getTranslation('totalPendingAmount', state.currentLanguage)}: ₹${totalSelectedDue.toFixed(2)}*`;
        } else {
            ...
            if ((customer.balanceDue || 0) > 0) {
                message += `${getTranslation('totalPendingBalance', state.currentLanguage)} ₹${balance}.`;
            } else {
                                                        ...
                                                        {(() => {
                                                            ...
                                                            return (
                                                                <div className="text-right">
                                                                    {due < total && (
                                                                        <div className="text-xs text-gray-400 line-through">{getTranslation('total', state.currentLanguage)}: ₹{total.toFixed(2)}</div>
                                                                    )}
