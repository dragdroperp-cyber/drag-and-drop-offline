...
const WhatsAppBillModal = ({ customer, orders, onClose }) => {
    ...
    const pendingOrders = customerOrders.filter(o => {
        ...
        const status = (o.paymentStatus || o.paymentMethod || o.status || '').toLowerCase();

        console.log(`[WhatsAppModal] Checking Order ${o.id}: Status=${status}, Balance=${balanceDue}`);

        // 1. "Due" status -> Always show
        if (status.includes('due') || status === 'unpaid') return true;

        // 2. "Split" status -> Show if positive balance
        if (status.includes('split')) {
