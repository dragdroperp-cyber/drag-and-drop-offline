...
const EditCustomerModal = ({ customer, onClose, onSubmit }) => {
  ...
  return (
    <div className="fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center z-50 px-3 sm:px-0">
      <div
        ref={containerRef}
        className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl w-full max-w-md max-h-[90vh] mx-auto p-0 flex flex-col transition-colors"
        role="dialog"
        aria-modal="true"
        aria-labelledby="edit-customer-title"
      >
        <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-gray-100 dark:border-slate-700">
          <h2 id="edit-customer-title" className="text-2xl font-bold text-gray-800 dark:text-white">Edit Customer</h2>
          <button
            onClick={onClose}
            data-modal-close
