...
import { X, AlertTriangle, Minus } from 'lucide-react';
      ...
      >
        ...
        >
          ...
          {(() => {
            ...
            return (
              <div className="text-sm font-medium text-center p-2 rounded-lg bg-blue-50 text-blue-700 dark:bg-blue-900/20 dark:text-blue-300 border border-blue-100 dark:border-blue-800">
                ({getTranslation('customerLimitLeft', state.currentLanguage)}: {maxCustomers === Infinity ? getTranslation('unlimited', state.currentLanguage) : Math.max(0, maxCustomers - totalCustomers)})
              </div>
            );
          })()}
          ...
          <div className="sticky bottom-0 flex justify-end gap-3 pt-4 pb-1 bg-white dark:bg-slate-800 border-t dark:border-slate-700">
            ...
            >
              {getTranslation('saveCustomer', state.currentLanguage)}
            </button>
          </div>
        </form>
      </div>
