...
import { canAddData, getLimitErrorMessage, DataCreationManager } from '../../utils/planUtils';
      ...
      >
        ...
        >
          ...
          {(() => {
            const activeCustomers = state.customers.filter(customer => !customer.isDeleted);
            const totalCustomers = activeCustomers.length;
            const limit = state.aggregatedUsage?.customers?.limit;
            const used = state.aggregatedUsage?.customers?.used || totalCustomers;
            const remaining = limit === 'Unlimited' || limit === null ? 'Unlimited' : Math.max(0, (limit || 0) - used);
            const canAdd = remaining === 'Unlimited' || remaining > 0;
              ...
              <div className={`rounded-lg border p-3 ${canAdd ? 'border-blue-200 bg-blue-50' : 'border-red-200 bg-red-50'}`}>
                <div className="flex items-center justify-between text-xs">
                  ...
                  <span className={`font-semibold ${canAdd ? 'text-blue-900' : 'text-red-900'}`}>
                    {used} / {limit === 'Unlimited' || limit === null ? '∞' : limit}
                    {!canAdd && ' (Full)'}
                  </span>
                </div>
