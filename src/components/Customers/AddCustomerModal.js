...
const AddCustomerModal = ({
  ...
  const handleChange = (e) => {
    ...
    } else if (nextValue && name === 'balanceDue') {
  ...
  const handleSubmit = async (e, forceCreate = false) => {
    ...
    if (!forceCreate) {
      ...
      const matched = existingCustomers.find(c => {
        const exMob = sanitizeMobileNumber(c.mobileNumber || c.phone || '');
        const exEmail = (c.email || '').trim().toLowerCase();
        return (mobile && exMob === mobile); // Only check mobile for strong duplicate warning
      });
    ...
    const customerData = { ...formData, mobileNumber: mobile, dueAmount, balanceDue: dueAmount };
        ...
        {duplicateWarning ? (
          <div className="flex-1 overflow-y-auto p-6 flex flex-col items-center justify-center text-center space-y-6">
            ...
            <div className="space-y-2">
              <h3 className="text-xl font-bold text-gray-900 dark:text-white">Customer Already Exists</h3>
              <p className="text-sm text-gray-500 dark:text-gray-400 max-w-xs mx-auto">
                A customer with mobile <b>{duplicateWarning.mobileNumber}</b> already exists as <b>{duplicateWarning.name}</b>.
              </p>
            </div>
            ...
            <div className="flex flex-col w-full max-w-sm gap-3">
              <button
                onClick={() => handleCloseModal()}
                className="w-full py-3.5 rounded-lg font-bold text-sm text-white bg-slate-900 dark:bg-slate-800 hover:opacity-90 transition-all shadow-sm"
              >
                Use Existing Customer
              </button>
              <button
                onClick={() => handleSubmit(null, true)}
                className="w-full py-3.5 rounded-lg font-bold text-sm text-slate-600 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:text-slate-300 transition-all"
              >
                Create New Customer
              </button>
            </div>
          ...
          <form onSubmit={handleSubmit} className="flex-1 flex flex-col overflow-hidden">
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              {(limitError || duplicateError) && (
                <div className="p-3 bg-red-50 dark:bg-red-900/20 border border-red-100 dark:border-red-900/30 rounded-lg flex items-start gap-2">
                  ...
                  <p className="text-xs text-red-700 dark:text-red-400 font-medium leading-relaxed">{limitError || duplicateError}</p>
                </div>
              )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
                ...
                {/* Balance */}
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-gray-400 uppercase tracking-wider px-0.5">{getTranslation('initialBalanceDue', state.currentLanguage)}</label>
            ...
            <div className="p-6 pt-0 pb-8 md:pb-6">
              <button
                ...
                className="w-full py-3.5 rounded-lg font-bold text-sm text-white bg-slate-900 hover:opacity-90 transition-all active:scale-[0.98] shadow-sm flex items-center justify-center gap-2"
              >
