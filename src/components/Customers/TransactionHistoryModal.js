...
const TransactionHistoryModal = ({ customer, transactions, onClose }) => {
    ...
    const allTransactions = useMemo(() => {
        ...
        // Calculate opening balance based on current due and tracked transactions
        // Current Due = Opening Balance + Sum(Dues) - Sum(Payments)
        // Opening Balance = Current Due - Sum(Dues) + Sum(Payments)
        ...
        filtered.forEach(t => {
            ...
            const isCredit = ['credit', 'due', 'add_due'].includes(t.type);
    ...
    const filteredTransactions = useMemo(() => {
        ...
        return allTransactions.filter(t => {
            ...
            if (filterType === 'credit') {
                return ['credit', 'due', 'add_due'].includes(t.type);
            }
    ...
    return (
        ...
        >
            <div
                className={`bg-white dark:bg-slate-800 rounded-t-2xl sm:rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col overflow-hidden transition-all duration-400 ${isClosing ? 'translate-y-full sm:translate-y-4 sm:scale-95' : 'translate-y-0 sm:scale-100'}`}
