  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-50 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 transition-colors duration-500">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        {usageCards.length > 0 && (
          <div className="bg-white dark:bg-slate-800 rounded-3xl p-8 shadow-xl border dark:border-slate-700 animate-fadeIn">
                    ...
                    <div className="h-32 bg-gradient-to-br from-[#2F3C7E] via-[#3d4a8f] to-[#2F3C7E] dark:from-slate-700 dark:to-slate-800 relative overflow-hidden">
                      <div className="absolute top-6 left-6">
                        ...
                        <div className="flex items-baseline gap-2">
                          <span className="text-4xl font-black text-white">{formatCurrencySmart(plan.rawPrice || plan.price || 0, state.currencyFormat)}</span>
                    ...
                    <div className="p-8">
                      ...
                      <div className="text-center">
                        {isCurrent && plan.planType !== 'mini' ? (
                          ...
                          // Check if this is a 0rs plan (Free Plan) and the user has a paid plan currently active
                          const planPrice = plan?.rawPrice || parseFloat(plan?.price?.replace(/[^0-9.]/g, '') || '0');
                          const currentPlanData = plans.find(p => p.current);
                          const currentPlanPrice = currentPlanData?.rawPrice || parseFloat(currentPlanData?.price?.replace(/[^0-9.]/g, '') || '0');

                          // Only show expired for free plans when user is on a paid plan
                          if (planPrice === 0 && currentPlanPrice > 0) {
                            return (
                              <div className="inline-flex items-center justify-center w-full py-4 px-6 bg-red-50 dark:bg-red-500/10 text-red-500 dark:text-red-400 rounded-2xl border-2 border-red-100 dark:border-red-500/20 font-bold shadow-sm opacity-80 cursor-not-allowed">
                                <X className="h-5 w-5 mr-2" /> Expired
                              </div>
                            );
                          }
                          ...
                          return (
                            ...
                            >
                              ...
                              <div className="relative z-10 flex items-center justify-center gap-2">
                                {upgradingPlanId === (plan.id || plan._id) ? <Loader className="animate-spin h-5 w-5" /> : <span>{plan.planType === 'mini' ? 'Top-Up' : '🚀 View Details →'}</span>}
                              </div>
                            </button>
                          );
                        })()}
