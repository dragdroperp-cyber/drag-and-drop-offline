...
const BarcodeScanner = ({ onScan, onClose, inline = false, keepOpen = false, containerWidth, containerHeight }) => {
  ...
  useEffect(() => {
    ...
    if (sizeChanged && isRunningRef.current) {
      console.log('Container size changed significantly, restarting camera...');
      // Stop current camera
      stopScanner().then(() => {
  ...
  const startScanner = async (cameraId) => {
    ...
    // Use entire container as scanning area for inline mode
    let qrboxConfig;
    if (inline && containerWidth && containerHeight) {
      // For inline mode, use the full container area
      qrboxConfig = { width: containerWidth - 10, height: containerHeight - 10 };
    } else {
      // For modal mode, use default scanning area
      qrboxConfig = { width: 320, height: 150 };
    }

    const config = {
      fps: 10,
      qrbox: qrboxConfig,
      aspectRatio: inline ? (containerWidth && containerHeight ? containerWidth / containerHeight : 1.33) : 2.0,
