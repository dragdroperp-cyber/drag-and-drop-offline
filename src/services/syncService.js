            ...
            if (successItems.length > 0) {
              ...
              if (item.isDeleted === true && syncedItemData.action === 'deleted') {
                ...
                if (storeName === 'purchaseOrders') {
                  ...
                  if (storeName === 'productBatches') {
                    console.log(`[SYNC DEBUG] Processing batch success:`, {
                      itemId: item.id,
                      mongoId,
                      hasIdChanged,
                      syncedItemData
                  ...
                  if (hasIdChanged && deleteItem) {
                    try {
                      // CRITICAL: Await the deletion properly
                      await deleteItem(item.id);
                      console.log(`[SYNC] 🗑️ Deleted temp item ${item.id} after ID swap`);
                    } catch (delErr) {
                      console.error(`[SYNC] ❌ Failed to delete temp item ${item.id}:`, delErr);
                    }
                  }
