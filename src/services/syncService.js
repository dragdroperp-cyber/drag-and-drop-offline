...
class SyncService {
  ...
  async syncStore(storeName, getAllItems, updateItem, deleteItem = null) {
    try {
      ...
      console.log(`[SYNC] 🚀 BATCH SYNC: Sending ${cleanedItems.length} items for ${storeName}`);
        ...
        for (const syncedItemData of successItems) {
          ...
          if (originalItem.isDeleted === true && syncedItemData.action === 'deleted') {
            ...
            if (storeName === 'purchaseOrders') {
              ...
              // This often happens if the batch endpoint returns success but omits the _id for new items.
              if (!originalItem._id && !mongoId) {
                ...
                // Treat as failure (implicit) by skipping the success processing for this item
                // It will remain isSynced: false and be retried, or picked up by processSyncQueue.
                continue;
              }

              const hasIdChanged = mongoId && originalItem.id !== mongoId;
              ...
              if (hasIdChanged) {
                ...
                console.log(`[SYNC] 🔄 Swapping temp ID ${originalItem.id} with Mongo ID ${mongoId} for ${storeName}`);
              }
              ...
              // Handle ID swap cleanup
              if (hasIdChanged && deleteItem) {
                try {
                  await deleteItem(originalItem.id);
                } catch (delErr) {
                  console.error(`[SYNC] ❌ Failed to delete temp item ${originalItem.id}:`, delErr);
                }
              }
    ...
    try {
      ...
      for (const syncedData of successItems) {
        ...
        if (originalItem.isDeleted === true && syncedData.action === 'deleted') {
          ...
          delete syncedItem.isUpdate;
          if (originalItem.id !== syncedItem.id && storeFuncs.deleteItem) await storeFuncs.deleteItem(originalItem.id);
  ...
  async syncAll(getStoreFunctions = null) {
    ...
    try {
      ...
      // Sync in order: categories -> products -> customers -> orders -> transactions -> refunds -> expenses -> customerTransactions -> productBatches -> purchaseOrders
      // CRITICAL: We sync orders BEFORE productBatches to avoid double-reduction on the first sync of a new batch.
      // The order sync performs a relative reduction, while batch sync performs an absolute overwrite.
      // By processing orders first, the batch sync acts as an idempotent confirmation of the final state.
      let syncOrder = ['categories', 'products', 'customers', 'orders', 'transactions', 'refunds', 'expenses', 'customerTransactions', 'productBatches', 'purchaseOrders', 'planOrders'];
  ...
  async retryFailedSyncs(getStoreFunctions = null) {
    ...
    const results = {};
    const syncOrder = ['categories', 'products', 'productBatches', 'customers', 'orders', 'transactions', 'purchaseOrders', 'refunds', 'expenses', 'customerTransactions'];
