...
class SyncService {
  constructor() {
    ...
    this.isProductBatchSyncing = false;
    this.syncQueue = [];
  ...
  async syncItem(storeName, item, sellerId) {
    try {
      ...
      // Prevent multiple simultaneous syncs for product batches
      if (storeName === 'productBatches') {
        if (this.isProductBatchSyncing) {
          console.log(`[SYNC] Skipping product batch sync for ${item.id} - another sync in progress`);
          return { success: true, results: { success: [{ id: item.id, action: 'skipped' }], failed: [] } };
        }
        this.isProductBatchSyncing = true;
      }
      ...
      // Clear the product batch sync flag on success
      if (storeName === 'productBatches') {
        this.isProductBatchSyncing = false;
      }
      ...
      // Clear the product batch sync flag on error
      if (storeName === 'productBatches') {
        this.isProductBatchSyncing = false;
      }
        ...
        try {
          ...
          if (result && result.success === true && (result.results || result.data?.results)) {
            ...
            if (successItems.length > 0) {
              ...
              if (!syncedItemData) {
                // This specific item was not in the success list, treat as failed
                console.error(`[SYNC] Item ${item.id} not found in backend success response for ${storeName}`);
              ...
              if (item.isDeleted === true && syncedItemData.action === 'deleted') {
                ...
                if (storeName === 'purchaseOrders') {
                  ...
                  const syncedItem = {
                    ...
                    isSynced: true,
                    syncedAt: new Date().toISOString(),
                    _id: syncedItemData._id || item._id // Preserve existing _id or use new one, but don't fallback to temporary id
                  };
  ...
  async syncAll(getStoreFunctions = null) {
    ...
    try {
      ...
      // Sync in order: categories -> products -> productBatches -> customers -> orders -> transactions -> vendorOrders -> refunds -> expenses
