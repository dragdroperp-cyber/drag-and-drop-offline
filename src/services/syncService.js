...
class SyncService {
  ...
  async syncItem(storeName, item, sellerId) {
    try {
      ...
      if (storeName === 'orders') {
        console.log(`[SYNC] 📤 SENDING ORDER TO BACKEND:`, {
          sellerId,
          itemId: item.id,
    ...
    try {
      ...
      for (const item of unsyncedItems) {
        try {
          ...
          if (storeName === 'orders') {
            console.log(`[SYNC] 🧾 ORDER SYNC: Item ID: ${item.id}, Invoice: ${item.invoiceNumber}, isSynced: ${item.isSynced}`);
            console.log(`[SYNC] 🧾 ORDER SYNC: Full item data:`, JSON.stringify(item, null, 2));
  ...
  async syncAll(getStoreFunctions = null) {
    ...
    try {
      ...
      // Sync in order: categories -> products -> customers -> orders -> transactions -> vendorOrders -> refunds
      const syncOrder = ['categories', 'products', 'customers', 'orders', 'transactions', 'purchaseOrders', 'refunds'];
