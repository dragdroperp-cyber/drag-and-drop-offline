...
export const getLastSync = async (collectionName) => {
  try {
    ...
    try {
      ...
      console.log(`syncMetadata store not available, using localStorage fallback:`, idbError.message);
    }
    ...
    console.error(`Error getting last sync for ${collectionName}:`, error);
    // Fallback to localStorage
    const stored = localStorage.getItem(`sync_${collectionName}`);
...
export const setLastSync = async (collectionName, timestamp = new Date()) => {
  try {
    ...
    try {
      ...
      console.log(`syncMetadata store not available, using localStorage only:`, idbError.message);
    }
    ...
    console.error(`Error setting last sync for ${collectionName}:`, error);
    // Fallback to localStorage
...
export const updateIndexedDB = async (storeName, item) => {
  try {
    ...
    console.error(`Error updating IndexedDB for ${storeName}:`, error);
    throw error;
  }
};
...
export const deleteIndexedDB = async (storeName, itemId) => {
  try {
    ...
    console.error(`Error deleting from IndexedDB for ${storeName}:`, error);
  ...
  try {
    ...
    for (const item of updated) {
      try {
        ...
        console.error(`Error updating item ${item.id} in ${storeName}:`, error);
      }
    }
      ...
      try {
        ...
        if (itemId) {
          ...
          if (existingItem) {
            ...
            console.log(`Item ${itemId} not found in IndexedDB, skipping deletion`);
          }
        }
      } catch (error) {
        console.error(`Error deleting item ${deletedItem.id} from ${storeName}:`, error);
      }
    }
    ...
    console.error(`Error syncing collection ${storeName}:`, error);
    return {
      success: false,
  ...
  try {
    ...
    for (const storeName of collectionsToSync) {
      try {
        ...
        console.error(`Error full syncing ${storeName}:`, error);
        results[storeName] = {
          success: false,
          error: error.message
        };
