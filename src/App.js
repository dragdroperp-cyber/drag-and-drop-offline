...
const ModuleGate = ({ viewKey, children }) => {
  ...
  if (isPlanInfoLoading) {
    console.log('🚪 MODULE GATE: Allowing access while plan details load');
    return children;
  }
  ...
  // For expired plans, allow access to all pages
  if (planExpired) {
    isUnlocked = true;
  } else {
    ...
    if (viewKey === 'settings') {
      ...
      if (viewKey === 'upgrade' || viewKey === 'dashboard') {
        ...
        if (isPlanInfoLoading) {
          console.log('🚪 MODULE GATE: Allowing access while plan details load for', viewKey);
          isUnlocked = true;
        } else {
          isUnlocked = isModuleUnlocked(viewKey, state.currentPlan, state.currentPlanDetails);
          console.log('🚪 MODULE GATE: Module unlock check result:', {
            viewKey,
            isUnlocked,
            unlockedModules: state.currentPlanDetails?.unlockedModules
          });
    ...
    if (!isUnlocked && !isUpgradeRoute && !isPlanInfoLoading) {
      console.log('🚫 MODULE GATE REDIRECT: Module locked, redirecting to dashboard', {
        viewKey,
        currentPlan: state.currentPlan,
        currentPlanDetails: state.currentPlanDetails,
        unlockedModules: state.currentPlanDetails?.unlockedModules,
        isModuleUnlocked: isModuleUnlocked(viewKey, state.currentPlan, state.currentPlanDetails)
      });
  ...
  // Allow access to all pages for expired plans - backend handles operation restrictions
  // No routing-level blocking for expired plans
