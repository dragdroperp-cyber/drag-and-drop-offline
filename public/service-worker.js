        ...
        await Promise.allSettled(
          CRITICAL_ASSETS.map(url => 
            cache.add(new Request(url, { cache: 'reload' }))
              .catch(err => //(`[SW] Failed to cache ${url}:`, err))
          )
        );
...
self.addEventListener('activate', (event) => {
  //('[Service Worker] Activating version', CACHE_VERSION);
...
async function cacheAppResources() {
  try {
    ...
    //('[Service Worker] Caching app routes and resources...');
    
    // Cache all app routes (SPA routes)
    await Promise.allSettled(
      APP_ROUTES.map(route => 
        cache.add(route).catch(err => 
          //(`[SW] Failed to cache route ${route}:`, err)
        )
      )
    );
    ...
    await Promise.allSettled(
      EXTERNAL_RESOURCES.map(url => 
        fetch(url)
          ...
          .catch(err => //(`[SW] Failed to cache external ${url}:`, err))
      )
    );
    ...
    //('[Service Worker] App resources cached successfully');
  } catch (error) {
...
async function cacheAssetManifestFiles(cache) {
  try {
    ...
    //('[Service Worker] Unable to cache from asset manifest:', error);
  }
}
...
self.addEventListener('sync', (event) => {
  //('[Service Worker] Background sync:', event.tag);
