...
const Financial = () => {
  ...
  // totalPurchaseExpenses is tracked separately as "Inventory Spend"
  ...
  const exportFinancialPDF = async () => {
    try {
      ...
      pdf.text(normalizeText(getTranslation('financialManagement', state.currentLanguage)), logoX + 22, 19);

      /* -------- RIGHT META -------- */
      pdf.setFontSize(12);
      pdf.setFont('helvetica', 'bold');
      pdf.setTextColor(...COLORS.black);
      pdf.text(normalizeText(getTranslation('financialReport', state.currentLanguage)), pageWidth - margin, 14, { align: 'right' });
      ...
      const metrics = [
        { label: normalizeText(getTranslation('revenue', state.currentLanguage)), value: formatCurrencySmart(totalRevenue, state.currencyFormat).replace(/₹/g, 'Rs ') },
        { label: normalizeText(getTranslation('expenses', state.currentLanguage)), value: formatCurrencySmart(totalExpenses, state.currencyFormat).replace(/₹/g, 'Rs ') },
        { label: normalizeText(getTranslation('netProfit', state.currentLanguage)), value: formatCurrencySmart(netProfit, state.currencyFormat).replace(/₹/g, 'Rs ') }
      ];
    ...
    <div className="space-y-6 sm:space-y-8 fade-in-up">
        ...
        <div className="mt-4 sm:mt-0 flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={`flex items-center justify-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors shadow-sm whitespace-nowrap ${isPlanExpired(state)
            ...
            <div className="relative" ref={exportMenuRef}>
              <button
                ...
                className="px-4 py-2 bg-white dark:bg-slate-800 text-gray-700 dark:text-slate-200 rounded-lg text-sm font-medium flex items-center justify-center gap-2 transition-colors whitespace-nowrap shadow-sm border border-slate-200 dark:border-slate-700 hover:bg-gray-50 dark:hover:bg-slate-700"
              >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
        {[
          ...
          const getTextClass = (c) => {
            // STRICT GLOBAL RULE: 
            // Emerald (Green) = Positive/Income
            // Rose (Red) = Negative/Expense
            // Others = Neutral (White/Black)
      ...
      {/* Delete Confirmation Modal */}
      {
        showDeleteConfirmModal && (
          <div className="fixed inset-0 z-[1200] flex items-end md:items-center justify-center bg-black/60" onClick={() => setShowDeleteConfirmModal(false)}>
            <div className="bg-white dark:bg-slate-800 rounded-none md:rounded-[2rem] shadow-2xl p-6 text-center animate-slideUp border border-gray-100 dark:border-slate-700 w-full md:max-w-sm h-full md:h-auto fixed inset-0 md:relative md:inset-auto flex flex-col justify-center items-center" onClick={e => e.stopPropagation()}>
      ...
      {
        showCustomDateModal && (
          <div className="fixed inset-0 z-[200] flex items-end md:items-center justify-center bg-black/50 animate-fadeIn" onClick={() => setShowCustomDateModal(false)}>
            <div className="bg-white dark:bg-slate-800 w-full md:max-w-sm rounded-none md:rounded-2xl shadow-xl overflow-hidden animate-slideUp fixed inset-0 md:relative md:inset-auto h-full md:h-auto flex flex-col" onClick={e => e.stopPropagation()}>
              ...
              <div className="p-6 space-y-4">
                ...
                <div className="pt-2 flex flex-col gap-2">
                  <button
                    ...
                    className="w-full py-3 bg-slate-900 hover:bg-slate-800 dark:bg-white dark:hover:bg-gray-100 text-white dark:text-slate-900 font-bold rounded-xl transition-all shadow-lg shadow-indigo-200 dark:shadow-none"
                  >
