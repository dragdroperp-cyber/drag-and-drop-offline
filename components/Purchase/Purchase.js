    ...
    try {
      ...
      try {
        ...
        if (res.ok) {
          ...
          pdf.addImage(base64, 'PNG', margin, 6, 16, 16);
        }
      } catch (e) { }
      ...
      safeDrawText(pdf, 'Drag & Drop', margin + 22, 15, { color: '#2F3C7E', fontSize: 16 });

      pdf.setFontSize(9);
      pdf.setFont('helvetica', 'normal');
      pdf.setTextColor(...COLORS.gray);
      safeDrawText(pdf, getTranslation('purchaseOrders', state.currentLanguage), margin + 22, 19, { color: '#787878', fontSize: 9 });

      /* -------- RIGHT META -------- */
      pdf.setFontSize(12);
      pdf.setFont('helvetica', 'bold');
      pdf.setTextColor(...COLORS.black);
      safeDrawText(pdf, getTranslation('purchaseReport', state.currentLanguage) || 'Purchase Report', pageWidth - margin, 14, { align: 'right', color: '#000000', fontSize: 12 });

      pdf.setFontSize(9);
      pdf.setFont('helvetica', 'normal');
      pdf.setTextColor(...COLORS.gray);
      safeDrawText(pdf, `${getTranslation('generated', state.currentLanguage)}: ${formatDate(new Date())}`, pageWidth - margin, 19, { align: 'right', color: '#787878', fontSize: 9 });
      ...
      const stats_totalValue = filteredOrders.reduce((sum, o) => sum + (o.status === 'completed' ? (Number(o.totalValue || o.total || 0)) : 0), 0);
      const metrics = [
        { label: getTranslation('totalValue', state.currentLanguage) || 'Total Value', value: formatCurrencySmart(stats_totalValue, state.currencyFormat) },
        { label: getTranslation('completed', state.currentLanguage), value: filteredOrders.filter(o => o.status === 'completed').length.toString() },
        { label: getTranslation('pending', state.currentLanguage), value: filteredOrders.filter(o => o.status === 'pending').length.toString() },
      ...
      metrics.forEach((m, i) => {
        ...
        pdf.setFont('helvetica', 'bold');
        pdf.setTextColor(...COLORS.primary);
        safeDrawText(pdf, String(m.value), x + 4, startY + 16, { color: '#2F3C7E', fontSize: 16 });
      });
      ...
      pdf.setFont('helvetica', 'bold');
      pdf.setTextColor(...COLORS.primary);
      safeDrawText(pdf, getTranslation('orderSummary', state.currentLanguage), margin, y, { color: '#2F3C7E', fontSize: 15 });
      ...
      const colWeights = [15, 30, 15, 15, 10, 15]; // Sum = 100
      ...
      headers.forEach((h, i) => {
        ...
        safeDrawText(pdf, h, x + colW_adjusted[i] / 2, y + 6, { align: 'center', color: '#2F3C7E', fontSize: 11 });
      });
      ...
      filteredOrders.forEach((order, index) => {
        ...
        const rowData = [
          ...
          (order.items || order.batches || []).length.toString(),
          formatCurrencySmart(order.totalValue || order.total || 0, state.currencyFormat)
        ];

        rowData.forEach((val, j) => {
          const x = margin + colW_adjusted.slice(0, j).reduce((a, b) => a + b, 0);
          safeDrawText(pdf, String(val), x + colW_adjusted[j] / 2, y + 6.5, { align: 'center', color: '#000000', fontSize: 10 });
        });
        y += rowH;
      });
      ...
      for (let i = 1; i <= pageCount; i++) {
        ...
        safeDrawText(pdf, state.currentUser?.shopName || 'Store', pageWidth - margin, pageHeight - 10, { align: 'right', color: '#787878', fontSize: 8 });
      }
                ...
                <tr key={order.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors">
                  <td className="px-4 py-4 align-top">
                    <div className="flex items-center gap-3">
                      <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-indigo-100 dark:bg-indigo-900/40 text-slate-900 dark:text-slate-100">
