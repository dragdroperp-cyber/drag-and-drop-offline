...
const SalesOrderHistory = () => {
  ...
  const exportToPDF = async () => {
    try {
      ...
      safeDrawText(doc, 'Drag & Drop', logoX + 22, 15, { color: '#2F3C7E', fontSize: 16 });

      doc.setFontSize(9);
      doc.setFont('helvetica', 'normal');
      doc.setTextColor(...COLORS.gray);
      safeDrawText(doc, getTranslation('salesOrderHistory', state.currentLanguage), logoX + 22, 19, { color: '#787878', fontSize: 9 });

      /* -------- RIGHT META -------- */
      doc.setFontSize(12);
      doc.setFont('helvetica', 'bold');
      doc.setTextColor(...COLORS.black);
      safeDrawText(doc, getTranslation('salesOrderHistory', state.currentLanguage), pageWidth - margin, 14, { align: 'right', color: '#000000', fontSize: 12 });

      doc.setFontSize(9);
      doc.setFont('helvetica', 'normal');
      doc.setTextColor(...COLORS.gray);

      let periodText = `${getTranslation('generated', state.currentLanguage) || 'Generated'}: ${formatDate(new Date())}`;

      if (filterDateRange === 'today') {
        periodText = `${getTranslation('period', state.currentLanguage)}: ${getTranslation('today', state.currentLanguage)}`;
      } else if (filterDateRange === 'week') {
        periodText = `${getTranslation('period', state.currentLanguage)}: ${getTranslation('last7Days', state.currentLanguage)}`;
      } else if (filterDateRange === 'month') {
        periodText = `${getTranslation('period', state.currentLanguage)}: ${getTranslation('last30Days', state.currentLanguage)}`;
      } else if (filterDateRange === 'custom' && customDateRange.start && customDateRange.end) {
        periodText = `${getTranslation('period', state.currentLanguage)}: ${formatDate(customDateRange.start)} - ${formatDate(customDateRange.end)}`;
      }

      safeDrawText(doc, periodText, pageWidth - margin, 19, { align: 'right', color: '#787878', fontSize: 9 });
      ...
      const metrics = [
        { label: getTranslation('totalSales', state.currentLanguage), value: formatCurrencySmart(totalValue, state.currencyFormat) },
        { label: getTranslation('received', state.currentLanguage), value: formatCurrencySmart(paidValue, state.currencyFormat) },
        { label: getTranslation('due', state.currentLanguage), value: formatCurrencySmart(dueValue, state.currencyFormat) },
        { label: getTranslation('totalOrders', state.currentLanguage), value: filteredOrders.length.toString() }
      ];
      ...
      if (reportSettings.showSummary !== false) {
        ...
        metrics.forEach((m, i) => {
          ...
          doc.rect(x + 0.5, y + 0.5, cardW, cardH, 'F');
          ...
          // Side accent bar
          doc.setFillColor(...COLORS.primary);
          doc.rect(x + cardW - 1.5, y, 1.5, cardH, 'F');

          doc.setFontSize(8);
          doc.setTextColor(...COLORS.gray);
          safeDrawText(doc, m.label.toUpperCase(), x + 4, y + 7, { color: `rgb(${COLORS.gray.join(',')})`, fontSize: 8 });

          doc.setFontSize(14);
          doc.setFont('helvetica', 'bold');
          doc.setTextColor(...COLORS.black);
          safeDrawText(doc, String(m.value), x + 4, y + 16, { color: '#000000', fontSize: 13 });
        });
        y += cardH + 16;
      } else {
        y += 4;
      ...
      doc.setFont('helvetica', 'bold');
      doc.setTextColor(...COLORS.primary);
      safeDrawText(doc, getTranslation('orderDetails', state.currentLanguage), margin, y, { color: '#2F3C7E', fontSize: 15 });

      /* ================= TABLE ================= */
      const headers = [
        getTranslation('orderId', state.currentLanguage) || 'Order ID',
        ...
        getTranslation('amount', state.currentLanguage),
        getTranslation('payment', state.currentLanguage),
      ...
      headers.forEach((h, i) => {
        ...
        safeDrawText(doc, h, x + colW_adjusted[i] / 2, y + 6, { align: 'center', color: '#ffffff', fontSize: 10 });
      });
      ...
      filteredOrders.forEach((order, index) => {
        ...
        if (y + rowH > pageHeight - 20) {
          ...
          headers.forEach((h, i) => {
            const x = margin + colW_adjusted.slice(0, i).reduce((a, b) => a + b, 0);
            safeDrawText(doc, h, x + colW_adjusted[i] / 2, y + 6, { align: 'center', color: '#2F3C7E', fontSize: 11 });
          });
        ...
        const row = [
          ...
          formatDate(order.createdAt || order.date),
          (order.customerName || getTranslation('walkInCustomer', state.currentLanguage)).substring(0, 15),
          (order.items || []).length.toString(),
          formatCurrencySmart(order.totalAmount || 0, state.currencyFormat),
          getPaymentMethodLabel(order.paymentMethod, order.splitPaymentDetails) || getTranslation('cash', state.currentLanguage),
          order.status || getTranslation('completed', state.currentLanguage)
        ];

        row.forEach((val, j) => {
          const x = margin + colW_adjusted.slice(0, j).reduce((a, b) => a + b, 0);
          safeDrawText(doc, String(val), x + colW_adjusted[j] / 2, y + 5.5, { align: 'center', color: '#000000', fontSize: 10 });
        });
      ...
      if (pageCount > 1) { // Conditional Footer
        for (let i = 1; i <= pageCount; i++) {
          ...
          doc.setFontSize(8);
          doc.setTextColor(...COLORS.gray);
          safeDrawText(doc, `${getTranslation('page', state.currentLanguage)} ${i} ${getTranslation('ofPage', state.currentLanguage)} ${pageCount} `, margin, pageHeight - 10, { color: '#787878', fontSize: 8 });
          safeDrawText(doc,
      ...
      if (window.showToast) {
        window.showToast(getTranslation('exportPDFSuccess', state.currentLanguage) || 'Sales Report exported as PDF.', 'success');
      }
      setShowExportMenu(false);
    } catch (error) {
      if (window.showToast) {
        window.showToast(getTranslation('exportError', state.currentLanguage) || 'Error generating PDF. Please try again.', 'error');
      }
    }
  };
