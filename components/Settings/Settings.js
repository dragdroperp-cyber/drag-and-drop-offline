            ...
            {isEditing ? (
              <div className="relative">
                <select
                  ...
                  className={`w-full px-3 py-3 border rounded-xl focus:ring-2 focus:ring-slate-900 focus:border-blue-500 dark:bg-slate-700 dark:border-slate-600 dark:text-white appearance-none pr-10 ${errors.businessType ? 'border-red-400' : 'border-gray-300'}`}
                >
            ...
            {isEditing ? (
              <div className="relative">
                <select
                  ...
                  className={`w-full px-3 py-3 border rounded-xl focus:ring-2 focus:ring-slate-900 focus:border-blue-500 dark:bg-slate-700 dark:border-slate-600 dark:text-white appearance-none pr-10 ${errors.state ? 'border-red-400' : 'border-gray-300'}`}
                >
          ...
          {isEditing ? (
            <div className="relative">
              <select
                ...
                className={`w-full px-3 py-3 border rounded-xl focus:ring-2 focus:ring-slate-900 focus:border-blue-500 dark:bg-slate-700 dark:border-slate-600 dark:text-white appearance-none pr-10 ${errors.gender ? 'border-red-400' : 'border-gray-300'}`}
              >
                <option value="">{getTranslation('selectGender', state.currentLanguage)}</option>
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 p-6 transition-colors duration-300">
      <div className="flex items-center space-x-3 mb-6">
        ...
        <h2 className="text-xl font-semibold text-gray-900 dark:text-white">{getTranslation('notificationsAndAlerts', state.currentLanguage)}</h2>
      </div>

      <div className="space-y-6">
        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">
            {getTranslation('lowStockThreshold', state.currentLanguage)}
          </label>
          <div className="flex items-center space-x-3">
            <input
              type="text"
              ...
              onChange={(e) => {
                ...
                if (value === '' || /^[0-9]*$/.test(value)) {
                  setSettings(prev => ({ ...prev, lowStockThreshold: value }));
                }
              }}
              className="w-24 px-3 py-2 border border-gray-300 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-green-500 dark:bg-slate-700 dark:border-slate-600 dark:text-white"
            />
            <span className="text-sm text-gray-600 dark:text-slate-400">{getTranslation('unitsRemaining', state.currentLanguage)}</span>
          </div>
          <p className="text-xs text-gray-500 dark:text-slate-500 mt-1">{getTranslation('lowStockDesc', state.currentLanguage)}</p>
        </div>

        <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">
            {getTranslation('expiryAlertThreshold', state.currentLanguage)}
          </label>
          <div className="flex items-center space-x-3">
            <input
              type="text"
              ...
              onChange={(e) => {
                const value = e.target.value;
                if (value === '' || /^[0-9]*$/.test(value)) {
                  setSettings(prev => ({ ...prev, expiryDaysThreshold: value }));
                }
              }}
              className="w-24 px-3 py-2 border border-gray-300 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-green-500 dark:bg-slate-700 dark:border-slate-600 dark:text-white"
            />
            <span className="text-sm text-gray-600 dark:text-slate-400">{getTranslation('daysBeforeExpiry', state.currentLanguage)}</span>
          </div>
          <p className="text-xs text-gray-500 dark:text-slate-500 mt-1">{getTranslation('expiryDesc', state.currentLanguage)}</p>
        </div>

        <button
          onClick={handleSave}
          disabled={isPlanExpired(state)}
          className={`w-full py-2 px-4 rounded-lg transition-colors flex items-center justify-center space-x-2 text-white ${isPlanExpired(state) ? 'bg-gray-400 cursor-not-allowed' : 'bg-green-600 hover:bg-green-700'}`}
        >
          <Save className="h-4 w-4" />
          <span>{getTranslation('saveNotificationSettings', state.currentLanguage)}</span>
        </button>
      </div>
    </div>
  );
    ...
    <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 p-6 transition-colors duration-300">
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-sm font-medium text-gray-900 dark:text-white">{getTranslation('voiceAssistant', state.currentLanguage)}</h3>
            <p className="text-xs text-gray-500 dark:text-slate-400">{getTranslation('voiceAssistantDesc', state.currentLanguage)}</p>
          </div>
          <button
            onClick={() => dispatch({
            ...
            className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${state.voiceAssistantEnabled ? 'bg-slate-900' : 'bg-gray-200 dark:bg-slate-700'
              }`}
          >
            <span
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 p-6 transition-colors duration-300">
      <div className="text-center">
        ...
        <p className="text-xs text-gray-400 dark:text-slate-500 mt-1">{getTranslation('serviceWorker', state.currentLanguage)}: {swStatus}</p>
        ...
        >
          {getTranslation('checkForUpdates', state.currentLanguage)}
        </button>
